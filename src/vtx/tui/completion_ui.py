...
class CompletionUIMixin:
    ...
    def _apply_selection_mode_choice(
        ...
        match selection_mode:
            ...
            case SelectionMode.LOGIN_METHOD:
                self._select_login_method(item.value)
            case SelectionMode.LOGIN:
                self._select_login_provider(item.value)
            case SelectionMode.LOGIN_API_KEY:
                self._select_apikey_provider(item.value)
            case SelectionMode.LOGIN_KEYLESS:
                self._select_keyless_provider(item.value)
