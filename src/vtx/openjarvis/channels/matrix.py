...
def _build_matrix_text_content(
    text: str, event_id: str | None = None, thread_relates_to: dict[str, object] | None = None
) -> dict[str, object]:
    """
    ...
    if event_id:
        new_content: dict[str, object] = {"body": text, "msgtype": "m.text"}
...
class MatrixChannel(BaseChannel):
    ...
    def _write_session_to_disk(self, resp: LoginResponse) -> None:
        ...
        session = {"access_token": resp.access_token, "device_id": resp.device_id}
        try:
    ...
    def _build_outbound_attachment_content(
        *,
        ...
        prefix = mime.split("/")[0]
        msgtype = {"image": "m.image", "audio": "m.audio", "video": "m.video"}.get(
            prefix, "m.file"
        )
    ...
    async def send_delta(
        ...
        if meta.get("_stream_end"):
            ...
            content = _build_matrix_text_content(
                buf.text, buf.event_id, thread_relates_to=relates_to
            )
    ...
    def _register_event_callbacks(self) -> None:
        ...
        self.client.add_event_callback(
            cast(Callable[[MatrixRoom, Event], Awaitable[None]], self._on_message),
            cast(type[Event], RoomMessageText),
        )
        self.client.add_event_callback(
            cast(Callable[[MatrixRoom, Event], Awaitable[None]], self._on_media_message),
            cast(tuple[type[Event], None], MATRIX_MEDIA_EVENT_FILTER),
        )
        self.client.add_event_callback(
            cast(Callable[[MatrixRoom, Event], Awaitable[None]], self._on_room_invite),
            cast(type[Event], InviteEvent),
        )
    ...
    def _register_to_device_callbacks(self) -> None:
        if self.config.e2ee_enabled and self.config.sas_verification:
            ...
            self.client.add_to_device_callback(
                cast(Callable[[ToDeviceEvent], None], self._on_key_verification_event),
                (KeyVerificationEvent,),
            )
    ...
    async def _handle_key_verification_event(self, event: KeyVerificationEvent) -> None:
        ...
        if isinstance(event, KeyVerificationStart):
            if "emoji" not in (getattr(event, "short_authentication_string", None) or []):
                self.logger.info(
                    "Ignoring Matrix SAS verification from {} without emoji support", sender
                )
                return
        ...
        if isinstance(event, KeyVerificationCancel):
            self.logger.info(
                "Matrix SAS verification cancelled by {}: {}", sender, getattr(event, "reason", "")
            )
    ...
    async def _fetch_media_attachment(
        ...
        if encrypted:
            data = self._decrypt_media_bytes(event, downloaded)
            if data is None:
                return None, fail
        else:
            data = downloaded
