...
try:
    ...
    CallbackMessage: Any = None
    AckMessage: Any = None
    ChatbotMessage: Any = None


class AgeniteClawDingTalkHandler(CallbackHandler):
    """
    Standard DingTalk Stream SDK Callback Handler.
    Parses incoming messages and forwards them to the AgeniteClaw channel.
    """
    ...
    async def process(self, message: CallbackMessage):
        ...
        try:
            ...
            # Forward to AgeniteClaw via _on_message (non-blocking).
...
class DingTalkChannel(BaseChannel):
    ...
    _IMAGE_EXTS = {".jpg", ".jpeg", ".png", ".gif", ".bmp", ".webp"}  # noqa
    _AUDIO_EXTS = {".amr", ".mp3", ".wav", ".ogg", ".m4a", ".aac"}  # noqa
    _VIDEO_EXTS = {".mp4", ".mov", ".avi", ".mkv", ".webm"}  # noqa
    _ZIP_BEFORE_UPLOAD_EXTS = {".htm", ".html"}  # noqa
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                "Initializing Stream Client with Client ID: {}...", self.config.client_id
            )
            credential = Credential(self.config.client_id, self.config.client_secret)
            self._client = DingTalkStreamClient(credential)

            # Register standard handler
            handler = AgeniteClawDingTalkHandler(self)
    ...
    def _normalize_upload_payload(
        self, filename: str, data: bytes, content_type: str | None
    ) -> tuple[bytes, str, str | None]:
        ext = Path(filename).suffix.lower()
        if ext in self._ZIP_BEFORE_UPLOAD_EXTS or content_type == "text/html":
            self.logger.info(
                "does not accept raw HTML attachments, zipping {} before upload", filename
            )
    ...
    def _next_remote_media_url(self, current_url: str, location: str | None) -> str | None:
        ...
        if not self._redirect_host_allowed(current_url, next_url):
            self.logger.warning(
                "media download cross-host redirect refused ref={} next={}", current_url, next_url
            )
            return None
    ...
    async def _fetch_remote_media_bytes(self, media_ref: str) -> tuple[bytes | None, str | None]:
        ...
        try:
            ...
            for _ in range(DINGTALK_MAX_REMOTE_MEDIA_REDIRECTS + 1):
                ...
                if resp.status_code >= 400:
                    self.logger.warning(
                        "media download failed status={} ref={}", resp.status_code, current_url
                    )
                    return None, None
    ...
    async def _upload_media(
        self, token: str, data: bytes, media_type: str, filename: str, content_type: str | None
    ) -> str | None:
        if not self._http:
            return None
    ...
    async def _send_batch_message(
        ...
        try:
            ...
            if resp.status_code != 200:
                self.logger.error(
                    "send failed msgKey={} status={} body={}",
                    msg_key,
                    resp.status_code,
                    body[:500],
                )
    ...
    async def _send_markdown_text(self, token: str, chat_id: str, content: str) -> bool:
        return await self._send_batch_message(
            token, chat_id, "sampleMarkdown", {"text": content, "title": "AgeniteClaw Reply"}
        )
    ...
    async def _send_media_ref(self, token: str, chat_id: str, media_ref: str) -> bool:
        ...
        if upload_type == "image" and self._is_http_url(media_ref):
            ok = await self._send_batch_message(
                token, chat_id, "sampleImageMsg", {"photoURL": media_ref}
            )
            if ok:
                return True
        ...
        if upload_type == "image":
            ...
            ok = await self._send_batch_message(
                token, chat_id, "sampleImageMsg", {"photoURL": media_id}
            )
            if ok:
                return True
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        for media_ref in msg.media or []:
            ...
            await self._send_markdown_text(
                token, msg.chat_id, f"[Attachment send failed: {filename}]"
            )
    ...
    async def _on_message(
        ...
        """Handle incoming message (called by AgeniteClawDingTalkHandler).
