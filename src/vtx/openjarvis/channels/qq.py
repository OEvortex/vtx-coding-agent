...
- Publish to AgeniteClaw bus via BaseChannel._handle_message()
...
class QQChannel(BaseChannel):
    ...
    def _init_media_root(self) -> Path:
        ...
        if self.config.media_dir:
            ...
            try:
                ...
                root = Path.home() / ".agenite" / "mini" / "media" / "qq"
        else:
            root = Path.home() / ".agenite" / "mini" / "media" / "qq"
    ...
    async def start(self) -> None:
        ...
        self._client = cast(Any, _make_bot_class(self)())  # ty: ignore[missing-argument]
    ...
    async def _read_media_bytes(self, media_ref: str) -> tuple[bytes | None, str | None]:
        ...
        try:
            async with self._http.get(media_ref, allow_redirects=True) as resp:
                if resp.status >= 400:
                    self.logger.warning(
                        "outbound media download failed status={} url={}", resp.status, media_ref
                    )
                    return None, None
    ...
    async def _post_base64file(
        ...
        if isinstance(result, dict) and "file_info" in result:
            return cast(Media, {"file_info": result["file_info"]})  # type: ignore[typeddict-item]
        return result
    ...
    async def _on_message(self, data: C2CMessage | GroupMessage, is_group: bool = False) -> None:
        ...
        try:
            if is_group:
                ...
                user_id = str(getattr(data.author, "member_openid", ""))
                chat_type = "group"
            else:
                chat_id = str(
            ...
            if not self.is_allowed(user_id):
                if not is_group:
                    await self._handle_message(
                        sender_id=user_id, chat_id=chat_id, content="", is_dm=True
                    )
                return
            ...
            await self._handle_message(
                ...
                metadata={"message_id": data.id, "attachments": att_meta},
                is_dm=not is_group,
            )
        except Exception:
    ...
    async def _handle_attachments(
        self, attachments: list[BaseMessage._Attachments]
        ...
        for att in attachments:
            ...
            att_meta.append(
                {"url": url, "filename": filename, "content_type": ctype, "saved_path": local_path}
            )
    ...
    async def _download_to_media_dir_chunked(
        ...
        try:
            async with self._http.get(
                ...
                def _open_tmp(path: Path):
                    path.parent.mkdir(parents=True, exist_ok=True)
                    return open(path, "wb")

                f = await asyncio.to_thread(_open_tmp, tmp_path)
                try:
                    async for chunk in resp.content.iter_chunked(chunk_size):
                        ...
                        if downloaded > max_bytes:
                            self.logger.warning(
                                "download exceeded max_bytes={} url={} -> abort", max_bytes, url
                            )
                            return None
