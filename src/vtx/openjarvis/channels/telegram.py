...
class TelegramChannel(BaseChannel):
    ...
    BOT_COMMANDS = [
        ...
        BotCommand("richtext", "Toggle rich message formatting on/off"),
    ]

    # Regex for slash commands routed to AgentLoop via ``_forward_command``.
    # Hyphenated ``dream-*`` commands stay on a separate handler (below).
    TELEGRAM_BUS_SLASH_COMMAND_RE = re.compile(
        r"^/(?:new|stop|restart|status|dream|history|goal|pairing|model|skill|richtext)(?:@\w+)?(?:\s+.*)?$"
    )
    ...
    def __init__(self, config: Any, bus: MessageBus):
        ...
        self._rich_draft_counter = 0  # Monotonic counter for draft IDs
    ...
    async def start(self) -> None:
        ...
        # Add command handlers (using Regex to support @username suffixes
        # before bot initialization)
        self._app.add_handler(MessageHandler(filters.Regex(r"^/start(?:@\w+)?$"), self._on_start))
        self._app.add_handler(
            MessageHandler(filters.Regex(r"^/richtext(?:@\w+)?\s+(on|off)$"), self._on_richtext)
        )
        self._app.add_handler(
            MessageHandler(
                filters.Regex(TelegramChannel.TELEGRAM_BUS_SLASH_COMMAND_RE), self._forward_command
            )
        )
        self._app.add_handler(
            MessageHandler(
    ...
    async def _try_send_rich(
        ...
        payload: dict[str, Any] = {"chat_id": chat_id, "rich_message": {"markdown": content}}
        ...
        try:
            await self._call_with_retry(
                self._app.bot.do_api_request, "sendRichMessage", api_kwargs=payload
            )
            return True
        except BadRequest as exc:
            if self._is_rich_capability_error(exc):
                self.logger.debug("sendRichMessage not available, skipping")
            else:
    ...
    async def _try_send_rich_draft(
        ...
        """Attempt sendRichMessageDraft (Bot API 10.1 streaming). Returns True on success."""
        if not self._app:
            return False
        ...
        try:
            await self._call_with_retry(
                self._app.bot.do_api_request, "sendRichMessageDraft", api_kwargs=payload
            )
            return True
        except BadRequest as exc:
            if self._is_rich_capability_error(exc):
                self.logger.debug("sendRichMessageDraft not available, skipping")
            else:
            ...
            is_timeout = "timed out" in err_str or isinstance(exc, TimedOut)
            if is_timeout:
                self.logger.debug("sendRichMessageDraft timeout, falling back")
                return False
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        if msg.content and msg.content != "[empty message]":
            ...
            if not render_as_blockquote and self.config.rich_messages:
    ...
    async def send_delta(
        ...
        if meta.get("_stream_end"):
            ...
            # Finalize rich draft if streaming draft was used.
            if buf.draft_id is not None and self.config.rich_messages:
                rich_ok = await self._try_send_rich(
                    int_chat_id, raw_text, None, thread_kwargs, None
                )
                if rich_ok:
        ...
        if buf.message_id is None:
            ...
            if self.config.rich_messages:
                self._rich_draft_counter += 1
                draft_id = self._rich_draft_counter
                rich_ok = await self._try_send_rich_draft(
                    int_chat_id, preview, draft_id, thread_kwargs=thread_kwargs
                )
                if rich_ok:
                    buf.message_id = 0
                    buf.draft_id = draft_id
                    buf.last_edit = now
                    return
            try:
                sent = await self._call_with_retry(
                    self._app.bot.send_message, chat_id=int_chat_id, text=preview, **thread_kwargs
                )
            ...
            if len(buf.text) > TELEGRAM_MAX_MESSAGE_LEN:
                ...
                return
            preview = _strip_md_block(buf.text)
            if self.config.rich_messages and buf.draft_id is not None:
                rich_ok = await self._try_send_rich_draft(
                    int_chat_id, preview, buf.draft_id, thread_kwargs=thread_kwargs
                )
                if rich_ok:
                    buf.last_edit = now
                    return
    ...
    async def _flush_stream_overflow(
        ...
        for chunk in chunks[1:-1]:
            if self._app is None:
                return
            await self._call_with_retry(
                self._app.bot.send_message, chat_id=chat_id, text=chunk, **thread_kwargs
            )
        tail = chunks[-1]
        if self._app is None:
            return
        sent = await self._call_with_retry(
            self._app.bot.send_message, chat_id=chat_id, text=tail, **thread_kwargs
        )
        buf.message_id = sent.message_id
        buf.text = tail
    ...
    async def _on_richtext(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Handle /richtext on|off command to toggle rich message formatting."""
        if not update.message or not update.effective_user:
            return
        user = update.effective_user
        sender_id = self._sender_id(user)
        ...
        match = re.search(r"/richtext(?:@\w+)?\s+(on|off)", text, re.IGNORECASE)
        if not match:
            return

        setting = match.group(1).lower() == "on"
        self.config.rich_messages = setting
        status = "enabled" if setting else "disabled"
        await update.message.reply_text(f"Rich message formatting {status}.")
    ...
    async def _download_message_media(
        ...
        if not media_file or not self._app or not media_type:
            return [], []
        try:
    ...
    async def _is_group_message_for_bot(self, message) -> bool:
        ...
        if bot_username:
            ...
            if self._has_mention_entity(
                text, getattr(message, "entities", None), bot_username, bot_id
            ):
                return True
            if self._has_mention_entity(
                caption, getattr(message, "caption_entities", None), bot_username, bot_id
            ):
                return True
    ...
    async def _remove_reaction(self, chat_id: str, message_id: int) -> None:
        ...
        try:
            await self._app.bot.set_message_reaction(
                chat_id=int(chat_id), message_id=message_id, reaction=[]
            )
        except Exception as e:
    ...
    def _get_extension(
        self, media_type: str, mime_type: str | None, filename: str | None = None
    ) -> str:
