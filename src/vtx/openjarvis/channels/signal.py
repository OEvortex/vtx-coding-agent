...
def _markdown_to_signal(text: str) -> tuple[str, list[str]]:
    ...
    def transform(
        pattern: re.Pattern, make_runs: Callable[[re.Match, frozenset[str]], list[_Run]]
    ) -> None:
        new_runs: list[_Run] = []
        for run in runs:
            if run.opaque:
        ...
        while self._running:
            try:
                ...
                try:
                    ...
                    raise ConnectionRefusedError(f"signal-cli daemon not responding: {e}")  # noqa: B904
        ...
        async with self._safe_handle("receive notification", params):
            ...
            if data_message:
                await self._handle_data_message(
                    sender_id, sender_number, data_message, source_name
                )
    ...
    def _check_inbound_policy(
        ...
        *,
        ...
        if not self.config.dm.enabled:
            ...
            return False, chat_id
        if self.config.dm.policy == "allowlist":  # noqa
