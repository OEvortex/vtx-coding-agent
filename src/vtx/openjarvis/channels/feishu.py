...
def _poll_registration(
    *, device_code: str, interval: int, expire_in: int, domain: str = "feishu"
) -> dict | None:
    ...
    while time.monotonic() < deadline:
        ...
        try:
            res = _post_registration(
                base_url, {"action": "poll", "device_code": device_code, "tp": "ob_app"}
            )
        except Exception:
            time.sleep(interval)
            continue
...
def _qr_register_inner(*, initial_domain: str) -> dict | None:
...
class FeishuChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        builder = self._register_optional_event(
            builder, "register_p2_im_chat_member_bot_added_v1", lambda _: None
        )
        builder = self._register_optional_event(
            builder, "register_p2_im_chat_member_bot_deleted_v1", lambda _: None
        )
            ...
            for el in elements:
                if el.get("tag") == "markdown":
                    ...
                    if isinstance(content_val, str):
                        el["content"] = content_val.replace(f"\x00CODE{i}\x00", cb)
    ...
    async def send_delta(
        ...
        if meta.get("_stream_end"):
            ...
            for chunk in self._split_elements_by_table_limit(self._build_card_elements(buf.text)):
                card = json.dumps(
                    {"config": {"wide_screen_mode": True}, "elements": chunk}, ensure_ascii=False
                )
        ...
        if buf.card_id is None:
            ...
            card_id = await loop.run_in_executor(
                ...
                lambda: self._create_streaming_card_sync(
                    rid_type, chat_id, reply_msg_id, reply_in_thread=use_reply_in_thread
                ),
            )
            if card_id:
    ...
    async def _on_message(self, data: P2ImMessageReceiveV1) -> None:
        ...
        try:
            ...
            sender_id = sender_id_obj.open_id if sender_id_obj else "unknown"
            ...
            if not self.is_allowed(str(sender_id)):
                if chat_type == "p2p":
                    await self._handle_message(
                        sender_id=str(sender_id), chat_id=str(sender_id), content="", is_dm=True
                    )
                return
            ...
            try:
                content_json = json.loads(str(message.content)) if message.content else {}
            except json.JSONDecodeError:
                content_json = {}
