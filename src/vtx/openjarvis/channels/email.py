...
class EmailConfig(Base):
    ...
    # Attachment handling — set allowed types to enable (e.g. ["application/pdf", "image/*"], or ["*"] for all)  # noqa: E501
...
class EmailChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        while self._running:
            try:
                inbound_items, skipped_uids = await asyncio.to_thread(lambda: [])
                should_apply_post_action = False
        ...
        for attempt in range(2):
            try:
                self._fetch_messages_once(
                    search_criteria, mark_seen, dedupe, limit, messages, skipped_uids, cycle_uids
                )
    ...
    def _apply_post_action(self, client: Any, uid: str, features: _ServerFeatures) -> None:
    ...
    def _extract_attachments(
        cls, msg: Any, uid: str, *, allowed_types: list[str], max_size: int, max_count: int
    ) -> list[Path]:
        ...
        for part in msg.walk():
            ...
            if len(payload) > max_size:
                logger.warning(
                    "Attachment skipped: size {} exceeds limit {}", len(payload), max_size
                )
                continue
