...
class MSTeamsChannel(BaseChannel):
    ...
    def __init__(self, config: Any, bus: MessageBus):
        ...
        with self._refs_guard:
            prune_result = self._prune_conversation_refs()
            if prune_result:
    ...
    async def start(self) -> None:
        ...
        if not MSTEAMS_AVAILABLE:
            self.logger.error("PyJWT not installed. Run: pip install agenite-claw[msteams]")
            return
        ...
        class Handler(BaseHTTPRequestHandler):
            def do_POST(self) -> None:
                ...
                if channel.config.validate_inbound_auth:
                    ...
                    validate_fut = asyncio.run_coroutine_threadsafe(
                        channel._validate_inbound_auth(auth_header, payload), channel._loop
                    )
                    try:
                        validate_fut.result(timeout=15)
                    except Exception as auth_err:
                        channel.logger.warning("Inbound auth validation failed: {}", auth_err)
                ...
                try:
                    ...
                    fut = asyncio.run_coroutine_threadsafe(
                        channel._handle_activity(payload), channel._loop
                    )
                    fut.result(timeout=15)
                except Exception as act_err:
                    channel.logger.warning("Activity handling failed: {}", act_err)
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        payload = {"type": "message", "text": msg.content or " "}
        if use_thread_reply:
    ...
    async def _handle_activity(self, activity: dict[str, Any]) -> None:
        ...
        if not self._is_trusted_service_url(service_url):
            self.logger.warning(
                "Ignoring MSTeams activity with untrusted serviceUrl host: {}", service_url
            )
            return
    ...
    async def _validate_inbound_auth(self, auth_header: str, activity: dict[str, Any]) -> None:
        ...
        if not MSTEAMS_AVAILABLE:
            raise RuntimeError("PyJWT not installed. Run: pip install agenite-claw[msteams]")
        ...
        if (
            claim_service_url
            and activity_service_url
            and claim_service_url != activity_service_url
        ):
    ...
    def _save_refs_locked(self, *, prune: bool = True) -> None:
        ...
        try:
            with self._refs_file_lock():
                ...
                refs_meta = {
                    key: {"updated_at": self._safe_float(ref.updated_at)}
                    for key, ref in self._conversation_refs.items()
                }
