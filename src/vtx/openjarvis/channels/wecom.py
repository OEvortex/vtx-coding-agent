...
class WecomChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if not WECOM_AVAILABLE:
            self.logger.error("SDK not installed. Run: pip install agenite-claw[wecom]")
            return
        ...
        self._generate_req_id: Callable[[str], str] = generate_req_id
    ...
    async def _download_and_save_media(
        self, file_url: str, aes_key: str, media_type: str, filename: str | None = None
    ) -> str | None:
        """
        ...
        try:
            ...
            if len(data) > WECOM_UPLOAD_MAX_BYTES:
                self.logger.warning(
                    "inbound media too large: {} bytes (max {})", len(data), WECOM_UPLOAD_MAX_BYTES
                )
                return None
    ...
    async def _upload_media_ws(
        ...
        try:
            ...
            resp = await client._ws_manager.send_reply(
                req_id, {"upload_id": upload_id}, "aibot_upload_media_finish"
            )
            if resp.errcode != 0:
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        try:
                ...
                if media_id:
                    if frame:
                        await self._client.reply(
                            frame, {"msgtype": media_type, media_type: {"media_id": media_id}}
                        )
                    else:
            ...
            if frame:
                ...
                assert self._generate_req_id is not None
                stream_id = self._generate_req_id("stream")
                await self._client.reply_stream(frame, stream_id, content, finish=not is_progress)
                self.logger.debug(
                    "{} sent to {}", "progress" if is_progress else "message", msg.chat_id
                )
            else:
                # No frame (e.g. cron push): proactive send only supports markdown
                await self._client.send_message(
                    msg.chat_id, {"msgtype": "markdown", "markdown": {"content": content}}
                )
