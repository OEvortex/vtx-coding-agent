...
class SlackChannel(BaseChannel):
    ...
    async def _resolve_user_handle(self, handle: str) -> str:
        ...
        cursor: str | None = None
        while True:
            response = await web_client.users_list(limit=200, cursor=cursor)
    ...
    async def _on_socket_request(self, client: Any, req: SocketModeRequest) -> None:
    ...
    async def _update_react_emoji(self, chat_id: str, ts: str | None) -> None:
        ...
        web_client = self._web_client
        try:
            await web_client.reactions_remove(
                channel=chat_id, name=self.config.react_emoji, timestamp=ts
            )
        except Exception as e:
