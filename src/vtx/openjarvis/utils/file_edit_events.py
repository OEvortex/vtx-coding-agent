...
def resolve_file_edit_path(
    tool: Any, workspace: Path | None, params: dict[str, Any] | None
) -> Path | None:
...
def prepare_file_edit_tracker(
    *,
    ...
    trackers = prepare_file_edit_trackers(
        call_id=call_id, tool_name=tool_name, tool=tool, workspace=workspace, params=params
    )
...
def resolve_file_edit_paths(
    tool_name: str, tool: Any, workspace: Path | None, params: dict[str, Any] | None
) -> list[Path]:
...
def _resolve_raw_file_edit_path(tool: Any, workspace: Path | None, raw_path: str) -> Path | None:
    resolver = getattr(tool, "_resolve", None)
    if callable(resolver):
        try:
            resolved = resolver(raw_path)
...
def build_file_edit_start_event(
    tracker: FileEditTracker, params: dict[str, Any] | None
) -> dict[str, Any]:
    ...
    return _event_payload(
        tracker, phase="start", status="editing", added=added, deleted=deleted, approximate=True
    )


def build_file_edit_end_event(
    tracker: FileEditTracker, params: dict[str, Any] | None = None
) -> dict[str, Any]:
...
def build_file_edit_error_event(
    tracker: FileEditTracker, error: str | None = None
) -> dict[str, Any]:
    payload = _event_payload(
        tracker, phase="error", status="error", added=0, deleted=0, approximate=False
    )
    if error:
...
def build_file_edit_live_event(
    tracker: FileEditTracker, *, added: int, deleted: int = 0, operation: str | None = None
) -> dict[str, Any]:
...
def build_file_edit_pending_event(
    *, call_id: str, tool_name: str, added: int = 0, deleted: int = 0
) -> dict[str, Any]:
...
class StreamingFileEditTracker:
    ...
    async def update(self, payload: dict[str, Any]) -> None:
        ...
        await self._emit([build_file_edit_live_event(state.tracker, added=added, deleted=deleted)])
    ...
    async def _update_apply_patch(self, state: _StreamingFileEditState) -> None:
        ...
        for i, m in enumerate(path_matches):
            ...
            events.append(
                build_file_edit_live_event(file_state.tracker, added=added, deleted=deleted)
            )
        if events:
    ...
    async def flush(self) -> None:
        ...
        for state in self._states.values():
            ...
            if (
                ...
                continue
            state.mark_emitted(added, deleted, now)
            events.append(build_file_edit_live_event(state.tracker, added=added, deleted=deleted))
        if events:
            await self._emit(events)
    ...
    async def error_unmatched(self, final_tool_calls: list[Any], error: str) -> None:
...
class _StreamingPatchFileState:
    ...
    def should_emit(self, added: int, deleted: int, now: float) -> bool:
        ...
        if added == self.last_emitted_added and deleted == self.last_emitted_deleted:
            return False
        if (
            max(abs(added - self.last_emitted_added), abs(deleted - self.last_emitted_deleted))
...
class _StreamingFileEditState:
    ...
    def should_emit_pending(self, added: int, deleted: int, now: float) -> bool:
        ...
        if added == self.last_pending_added and deleted == self.last_pending_deleted:
            return False
        if (
            max(abs(added - self.last_pending_added), abs(deleted - self.last_pending_deleted))
            >= _LIVE_EMIT_LINE_STEP
        ):
            return True
...
def _predict_after_text(
    tool_name: str, params: dict[str, Any], before: FileSnapshot
) -> str | None:
