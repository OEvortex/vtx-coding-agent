...
def _cache_tools_token_count(
    tools_id: int, fingerprint: tuple[int, ...], counts: dict[bool, int]
) -> None:
    if (
...
def _estimate_tools_tokens(
    enc: Any, tools: list[dict[str, Any]], *, leading_separator: bool
) -> int:
...
def extract_reasoning(
    ...
    if reasoning_content:
        return strip_reasoning_tags(reasoning_content), strip_think(
            content
...
_TOOL_RESULTS_DIR = ".agenite_claw/tool-results"
...
def recent_message_start_index(
    messages: list[dict[str, Any]], max_messages: int, *, extend_to_user: bool = False
) -> int:
    ...
    recovered_user = next(
        (i for i in range(start_idx - 1, -1, -1) if messages[i].get("role") == "user"), None
    )
...
def _render_tool_result_reference(
    filepath: Path, *, original_size: int, preview: str, truncated_preview: bool
) -> str:
    result = (
...
def build_status_content(
    *,
    ...
    lines = [
        f"\U0001f408 agenite_claw v{version}",
...
def sync_workspace_templates(workspace: Path, silent: bool = False) -> list[str]:
    """Sync bundled templates to workspace. Creates missing files without overwriting user files."""  # noqa: E501
    from importlib.resources import files as pkg_files

    try:
        tpl = pkg_files("agenite_claw") / "templates"
    except Exception:
        return []
    if not tpl.is_dir():
        return []
    ...
    try:
        from vtx.openjarvis.utils.gitstore import GitStore
...
def load_bundled_template(template_name: str) -> str | None:
    """Read a bundled template file from the agenite_claw package."""
    from importlib.resources import files as pkg_files

    with suppress(Exception):
        tpl = pkg_files("agenite_claw") / "templates" / template_name
        if tpl.is_file():
...
async def collect_stream_to_response(
    ...
    **kwargs: Any,
    ...
    from vtx.openjarvis._vtx_bridge import create_bridge_stream, vtx_stop_reason_to_claw
    from vtx.openjarvis.providers.base import LLMResponse, ToolCallRequest
