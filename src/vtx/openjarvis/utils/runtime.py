"""Runtime-specific helper functions and constants — vtx-backed types."""
...
from vtx.openjarvis.utils.helpers import stringify_text_blocks
...
def repeated_external_lookup_error(
    tool_name: str, arguments: Any, seen_counts: dict[str, int]
) -> str | None:
...
def workspace_violation_signature(tool_name: str, arguments: Any) -> str | None:
...
def repeated_workspace_violation_error(
    ...
    if count <= _MAX_REPEAT_WORKSPACE_VIOLATIONS:
        return None
    logger.warning(
        "Escalating repeated workspace bypass attempt {} (attempt {})", signature[:160], count
    )
