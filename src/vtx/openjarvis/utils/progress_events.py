...
from vtx.openjarvis.agent.hook import AgentHookContext
...
async def invoke_file_edit_progress(
    on_progress: Callable[..., Awaitable[None]], file_edit_events: list[dict[str, Any]]
) -> None:
...
def tool_event_result_extras(result: Any) -> tuple[list[Any], list[Any]]:
    if not isinstance(result, dict):
        return [], []
    files_result = result.get("files") if isinstance(result.get("files"), list) else []
    embeds_result = result.get("embeds") if isinstance(result.get("embeds"), list) else []
    return list(files_result), list(embeds_result)  # type: ignore[arg-type]
