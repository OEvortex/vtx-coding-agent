...
async def _fetch_tavily_usage(api_key: str | None) -> SearchUsageInfo:
    ...
    if not key:
        return SearchUsageInfo(
            provider="tavily", supported=True, error="TAVILY_API_KEY not configured"
        )

    try:
        async with httpx.AsyncClient(timeout=8.0) as client:
            r = await client.get(
                "https://api.tavily.com/usage", headers={"Authorization": f"Bearer {key}"}
            )
            r.raise_for_status()
        ...
        return SearchUsageInfo(
            provider="tavily", supported=True, error=f"HTTP {e.response.status_code}"
        )
    except Exception as e:
        return SearchUsageInfo(provider="tavily", supported=True, error=str(e)[:80])
