...
BUILTIN_COMMAND_SPECS: tuple[BuiltinCommandSpec, ...] = (
    BuiltinCommandSpec(
        "/new", "New chat", "Stop the current task and start a fresh conversation.", "square-pen"
    ),
    BuiltinCommandSpec(
        "/stop", "Stop current task", "Cancel the active agent turn for this chat.", "square"
    ),
    BuiltinCommandSpec(
        "/restart", "Restart openjarvis", "Restart the bot process in place.", "rotate-cw"
    ),
    BuiltinCommandSpec(
        "/status", "Show status", "Display runtime, provider, and channel status.", "activity"
    ),
    BuiltinCommandSpec(
        "/model",
        "Switch model preset",
    ...
    BuiltinCommandSpec(
        "/dream", "Run Dream", "Manually trigger memory consolidation.", "sparkles"
    ),
    BuiltinCommandSpec(
        "/dream-log",
        "Show Dream log",
    ...
    BuiltinCommandSpec(
        "/dream-restore", "Restore memory", "Revert memory to a previous Dream snapshot.", "undo-2"
    ),
    BuiltinCommandSpec(
        "/skill", "List skills", "List all enabled skills available to the agent.", "wrench"
    ),
...
async def cmd_restart(ctx: CommandContext) -> OutboundMessage:
    ...
    set_restart_notice_to_env(
        channel=msg.channel, chat_id=msg.chat_id, metadata=dict(msg.metadata or {})
    )

    async def _do_restart():
        await asyncio.sleep(1)
        os.execv(sys.executable, [sys.executable, "-m", "vtx.openjarvis", *sys.argv[1:]])
...
async def cmd_status(ctx: CommandContext) -> OutboundMessage:
    ...
    with suppress(Exception):
        from vtx.openjarvis.utils.searchusage import fetch_search_usage
...
async def cmd_new(ctx: CommandContext) -> OutboundMessage:
    ...
    session = ctx.session or loop.sessions.get_or_create(ctx.key)
    snapshot = session._entries[session.last_consolidated :]
    session.clear()
...
async def cmd_dream(ctx: CommandContext) -> OutboundMessage:
    ...
    async def _run_dream():
        ...
        from vtx.openjarvis.agent.memory import MemoryStore
    ...
    asyncio.create_task(_run_dream())  # noqa: RUF006
    return OutboundMessage(channel=msg.channel, chat_id=msg.chat_id, content="Dreaming...")
...
def _format_dream_restore_list(commits: list) -> str:
    lines = ["## Dream Restore", "", "Choose a Dream memory version to restore. Latest first:", ""]
    for c in commits:
...
async def cmd_history(ctx: CommandContext) -> OutboundMessage:
    ...
    return OutboundMessage(
        channel=ctx.msg.channel,
        chat_id=ctx.msg.chat_id,
        content=header + "\n".join(r for r in recent if r is not None),
        metadata={**dict(ctx.msg.metadata or {}), "render_as": "text"},
    )
...
async def cmd_pairing(ctx: CommandContext) -> OutboundMessage:
    """List, approve, deny or revoke pairing requests."""
    from vtx.openjarvis.pairing import PAIRING_COMMAND_META_KEY, handle_pairing_command
