...
This bus is separate from :mod:`openjarvis.bus.queue`: message bus events are
...
from vtx.openjarvis.bus.events import InboundMessage
...
class RuntimeEventBus:
    ...
    def subscribe(
        self, handler: RuntimeEventHandler, event_type: RuntimeEventType | None = None
    ) -> Callable[[], None]:
    ...
    def publish_nowait(self, event: RuntimeEvent) -> None:
        try:
            ...
            return
        loop.create_task(self.publish(event))  # noqa: RUF006
...
class RuntimeEventPublisher:
    ...
    @staticmethod
    def _context(
        *, channel: str, chat_id: str, session_key: str, metadata: dict[str, Any] | None
    ) -> RuntimeEventContext:
    ...
    async def session_turn_started(self, msg: InboundMessage, session_key: str) -> None:
    ...
    async def turn_completed(
        self, *, channel: str, chat_id: str, session_key: str, metadata: dict[str, Any] | None
    ) -> None:
        await self.bus.publish(
            TurnCompleted(
                context=self._context(
                    channel=channel, chat_id=chat_id, session_key=session_key, metadata=metadata
                ),
