...
def _cli_app_runtime_lines(
    text: str, metadata: Mapping[str, Any] | None, workspace: Path
) -> list[str]:
    ...
    if isinstance(structured, list):
        ...
        if mentions:
            return [
                ...
                f"entry_point={item.get('entry_point') or 'unknown'!s}; "
                f"skill=skills/cli-app-{str(item['name']).strip().lower()}/SKILL.md). "
                "Read the skill when useful, then run this app with `run_cli_app`; do not bypass it with shell."  # noqa: E501
    ...
    if "@" not in text:
        return []
    try:
        from vtx.openjarvis.apps.cli import CliAppManager

        mentions = CliAppManager(workspace=workspace).mentioned_installed_apps(text)  # type: ignore[assignment]
    except Exception:
        return []
    return [
        "CLI App Mention: "
