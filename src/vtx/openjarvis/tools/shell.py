...
class ExecTool(Tool):
    ...
    def __init__(
        ...
        self.deny_patterns = (
            ...
            + [
                ...
                r"\b(?:cp|mv)\b(?:\s+[^\s|;&<>]+)+\s+\S*(?:history\.jsonl|\.dream_cursor)",  # cp/mv target  # noqa: E501
    ...
    def _wrap_path_export(self, command: str, env: dict[str, str]) -> str:
        segments = []
        if self.path_prepend:
            env["AGENITE_CLAW_PATH_PREPEND"] = self.path_prepend
            segments.append("$AGENITE_CLAW_PATH_PREPEND")
        segments.append("$PATH")
        if self.path_append:
            env["AGENITE_CLAW_PATH_APPEND"] = self.path_append
            segments.append("$AGENITE_CLAW_PATH_APPEND")
    ...
    def _extract_absolute_paths(command: str) -> list[str]:
        # Windows: match drive-root paths like `C:\` as well as `C:\path\to\file`, and UNC paths like `\\server\share`  # noqa: E501
