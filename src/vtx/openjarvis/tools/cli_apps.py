...
@tool_parameters(
    tool_parameters_schema(
        required=["name"],
        name=StringSchema("Installed CLI app registry name (e.g. gimp, safari, obsidian)."),
        args=ArraySchema(
        ...
        timeout=IntegerSchema(
            description="Timeout seconds for this CLI call.", minimum=1, maximum=600, nullable=True
        ),
    )
)
class CliAppsTool(Tool):
    ...
    def description(self) -> str:
        ...
        return (
            ...
            "rejected; not for system CLIs (git, gh, python...). Uses argv, not shell."
            + installed_note
        )
