...
# Walking a large tree in Python is the bottleneck for find_files/grep. When
# vtx has provisioned `fd` (managed binary under ~/.vtx/bin, or one already on
# PATH) we shell out to it instead. Flags are chosen to match the os.walk
# fallback exactly: hidden files included, ignore-files disregarded, and the
# same noise dirs pruned, so results do not change based on availability.
_FD_TIMEOUT_SECONDS = 20
T = TypeVar("T")
_TYPE_GLOB_MAP = {
    "py": ("*.py", "*.pyi"),
    "python": ("*.py", "*.pyi"),
...
def _paginate[T](items: list[T], limit: int | None, offset: int) -> tuple[list[T], bool]:
    if limit is None:
...
def _fd_enumerate(root: Path, ignore_dirs: set[str], *, include_dirs: bool) -> list[Path] | None:
    """List paths under ``root`` with ``fd``, or None when it is unavailable.

    Returns absolute paths sorted the same way the ``os.walk`` fallback yields
    them. A None return means callers must fall back to the Python walk.
    """
    binary = get_tool_path("fd")
    if not binary:
        return None

    args = [binary, "--absolute-path", "--hidden", "--no-ignore", "--color", "never"]
    if not include_dirs:
        args += ["--type", "f"]
    for name in sorted(ignore_dirs):
        args += ["--exclude", name]
    args += [".", str(root)]

    try:
        completed = subprocess.run(
            args, capture_output=True, text=True, timeout=_FD_TIMEOUT_SECONDS, check=False
        )
    except (OSError, subprocess.SubprocessError):
        return None
    # Exit code 1 means "no matches"; anything higher is a real failure.
...
class _SearchTool(_FsTool):
    _IGNORE_DIRS = set(ListDirTool._IGNORE_DIRS)  # noqa
    ...
    def _iter_files(self, root: Path) -> Iterable[Path]:
        ...
        listed = _fd_enumerate(root, self._IGNORE_DIRS, include_dirs=False)
...
class FindFilesTool(_SearchTool):
    ...
    def description(self) -> str:
        return (
            "Locate files by fragment, glob, or type. Prefer over shell find/ls. "
            "Returns workspace-relative paths; skips build/dependency dirs."
        )
    ...
    def parameters(self) -> dict[str, Any]:
        return {
            ...
            "properties": {
                ...
                "query": {
                    "type": "string",
                    "description": "Case-insensitive path fragment (all terms match).",
                },
                "glob": {"type": "string", "description": "Glob filter e.g. '*.py'"},
                "type": {"type": "string", "description": "Type shorthand e.g. 'py', 'md'"},
                "include_dirs": {
                    "type": "boolean",
                    "description": "Include matching dirs (default false)",
                },
                "sort": {
                    "type": "string",
                    "enum": ["path", "modified"],
                    "description": "path or recently-modified (default path)",
                },
                "head_limit": {
                    "type": "integer",
                    "description": "Max paths (default 200, 0=all, max 1000)",
                    "minimum": 0,
                    "maximum": 1000,
                },
                "offset": {
    ...
    def _iter_paths(self, root: Path, *, include_dirs: bool) -> Iterable[Path]:
        ...
        listed = _fd_enumerate(root, self._IGNORE_DIRS, include_dirs=include_dirs)
        if listed is not None:
...
class GrepTool(_SearchTool):
    ...
    def description(self) -> str:
        return (
            "Regex search over files. Default lists matching file paths; "
            "set output_mode=content for matched lines. Prefer over shell grep."
        )
    ...
    def parameters(self) -> dict[str, Any]:
        return {
            ...
            "properties": {
                ...
                "type": {"type": "string", "description": "Type shorthand (py, ts, md, json...)"},
                "case_insensitive": {
                    "type": "boolean",
                ...
                "fixed_strings": {
                    "type": "boolean",
                    "description": "Plain text, not regex (default false)",
                },
                "output_mode": {
                    "type": "string",
                    "enum": ["content", "files_with_matches", "count"],
                    "description": "content=lines, files=paths, count=per-file (default files)",
                },
                "context_before": {
                    "type": "integer",
                    "description": "Context lines before match (0-20)",
                    "minimum": 0,
                    "maximum": 20,
                },
                "context_after": {
                    "type": "integer",
                    "description": "Context lines after match (0-20)",
                    "minimum": 0,
                    "maximum": 20,
                },
                "head_limit": {
                    "type": "integer",
                    "description": "Max results (default 250, 0=all)",
                    "minimum": 0,
                    "maximum": 1000,
                },
                "offset": {
    ...
    @staticmethod
    def _format_block(
        display_path: str, lines: list[str], match_line: int, before: int, after: int
    ) -> str:
            ...
            for file_path in self._iter_files(target):
                ...
                for idx, line in enumerate(lines, start=1):
                    ...
                    block = self._format_block(
                        display_path, lines, idx, context_before, context_after
                    )
                ...
                if output_mode == "count" and file_had_match:  # noqa
                    if display_path not in matching_files:
                        matching_files.append(display_path)
                        file_mtimes[display_path] = mtime
            ...
            if output_mode == "files_with_matches":
                if not matching_files:
                    ...
                    ordered_files = sorted(
                        matching_files, key=lambda name: (-file_mtimes.get(name, 0.0), name)
                    )
