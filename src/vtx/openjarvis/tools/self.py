...
def _has_real_attr(obj: Any, key: str) -> bool:
    ...
    if d is not None and key in d:
        return True
    return any(key in cls.__dict__ for cls in type(obj).__mro__)
...
class MyTool(Tool, ContextAware):
    ...
    RESTRICTED: dict[str, dict[str, Any]] = {  # noqa
    ...
    def parameters(self) -> dict[str, Any]:
        return {
            ...
            "properties": {
                ...
                "key": {
                    "type": "string",
                    "description": "Dot-path for check/set. Examples: 'max_iterations', 'workspace', 'provider_retry_mode'. "  # noqa: E501
                    "Use 'model_preset' to switch named model presets. For check without key, shows all config values.",  # noqa: E501
                },
                "value": {
                    "description": "New value (for set). Type must match target (int for max_iterations/context_window_tokens, str for model/model_preset)."  # noqa: E501
                },
            },
            "required": ["action"],
        }
    ...
    def _format_value(val: Any, key: str = "") -> str:
        ...
        if isinstance(val, (list, tuple)):
            ...
            r = repr(val)
            return f"{key}: {r}" if key else r
        # Complex object — small Pydantic models: show values; others: show field names for navigation  # noqa: E501
    ...
    async def execute(
        self, action: str, key: str | None = None, value: Any = None, **_kwargs: Any
    ) -> str:
    ...
    def _modify_free(self, key: str, value: Any) -> str:
        if _has_real_attr(self._runtime_state, key):
            ...
            if isinstance(old, (str, int, float, bool)):
                ...
                if old_t is float and new_t is int:
                    ...
                    self._audit(
                        "modify",
                        f"REJECTED type mismatch {key}: expects {old_t.__name__}, got {new_t.__name__}",  # noqa: E501
                    )
        ...
        if (
            ...
            return f"Error: scratchpad is full (max {self._MAX_RUNTIME_KEYS} keys). Remove unused keys first."  # noqa: E501
