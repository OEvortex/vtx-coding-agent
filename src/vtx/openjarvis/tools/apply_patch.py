    ...
    tool_parameters_schema(
        edits=ArraySchema(
            items=ObjectSchema(
                ...
                old_text=StringSchema("Exact text to find. Required for replace.", nullable=True),
...
class ApplyPatchTool(_FsTool):
    ...
    async def execute(
        self, edits: list[dict] | None = None, dry_run: bool = False, **kwargs: Any
    ) -> str:
        try:
            if not edits:
                ...
                if action == "add":
                    ...
                    if pending is not None:
                        ...
                        try:
                            ...
                            raise _PatchError(f"file is not UTF-8 text: {path}")  # noqa: B904
                        exists = True
                    else:
                        content = ""
                        exists = False
