...
class FileStates:
    ...
    def record_read(self, path: str | Path, offset: int = 1, limit: int | None = None) -> None:
        ...
        self._state[p] = ReadState(
            mtime=mtime, offset=offset, limit=limit, content_hash=_hash_file(p), can_dedup=True
        )
    ...
    def record_write(self, path: str | Path) -> None:
        ...
        try:
            ...
            return
        self._state[p] = ReadState(
            mtime=mtime, offset=1, limit=None, content_hash=_hash_file(p), can_dedup=False
        )
    ...
    def check_read(self, path: str | Path) -> str | None:
        ...
        if entry is None:
            return "Warning: file has not been read yet. Read it first to verify content before editing."  # noqa: E501
        try:
        ...
        if current_mtime != entry.mtime:
            if entry.content_hash and _hash_file(p) == entry.content_hash:
                ...
                return None
            return "Warning: file has been modified since last read. Re-read to verify content before editing."  # noqa: E501
    ...
    def is_unchanged(self, path: str | Path, offset: int = 1, limit: int | None = None) -> bool:
        ...
        if current_mtime != entry.mtime:
            ...
            if current_hash != entry.content_hash:
                ...
                return False
            # Content identical despite mtime change (e.g. touch) - mark as not dedupable to force full read next time  # noqa: E501
            entry.can_dedup = False
            return True
