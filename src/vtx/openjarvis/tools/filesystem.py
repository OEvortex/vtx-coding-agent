...
class _FsTool(Tool):
    ...
    def _resolve_read(self, path: str) -> Path:
        return self._resolve_with_extra(
            path, self._extra_read_allowed_dirs, None, include_media_dir=True
        )
...
def _is_blocked_device(path: str | Path) -> bool:
    ...
    return bool(resolved.startswith("/dev/"))
...
@tool_parameters(
    tool_parameters_schema(
        ...
        offset=IntegerSchema(1, description="Start line (1-indexed, default 1)", minimum=1),
        limit=IntegerSchema(2000, description="Max lines to read (default 2000)", minimum=1),
        pages=StringSchema("PDF page range e.g. '1-5' (default all, max 20)"),
        force=BooleanSchema(
            description="Re-read even if unchanged (bypass dedup).", default=False
        ),
        required=["path"],
    )
)
class ReadFileTool(_FsTool):
    ...
    _scopes = {"core", "subagent", "memory"}  # noqa
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if _is_blocked_device(path):
                return f"Error: Reading {path} is blocked (device path that could hang or produce infinite output)."  # noqa: E501

            fp = self._resolve_read(path)
            if _is_blocked_device(fp):
                return f"Error: Reading {fp} is blocked (device path that could hang or produce infinite output)."  # noqa: E501
            if not fp.exists():
            ...
            try:
                ...
                if mime and mime.startswith("image/"):
                    return build_image_content_blocks(raw, mime, str(fp), f"(Image file: {path})")
                return f"Error: Cannot read binary file {path} (MIME: {mime or 'unknown'}). Only UTF-8 text and images are supported."  # noqa: E501
            ...
            if end < total:
                result += f"\n\n(Showing lines {offset}-{end} of {total}. Use offset={end + 1} to continue.)"  # noqa: E501
            else:
    ...
    def _read_pdf(self, fp: Path, pages: str | None) -> str:
        ...
        if pages:
            ...
            if start > end or start >= total_pages:
                doc.close()
                return f"Error: Page range '{pages}' is out of bounds (document has {total_pages} pages)."  # noqa: E501
        else:
            start = 0
        ...
        if end < total_pages - 1:
            result += f"\n\n(Showing pages {start + 1}-{end + 1} of {total_pages}. Use pages='{end + 2}-{min(end + 1 + self._MAX_PDF_PAGES, total_pages)}' to continue.)"  # noqa: E501
        if len(result) > self._MAX_CHARS:
...
def _reindent_like_match(old_text: str, actual_text: str, new_text: str) -> str:
    ...
    comparable = [
        ...
        for old_line, actual_line in zip(old_lines, actual_lines, strict=False)
...
def _find_trim_matches(
    ...
    for i in range(len(content_lines) - window_size + 1):
        ...
        matches.append(_MatchSpan(start=start, end=end, text=content[start:end], line=i + 1))
    return matches
...
@tool_parameters(
    tool_parameters_schema(
        ...
        line_hint=IntegerSchema(
            1, description="1-based line hint to pick the nearest match.", minimum=1, nullable=True
        ),
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if fsize > self._MAX_EDIT_FILE_SIZE:
                return f"Error: File too large to edit ({fsize / (1024**3):.1f} GiB). Maximum is 1 GiB."  # noqa: E501
...
@tool_parameters(
    tool_parameters_schema(
        ...
        max_entries=IntegerSchema(200, description="Max entries (default 200)", minimum=1),
        required=["path"],
    )
)
