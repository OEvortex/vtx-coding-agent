...
def _is_path_exactly_allowed(
    logical_path: Path, resolved_path: Path, files: Iterable[str | Path]
) -> bool:
...
def require_path_within(path: str | Path, root: str | Path, *, message: str | None = None) -> Path:
...
def resolve_allowed_path(
    ...
    *,
    ...
    exact_allowed = bool(files) and _is_path_exactly_allowed(
        _resolve_logical_path(path, workspace), resolved, files
    )
