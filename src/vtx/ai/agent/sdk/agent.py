...
from vtx.ai.agent.sdk.guardrails import InputGuardrail, OutputGuardrail
from vtx.ai.agent.sdk.handoffs import Handoff
from vtx.ai.agent.sdk.tools import FunctionTool
...
class Agent[TContext]:
    ...
    def as_tool(
        ...
        """
        from vtx.ai.agent.sdk.tools import _AgentAsTool
    ...
    def resolve_provider(self) -> Any:
        ...
        3. Otherwise (``self.provider`` is ``None``), fall back to env
           vars and ``self.model``.
        """
        from vtx.ai import BaseProvider as _BaseProvider

        if isinstance(self.provider, _BaseProvider):
            return self.provider

        from vtx.ai import ApiType, ProviderConfig, get_model, get_provider_class
        from vtx.ai.base import resolve_api_key
        ...
        if name is not None and self._is_builtin_provider(name):
            ...
            if info is not None:
                ...
                from vtx.ai.providers import resolve_provider_api_type
