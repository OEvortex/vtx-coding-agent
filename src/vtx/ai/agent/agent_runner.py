...
from vtx.ai.agent.hooks.agent_hook import AgentHook
from vtx.ai.agent.tools import BaseTool
from vtx.core.events import Event, StreamEvent
from vtx.core.types import AssistantMessage, Message, StopReason, ToolResultMessage
...
class AgentRunSpec:
    ...
    # In-process lifecycle hooks (AgentHook). Fired by the engine around
    # run/iteration boundaries and stream deltas.
...
def _build_composite(hooks: list[AgentHook] | None) -> AgentHook | None:
    ...
    from vtx.ai.agent.hooks.agent_hook import CompositeHook
