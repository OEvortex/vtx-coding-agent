...
from agenite_claw import __version__
from agenite_claw.bus.events import OutboundMessage
from agenite_claw.command.router import CommandContext, CommandRouter
from agenite_claw.utils.helpers import build_status_content
from agenite_claw.utils.restart import set_restart_notice_to_env
...
BUILTIN_COMMAND_SPECS: tuple[BuiltinCommandSpec, ...] = (
    ...
    BuiltinCommandSpec(
        "/restart", "Restart agenite_claw", "Restart the bot process in place.", "rotate-cw"
    ),
    BuiltinCommandSpec(
...
async def cmd_restart(ctx: CommandContext) -> OutboundMessage:
    ...
    async def _do_restart():
        ...
        os.execv(sys.executable, [sys.executable, "-m", "agenite_claw", *sys.argv[1:]])
...
async def cmd_status(ctx: CommandContext) -> OutboundMessage:
    ...
    with suppress(Exception):
        from agenite_claw.utils.searchusage import fetch_search_usage
...
async def cmd_pairing(ctx: CommandContext) -> OutboundMessage:
    """List, approve, deny or revoke pairing requests."""
    from agenite_claw.pairing import PAIRING_COMMAND_META_KEY, handle_pairing_command
