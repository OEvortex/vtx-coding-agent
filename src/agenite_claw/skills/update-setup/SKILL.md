---
name: update-setup
description: One-time setup wizard for the agenite-claw upgrade skill. Triggers: setup update, configure update, 切设置更新, 初始化更新.
---
...
Use `exec` to run `agenite-claw --version`. Tell the user the current version.

Then collect install clues with `exec`. These commands are best-effort; if one fails,
keep going and show the useful output:

```
command -v agenite-claw || true
python -m pip show agenite-claw || true
pipx list | sed -n '/agenite-claw/,+3p' || true
uv tool list | sed -n '/agenite-claw/,+3p' || true
```
...
answer, stop and ask the user to rerun this setup when they know how agenite-claw was
installed.
...
`question: "Where is your agenite-claw source checkout? Enter an absolute path or a path relative to this workspace:"`.
...
| Method | Command |
|--------|---------|
| uv | `uv tool install "agenite-claw[EXTRAS]" --force` |
| pipx | `pipx install --force "agenite-claw[EXTRAS]"` |
| pip | `python -m pip install --upgrade "agenite-claw[EXTRAS]"` |
...
---
name: update
description: "Upgrade agenite-claw to the latest version. Triggers: upgrade agenite-claw, update agenite-claw, 升级agenite-claw, 更新agenite-claw."
---
...
4. Use `exec` to verify: `agenite-claw --version`
5. Tell the user the new version. Say: "Run `/restart` to restart agenite-claw and apply the update. If `/restart` is unavailable in this channel, restart the agenite-claw process manually."
```

## Step 5: Confirm

Tell the user: "Upgrade skill created. Say 'upgrade agenite-claw' when you want to update."
