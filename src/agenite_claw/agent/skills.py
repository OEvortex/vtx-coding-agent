...
_STRIP_SKILL_FRONTMATTER = re.compile(r"^---\s*\r?\n(.*?)\r?\n---\s*\r?\n?", re.DOTALL)
...
class SkillsLoader:
    ...
    def _parse_agenite_claw_metadata(self, raw: object) -> dict:
        """Extract agenite_claw/openclaw metadata from a frontmatter field.
        ...
        if not isinstance(data, dict):
            return {}
        payload = data.get("agenite_claw", data.get("openclaw", {}))
    ...
    def _get_skill_meta(self, name: str) -> dict:
        """Get agenite_claw metadata for a skill (cached in frontmatter)."""
        raw_meta = self.get_skill_metadata(name) or {}
        return self._parse_agenite_claw_metadata(raw_meta.get("metadata"))
    ...
    def get_always_skills(self) -> list[str]:
        ...
        return [
            ...
            and (
                self._parse_agenite_claw_metadata(meta.get("metadata")).get("always")
                or meta.get("always")
            )
        ]
