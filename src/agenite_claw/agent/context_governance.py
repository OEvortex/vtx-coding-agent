...
from agenite_claw.utils.runtime import ensure_nonempty_tool_result
...
PLACEHOLDER_TEXTS = frozenset({"[Previous assistant message omitted.]"})
...
class ContextGovernor:
    ...
    def input_budget(config: ContextGovernanceConfig) -> int:
        ...
        provider_max_tokens = getattr(
            getattr(config.provider, "generation", None), "max_tokens", 4096
        )
        max_output = (
            config.max_tokens
    ...
    def normalize_tool_result(
        config: ContextGovernanceConfig, tool_call_id: str, tool_name: str, result: Any
    ) -> Any:
    ...
    def strip_placeholder_assistant_messages(
        ...
        for idx, msg in enumerate(messages):
            ...
            if is_placeholder and not has_tool_calls:
                ...
                logger.debug(
                    "Stripping placeholder assistant message from history: {!r}", text[:60]
                )
                continue
            if updated is not None:
                updated.append(msg)
    ...
    @staticmethod
    def strip_malformed_tool_calls(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    def snip_history(
        ...
        tools = config.tools.get_definitions()
        estimate, _ = estimate_prompt_tokens_chain(config.provider, config.model, messages, tools)
        if estimate <= budget:
            return messages
        ...
        fixed_tokens, _ = estimate_prompt_tokens_chain(
            config.provider, config.model, system_messages, tools
        )
    ...
    def _legal_history_tail(
        self, kept: list[dict[str, Any]], non_system: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
    ...
    def _apply_recorded_compactions(
        self, messages: list[dict[str, Any]], compacted_tool_call_ids: set[str]
    ) -> list[dict[str, Any]]:
