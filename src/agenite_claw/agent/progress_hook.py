...
from agenite_claw.utils.helpers import IncrementalThinkExtractor, strip_think
...
from agenite_claw.utils.tool_hints import format_tool_hints
...
class AgentProgressHook(AgentHook):
    ...
    async def before_iteration(self, context: AgentHookContext) -> None:
        ...
        logger.debug(
            "Starting agent loop iteration {} for session {}", context.iteration, self._session_key
        )
    ...
    async def after_iteration(self, context: AgentHookContext) -> None:
        if (
            ...
            if tool_events:
                await invoke_on_progress(
                    self._on_progress, "", tool_hint=False, tool_events=tool_events
                )
        u = context.usage or {}
        logger.debug(
