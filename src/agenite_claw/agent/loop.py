...
from agenite_claw._vtx_bridge import vtx_stop_reason_to_claw
from agenite_claw.agent import context as agent_context
from agenite_claw.agent import model_presets as preset_helpers
from agenite_claw.agent.autocompact import AutoCompact
from agenite_claw.agent.context import ContextBuilder
from agenite_claw.agent.cron_turns import CronTurnCoordinator
from agenite_claw.agent.hook import AgentHook, CompositeHook
from agenite_claw.agent.memory import Consolidator
from agenite_claw.agent.progress_hook import AgentProgressHook
from agenite_claw.agent.runner import _MAX_INJECTIONS_PER_TURN, AgentRunner, AgentRunSpec
from agenite_claw.agent.subagent import SubagentManager
from agenite_claw.agent.tools.context import (
    RequestContext,
    bind_request_context,
    reset_request_context,
)
from agenite_claw.agent.tools.file_state import FileStateStore, bind_file_states, reset_file_states
from agenite_claw.agent.tools.message import MessageTool
from agenite_claw.agent.tools.registry import ToolRegistry
from agenite_claw.agent.tools.self import MyTool
from agenite_claw.bus.events import InboundMessage, OutboundMessage
from agenite_claw.bus.progress import build_bus_progress_callback
from agenite_claw.bus.queue import MessageBus
from agenite_claw.bus.runtime_events import (
    RuntimeEventBus,
    RuntimeEventPublisher,
    ensure_runtime_event_publisher,
)
from agenite_claw.command import CommandContext, CommandRouter, register_builtin_commands
from agenite_claw.config.schema import AgentDefaults, ModelPresetConfig
from agenite_claw.cron.session_turns import cron_history_overrides
from agenite_claw.providers.base import LLMProvider
...
from agenite_claw.session import turn_continuation
from agenite_claw.session.goal_state import (
    goal_state_runtime_lines,
    runner_wall_llm_timeout_s,
    sustained_goal_active,
)
from agenite_claw.session.keys import UNIFIED_SESSION_KEY, session_key_for_channel
from agenite_claw.session.manager import Session, SessionManager, replay_max_messages_for_context
from agenite_claw.utils.document import extract_documents, reference_non_image_attachments
from agenite_claw.utils.helpers import image_placeholder_text
from agenite_claw.utils.helpers import truncate_text as truncate_text_fn
from agenite_claw.utils.llm_runtime import LLMRuntime
from agenite_claw.utils.runtime import EMPTY_FINAL_RESPONSE_MESSAGE

if TYPE_CHECKING:
    from agenite_claw.config.schema import ChannelsConfig, ProviderConfig, ToolsConfig
...
class AgentLoop:
    ...
    def __init__(
        ...
        from agenite_claw.config.schema import ToolsConfig
        ...
        if (
            ...
            self._image_generation_provider_configs["openrouter"] = (
                image_generation_provider_config
            )
        self.cron_service = cron_service
        self.restrict_to_workspace = restrict_to_workspace
        self.workspace_scopes = WorkspaceScopeResolver(
            default_workspace=workspace, default_restrict_to_workspace=restrict_to_workspace
        )
        ...
        # AGENITE_CLAW_MAX_CONCURRENT_REQUESTS: <=0 means unlimited; default 3.
        _max = int(os.environ.get("AGENITE_CLAW_MAX_CONCURRENT_REQUESTS", "3"))
    ...
    @classmethod
    def from_config(cls, config: Any, bus: MessageBus | None = None, **extra: Any) -> AgentLoop:
        ...
        """
        from agenite_claw.providers.factory import make_provider
        ...
        ) or preset_helpers.make_preset_snapshot_loader(config, provider_snapshot_loader)
        return cls(
            bus=bus,
            provider=provider,
        ...
        *,
        ...
        if publish_update and self._runtime_model_publisher is not None:
            self._runtime_model_publisher(
                self.model, model_preset if model_preset is not None else self.model_preset
            )
        if publish_update:
    ...
    def _register_default_tools(self) -> None:
        ...
        if self.tools_config.my.enable:
            ...
            from agenite_claw.agent.tools import runtime_state as _rs
    ...
    def _set_tool_context(
        ...
        from agenite_claw.agent.tools.context import ContextAware

        effective_key = session_key or session_key_for_channel(
            channel, chat_id, unified_session=self._unified_session
        )
        ...
        async def _drain_pending(*, limit: int = _MAX_INJECTIONS_PER_TURN) -> list[dict[str, Any]]:
            ...
            if (
                ...
                try:
                    ...
                    logger.warning(
                        "Timeout waiting for sub-agent completion in session {}", session.key
                    )
                    return items
        ...
        try:
            result = await self.runner.run(
                AgentRunSpec(
                    ...
                    # Sustained goals may legitimately exceed AGENITE_CLAW_LLM_TIMEOUT_S; idle stall
                    # is still capped by AGENITE_CLAW_STREAM_IDLE_TIMEOUT_S in streaming providers.
            ...
            while self._running:
                try:
                    ...
                    self.auto_compact.check_expired(
                        self._schedule_background, active_session_keys=self._pending_queues.keys()
                    )
                    continue
                ...
                if self.commands.is_priority(raw):
                    await self._dispatch_command_inline(
                        msg, effective_key, raw, self.commands.dispatch_priority
                    )
                    continue
                if self._cron_turns.defer_if_active(
                    msg, session_key=effective_key, active_session_keys=self._pending_queues.keys()
                ):
                    logger.info("Deferred cron turn for active session {}", effective_key)
                    continue
                ...
                if effective_key in self._pending_queues:
                    ...
                    if self.commands.is_dispatchable_command(raw):
                        await self._dispatch_command_inline(
                            msg, effective_key, raw, self.commands.dispatch
                        )
                        continue
                    pending_msg = msg
        ...
        try:
            async with lock, gate:
                ...
                try:
                    ...
                    self._cron_turns.complete(msg, error=asyncio.CancelledError())
    ...
    async def _process_system_message(
        ...
        await self.consolidator.maybe_consolidate_by_tokens(
            session, replay_max_messages=self._max_messages
        )
        ...
        self._set_tool_context(
            channel, chat_id, msg.metadata.get("message_id"), msg.metadata, session_key=key
        )
        ...
        t_wall = time.time()
        final_content, _, all_msgs, _stop_reason, _ = await self._run_agent_loop(
            messages,
            session=session,
            channel=channel,
            chat_id=chat_id,
        ...
        return OutboundMessage(
            channel=channel, chat_id=chat_id, content=content, metadata=outbound_metadata
        )
    ...
    async def _process_message(
        ...
        while ctx.state is not TurnState.DONE:
            ...
            duration = (time.perf_counter() - t0) * 1000
            ctx.trace.append(
                StateTraceEntry(state=ctx.state, started_at=t0, duration_ms=duration, event=event)
            )
            logger.debug(
        ...
        logger.debug("[turn {}] Turn completed after {} states", ctx.turn_id, len(ctx.trace))
        return ctx.outbound
    ...
    async def _state_save(self, ctx: TurnContext) -> str:
        ...
        self._save_turn(
            ctx.session, ctx.all_messages, ctx.save_skip, turn_latency_ms=ctx.turn_latency_ms
        )
        self._runtime_events().record_turn_latency(ctx.session_key, ctx.turn_latency_ms)
        if not ctx.ephemeral:
