...
from agenite_claw.agent.context_governance import ContextGovernanceConfig, ContextGovernor
from agenite_claw.agent.hook import AgentHook, AgentHookContext, AgentRunHookContext
from agenite_claw.agent.tools.registry import ToolRegistry
from agenite_claw.providers.base import LLMProvider, LLMResponse, ToolCallRequest
from agenite_claw.utils.file_edit_events import (
...
from agenite_claw.utils.helpers import (
...
from agenite_claw.utils.progress_events import (
    invoke_file_edit_progress,
    on_progress_accepts_file_edit_events,
)
from agenite_claw.utils.prompt_templates import render_template
from agenite_claw.utils.runtime import (
...
class AgentRunResult:
    """Outcome of a shared agent execution — vtx-backed types natively."""

    final_content: str | None
    messages: list[dict[str, Any]]
    tools_used: list[str] = field(default_factory=list)
    usage: VtxUsage | None = None
    stop_reason: VtxStopReason = VtxStopReason.STOP
    error: str | None = None
...
class AgentRunner:
    ...
    def _append_injected_messages(
        cls, messages: list[dict[str, Any]], injections: list[dict[str, Any]]
    ) -> None:
    ...
    async def run(self, spec: AgentRunSpec) -> AgentRunResult:
        ...
        try:
            ...
            context.stop_reason = vtx_stop_reason_to_claw(result.stop_reason)
    ...
    async def _run_core(
        self, spec: AgentRunSpec, hook: AgentHook, messages: list[dict[str, Any]]
    ) -> AgentRunResult:
        final_content: str | None = None
        tools_used: list[str] = []
        usage: dict[str, int] = {}
        error: str | None = None
        stop_reason = VtxStopReason.STOP
        ...
        for iteration in range(spec.max_iterations):
            ...
            context = AgentHookContext(
                iteration=iteration, messages=messages, session_key=spec.session_key
            )
            ...
            if response.should_execute_tools:
                ...
                results, new_events, fatal_error = await self._execute_tools(
                    spec, response.tool_calls, external_lookup_counts, workspace_violation_counts
                )
                tool_events.extend(new_events)
                tools_used.extend(
                    tool_call.name
                    for tool_call, event in zip(response.tool_calls, new_events, strict=False)
                ...
                if fatal_error is not None:
                    ...
                    final_content = error
                    stop_reason = VtxStopReason.ERROR
                    self._append_final_message(messages, final_content)
                    context.final_content = final_content
                    context.error = error
                    context.stop_reason = "tool_error"
            ...
            if is_blank_text(clean):
                ...
                context.final_content = final_content
                context.error = error
                context.stop_reason = "empty_final_response"
                await hook.after_iteration(context)
                should_continue, injection_cycles = await self._try_drain_injections(
            ...
            stop_reason = VtxStopReason.LENGTH
    ...
    async def _request_model(
        ...
        *,
        ...
        if timeout_s is None:
            ...
            # Set AGENITE_CLAW_LLM_TIMEOUT_S=0 to disable.
            raw = os.environ.get("AGENITE_CLAW_LLM_TIMEOUT_S", "300").strip()
            try:
                timeout_s = float(raw)
        ...
        tool_defs: list[dict[str, Any]] | None = (
            None if malformed_retry else spec.tools.get_definitions()
        )
        ...
        from agenite_claw._vtx_bridge import create_bridge_stream
        ...
        if wants_streaming:
            ...
            _stream_recover_cb = _stream_recover
        ...
        adapter, stream = await create_bridge_stream(
            ...
            on_stream_recover=_stream_recover_cb,
        )

        # Consume stream parts and build LLMResponse
        buf_text: list[str] = []
        ...
        thinking_buf = ""
        stream_buf = ""
        think_extractor: IncrementalThinkExtractor | None = None

        if wants_progress_streaming:
            think_extractor = IncrementalThinkExtractor()
            progress_state = {"reasoning_open": False}

        # Streaming requests already have provider-level idle timeouts
        # (AGENITE_CLAW_STREAM_IDLE_TIMEOUT_S). Do not also apply the outer wall-clock
        # LLM timeout here, or healthy long reasoning streams can be killed just
        # because total elapsed time exceeded AGENITE_CLAW_LLM_TIMEOUT_S.
        ...
        try:
            while True:
                if outer_timeout_s is not None and not buf_text:
                    # Apply timeout on first part (non-streaming only)
                    try:
                        part = await asyncio.wait_for(
                            stream_iter.__anext__(), timeout=outer_timeout_s
                        )
                    except TimeoutError:
                        return LLMResponse(
                    ...
                    try:
                        part = await stream_iter.__anext__()
                    except StopAsyncIteration:
                        break
                ...
                if isinstance(part, TextPart):
                    ...
                    if wants_streaming:
                        ...
                        await hook.on_stream(context, t)
                        ...
                        if incremental:
                            if progress_state is not None and progress_state.get("reasoning_open"):
                                await hook.emit_reasoning_end()
                                if progress_state is not None:
                                    progress_state["reasoning_open"] = False
                            context.streamed_content = True
                    ...
                    tool_call_data[part.index] = {
                        "id": part.id,
                        "name": part.name,
                        "arguments": part.arguments
                        if isinstance(part.arguments, str)
                        else json.dumps(part.arguments)
                        if part.arguments
                        else "",
                    }
                    if live_file_edits is not None:
                        await live_file_edits.update(
                            {"id": part.id, "name": part.name, "arguments": part.arguments}
                        )
                elif isinstance(part, ToolCallDelta):
                    if part.index in tool_call_data:
                        existing = tool_call_data[part.index]["arguments"]
                        if part.replace:
                            tool_call_data[part.index]["arguments"] = part.arguments_delta
                        else:
                            tool_call_data[part.index]["arguments"] = (
                                existing + part.arguments_delta
                            )
                    if live_file_edits is not None:
                        await live_file_edits.update(
                            {
                                "id": tool_call_data.get(part.index, {}).get("id", ""),
                                "arguments_delta": part.arguments_delta,
                            }
                        )
                elif isinstance(part, StreamDone):
                    finish_reason = vtx_stop_reason_to_claw(part.stop_reason)
        ...
        # Flush live file edits after stream consumption
        if live_file_edits is not None:
            await live_file_edits.flush()

        # Build tool_calls from collected data
        for idx in sorted(tool_call_data.keys()):
            ...
            tool_call_requests.append(
                ToolCallRequest(id=data["id"], name=data["name"], arguments=arguments)
            )

        text_content = "".join(buf_text) or None
        reasoning_content = "".join(buf_reasoning) or None
        ...
        # Preserve thinking_blocks from the raw provider response if available
        if adapter.last_response is not None:
            tb = getattr(adapter.last_response, "thinking_blocks", None)
            if tb is not None:
                response.thinking_blocks = tb

        # Apply final call IDs to live file edits if tools were called
        if live_file_edits is not None and tool_call_requests:
            live_file_edits.apply_final_call_ids(tool_call_requests)
            await live_file_edits.error_unmatched(
                tool_call_requests, "Tool call did not complete."
            )
        ...
        if (
            ...
            logger.warning(
                "Retrying LLM request after all {} malformed tool call(s) were dropped", dropped
            )
    ...
    def _malformed_tool_call_retry_messages(
        messages: list[dict[str, Any]], assistant_text: str | None
    ) -> list[dict[str, Any]]:
    ...
    async def _request_no_tools(
        ...
        from agenite_claw.utils.helpers import collect_stream_to_response
    ...
    def _max_iterations_fallback(spec: AgentRunSpec) -> str:
        ...
        return render_template(
            "agent/max_iterations_message.md", strip=True, max_iterations=spec.max_iterations
        )
    ...
    async def _run_tool(
        ...
        if file_edit_trackers and progress_callback is not None:
            await invoke_file_edit_progress(
                ...
                [
                    build_file_edit_start_event(
                        file_edit_tracker, params if isinstance(params, dict) else None
                    )
        ...
        *,
        ...
        if self._is_workspace_violation(raw_text):
            ...
            if escalation is not None:
                logger.warning(
                    "Tool {} hit workspace boundary repeatedly; escalating hint", tool_call.name
                )
