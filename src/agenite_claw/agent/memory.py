...
from agenite_claw.session.manager import Session
from agenite_claw.utils.gitstore import GitStore
...
class MemoryStore:
    ...
    def _should_start_new_legacy_chunk(self, line: str, current: list[str]) -> bool:
        ...
        if not self._LEGACY_ENTRY_START_RE.match(line):
            return False
        return not (self._is_raw_legacy_chunk(current) and self._LEGACY_RAW_MESSAGE_RE.match(line))
    ...
    def _legacy_fallback_timestamp(self) -> str:
        try:
            return datetime.fromtimestamp(self.legacy_history_file.stat().st_mtime).strftime(
                "%Y-%m-%d %H:%M"
            )
        except OSError:
    ...
    def append_history(
        self, entry: str, *, max_chars: int | None = None, session_key: str | None = None
    ) -> int:
    ...
    def read_recent_history_for_prompt(
        self, since_cursor: int, *, session_key: str | None, unified_session: bool = False
    ) -> list[dict[str, Any]]:
    ...
    def build_dream_tools(self):
        ...
        from agenite_claw.agent.skills import BUILTIN_SKILLS_DIR
        from agenite_claw.agent.tools.apply_patch import ApplyPatchTool
        from agenite_claw.agent.tools.file_state import FileStates
        from agenite_claw.agent.tools.filesystem import EditFileTool, ReadFileTool, WriteFileTool
    ...
    def raw_archive(
        self, messages: list[dict], *, max_chars: int | None = None, session_key: str | None = None
    ) -> None:
        """Fallback: dump raw messages to history.jsonl without LLM summarization."""
        limit = max_chars if max_chars is not None else _RAW_ARCHIVE_MAX_CHARS
        formatted = truncate_text(self._format_messages(messages), limit)
        self.append_history(
            f"[RAW] {len(messages)} messages\n{formatted}", session_key=session_key
        )
    ...
    def prune_dream_sessions(sessions_dir: Path, *, keep: int = 10) -> None:
        ...
        """
        dream_files = sorted(sessions_dir.glob("dream_*.jsonl"), key=lambda p: p.stat().st_mtime)
...
class Consolidator:
    ...
    def set_provider(self, provider: LLMProvider, model: str, context_window_tokens: int) -> None:
        self.provider = provider
        self.model = model
        self.context_window_tokens = context_window_tokens
    ...
    @staticmethod
    def _replay_overflow_boundary(session: Session, replay_max_messages: int | None) -> int | None:
        ...
        start_idx = recent_message_start_index(
            tail_messages, replay_max_messages, extend_to_user=True
        )
    ...
    async def _consolidate_replay_overflow(
        self, session: Session, replay_max_messages: int | None
    ) -> str | None:
    ...
    async def maybe_consolidate_by_tokens(
        self, session: Session, *, replay_max_messages: int | None = None
    ) -> None:
