...
TOKEN_APP_NAME = "agenite-claw"
USER_AGENT = "agenite_claw/0.1"
...
def get_storage() -> FileTokenStorage:
    return FileTokenStorage(
        token_filename=TOKEN_FILENAME, app_name=TOKEN_APP_NAME, import_codex_cli=False
    )
...
def login_github_copilot(
    print_fn: Callable[[str], None] | None = None, prompt_fn: Callable[[str], str] | None = None
) -> OAuthToken:
...
class GitHubCopilotProvider(LLMProvider):
    ...
    async def _get_copilot_access_token(self) -> str:
        ...
        if not github_token or not github_token.access:
            raise RuntimeError(
                "GitHub Copilot is not logged in. Run: agenite-claw provider login github-copilot"
            )
        ...
        async with httpx.AsyncClient(
            ...
            response = await client.get(
                DEFAULT_COPILOT_TOKEN_URL, headers=_copilot_headers(github_token.access)
            )
            response.raise_for_status()
            payload = response.json()
