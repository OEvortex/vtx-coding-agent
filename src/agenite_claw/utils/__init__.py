...
from agenite_claw.utils.path import abbreviate_path

__all__ = ["abbreviate_path", "ensure_dir"]
...
for _legacy_name, _target_name in _LEGACY_MODULE_ALIASES.items():
    sys.modules.setdefault(
        f"{__name__}.{_legacy_name}", _LazyModuleAlias(f"{__name__}.{_legacy_name}", _target_name)
    )
