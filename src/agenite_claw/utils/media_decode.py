...
Historically lived in ``agenite_claw.api.server``; now shared by the WebSocket
...
def save_base64_data_url(
    data_url: str, media_dir: Path, *, max_bytes: int | None = None
) -> str | None:
    ...
    ext = (
        _MIME_EXTENSION_OVERRIDES.get(mime_type) or mimetypes.guess_extension(mime_type) or ".bin"
    )
