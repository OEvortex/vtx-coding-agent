...
# Remove default handler and re-add with unified agenite_claw format
logger.remove()
...
from agenite_claw import __logo__, __version__  # noqa: E402
from agenite_claw.agent.loop import AgentLoop  # noqa: E402
from agenite_claw.cli.gateway import create_gateway_app  # noqa: E402
from agenite_claw.cli.stream import StreamRenderer, ThinkingSpinner  # noqa: E402
from agenite_claw.config.paths import get_workspace_path, is_default_workspace  # noqa: E402
from agenite_claw.config.schema import Config  # noqa: E402
from agenite_claw.utils.evaluator import evaluate_response  # noqa: E402
from agenite_claw.utils.helpers import sync_workspace_templates  # noqa: E402
from agenite_claw.utils.restart import (  # noqa: E402
...
app = typer.Typer(
    ...
    help=f"{__logo__} agenite-claw - Personal AI Assistant",
...
def _init_prompt_session() -> None:
    ...
    from agenite_claw.config.paths import get_cli_history_path
...
def _print_agent_response(
    response: str, render_markdown: bool, metadata: dict | None = None, show_header: bool = True
) -> None:
    ...
    if show_header:
        console.print()
        console.print(f"[cyan]{__logo__} agenite_claw[/cyan]")
    console.print(body)
    console.print()
...
async def _print_interactive_response(
    response: str, render_markdown: bool, metadata: dict | None = None
) -> None:
    ...
    def _write() -> None:
        ...
        ansi = _render_interactive_ansi(
            lambda c: (
                c.print(),
                c.print(f"[cyan]{__logo__} agenite_claw[/cyan]"),
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} agenite_claw v{__version__}")
        raise typer.Exit()
...
def main(
    ...
    """agenite-claw - Personal AI Assistant."""
    pass
...
def onboard(
    ...
    """Initialize agenite_claw configuration and workspace."""
    from agenite_claw.config.loader import (
        get_config_path,
        load_config,
        save_config,
        set_config_path,
    ...
    if wizard:
        from agenite_claw.cli.onboard import run_onboard
        ...
        try:
            ...
            console.print(
                "[yellow]Please run 'agenite_claw onboard' again to complete setup.[/yellow]"
            )
            raise typer.Exit(1)
    ...
    agent_cmd = 'agenite_claw agent -m "Hello!"'
    gateway_cmd = "agenite-claw gateway"
    if config:
        agent_cmd += f" --config {config_path}"
        gateway_cmd += f" --config {config_path}"

    console.print(f"\n{__logo__} agenite_claw is ready!")
    console.print("\nNext steps:")
    if wizard:
    ...
    console.print(
        "\n[dim]Want Telegram/WhatsApp? See: https://github.com/HKUDS/agenite_claw#-chat-apps[/dim]"
    )
...
def _onboard_plugins(config_path: Path) -> None:
    ...
    from agenite_claw.channels.registry import discover_all
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from agenite_claw.config.loader import load_config, resolve_config_env_vars, set_config_path
    ...
    # the agenite_claw config has no API keys configured.
...
def _warn_deprecated_config_keys(config_path: Path | None) -> None:
    ...
    from agenite_claw.config.loader import get_config_path
...
def _migrate_cron_store(config: "Config") -> None:
    ...
    from agenite_claw.config.paths import get_cron_dir
...
def serve(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show agenite-claw runtime logs"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
):
    ...
    try:
        ...
        console.print(
            "[red]aiohttp is required. Install with: pip install 'agenite-claw[api]'[/red]"
        )
        raise typer.Exit(1)

    from loguru import logger

    from agenite_claw.api.server import create_app
    from agenite_claw.bus.queue import MessageBus
    from agenite_claw.providers.image_generation import image_gen_provider_configs
    from agenite_claw.session.manager import SessionManager

    if verbose:
        logger.enable("agenite_claw")
    else:
        logger.disable("agenite_claw")
...
def _run_gateway(
    ...
    *,
    ...
    from agenite_claw.agent.tools.message import MessageTool
    from agenite_claw.bus.queue import MessageBus
    from agenite_claw.bus.runtime_events import RuntimeEventBus
    from agenite_claw.channels.manager import ChannelManager
    from agenite_claw.cron.bound_runner import run_bound_cron_job
    from agenite_claw.cron.service import CronJobSkippedError, CronService
    from agenite_claw.cron.session_turns import is_bound_cron_job
    from agenite_claw.cron.types import CronJob
    from agenite_claw.providers.factory import build_provider_snapshot, load_provider_snapshot
    from agenite_claw.providers.image_generation import image_gen_provider_configs
    from agenite_claw.session.manager import SessionManager
    ...
    console.print(
        f"{__logo__} Starting agenite-claw gateway version {__version__} on port {port}..."
    )
    ...
    from agenite_claw.session.keys import session_key_for_channel

    def _channel_session_key(channel: str, chat_id: str) -> str:
        return session_key_for_channel(
            channel, chat_id, unified_session=config.agents.defaults.unified_session
        )
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        logger.warning("Cron: skipped unbound agent job '{}' ({}): {}", job.name, job.id, reason)
    ...
    from agenite_claw.cron.types import CronJob, CronPayload, CronSchedule
    ...
    async def run():
        ...
        restore_shutdown_handlers = _install_gateway_shutdown_handlers(
            asyncio.get_running_loop(), shutdown_event, tasks, console.print
        )
        try:
            await cron.start()
            tasks = [
                asyncio.create_task(agent.run(), name="agenite_claw-agent-loop"),
                asyncio.create_task(channels.start_all(), name="agenite_claw-channels"),
            ]
            if health_server_enabled:
                tasks.append(
                    asyncio.create_task(
                        _health_server(config.gateway.host, port),
                        name="agenite_claw-health-server",
                    )
                )
            if open_browser_url:
                tasks.append(
                    asyncio.create_task(
                        _open_browser_when_ready(), name="agenite_claw-open-browser"
                    )
                )
            runtime_tasks = asyncio.gather(*tasks)
            shutdown_task = asyncio.create_task(
                shutdown_event.wait(), name="agenite_claw-gateway-shutdown"
            )
            done, _pending = await asyncio.wait(
                {runtime_tasks, shutdown_task}, return_when=asyncio.FIRST_COMPLETED
            )
...
def agent(
    ...
    logs: bool = typer.Option(
        False, "--logs/--no-logs", help="Show agenite-claw runtime logs during chat"
    ),
):
    ...
    from agenite_claw.config.loader import get_config_path as _get_config_path

    cfg_path = Path(config) if config else _get_config_path()
    if not cfg_path.exists():
        console.print("\n[cyan]No configuration found. Let's set things up![/cyan]\n")
        from agenite_claw.cli.onboard import run_onboard
        from agenite_claw.config.loader import save_config as _save_config
    ...
    if restart_notice and should_show_cli_restart_notice(restart_notice, session_id):
        _print_agent_response(
            format_restart_completed_message(restart_notice.started_at_raw), render_markdown=False
        )
    ...
    if message:
        ...
        from agenite_claw.bus.events import InboundMessage
...
def channels_status(
    ...
    """Show channel status."""
    from agenite_claw.channels.registry import discover_all
    from agenite_claw.config.loader import load_config, set_config_path
    ...
    for name, cls in sorted(discover_all().items()):
        ...
        table.add_row(
            cls.display_name, "[green]\u2713[/green]" if enabled else "[dim]\u2717[/dim]"
        )
...
def plugins_list():
    ...
    from agenite_claw.channels.registry import discover_all, discover_channel_names
    ...
    for name in sorted(all_channels):
        ...
        if section is None:
            ...
            enabled = getattr(section, "enabled", False)
        table.add_row(
            cls.display_name, source, "[green]yes[/green]" if enabled else "[dim]no[/dim]"
        )
...
def status():
    ...
    console.print(f"{__logo__} agenite_claw Status\n")
    ...
    if config_path.exists():
        ...
        # Check API keys from registry (vtx catalog + agenite_claw-specific OAuth)
        from agenite_claw.providers.registry import list_providers
...
def _resolve_oauth_provider(provider: str):
    ...
    from agenite_claw.providers.registry import ProviderSpec, find_by_name, list_providers

    key = provider.replace("-", "_")
    spec = find_by_name(provider)
    if not spec or not spec.is_oauth:
        # AgeniteClaw-specific OAuth providers not in vtx's catalog
        oauth_providers = {"openai_codex": "OpenAI Codex", "github_copilot": "GitHub Copilot"}
...
def _logout_github_copilot() -> None:
    ...
    try:
        from agenite_claw.providers.github_copilot_provider import get_storage
    except ImportError:
        console.print(
...
def _login_github_copilot() -> None:
    try:
        from agenite_claw.providers.github_copilot_provider import login_github_copilot
