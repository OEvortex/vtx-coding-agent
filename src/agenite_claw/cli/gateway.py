...
from agenite_claw.gateway import (
    GatewayRuntime,
    GatewayRuntimePaths,
    GatewayStartOptions,
    GatewayStatus,
)
from agenite_claw.gateway.service import (
...
def create_gateway_app(
    *,
    ...
    gateway_app = typer.Typer(
        help="Start and manage the agenite-claw gateway.",
    ...
    def runtime_for_instance(*, workspace: str | None = None, config: str | None = None):
        ...
        return GatewayRuntime(
            paths=GatewayRuntimePaths.for_instance(
                data_dir=data_dir, workspace=workspace_path, config_path=config_path
            )
        )
    ...
    def start_options(
        *, port: int | None, verbose: bool, workspace: str | None, config: str | None
    ) -> GatewayStartOptions:
    ...
    def gateway(
        ...
        """Start the agenite-claw gateway."""
        ...
        if background:
            ...
            result = runtime.start_background(
                start_options(port=port, verbose=verbose, workspace=workspace, config=config)
            )
            if result.ok:
    ...
    def gateway_install_service(
        ...
        verbose: bool = typer.Option(False, "--verbose", "-v", help="Verbose output"),
        config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
        name: str = typer.Option("agenite_claw-gateway", "--name", help="Service name"),
    ...
    def gateway_uninstall_service(
        ...
        dry_run: bool = typer.Option(
            False, "--dry-run", help="Print actions without uninstalling"
        ),
    ) -> None:
        ...
        try:
            ...
            console.print(
                f"[red]Service uninstall failed while running: {' '.join(exc.cmd)}[/red]"
            )
            raise typer.Exit(exc.returncode or 1) from exc
        except OSError as exc:
