"""Interactive onboarding questionnaire for agenite_claw."""
...
from agenite_claw.cli.models import (
    format_token_count,
    get_model_context_limit,
    get_model_suggestions,
)
from agenite_claw.config.loader import get_config_path, load_config
from agenite_claw.config.schema import Config, ModelPresetConfig
...
_QUICK_START_ENDPOINT_CHOICES: dict[str, tuple[_QuickStartEndpointChoice, ...]] = {
    ...
    "minimax_anthropic": (
        ...
        _QuickStartEndpointChoice(
            "Mainland China Anthropic Token Plan", "https://api.minimaxi.com/anthropic"
        ),
    ),
    "stepfun": (
...
def _select_with_back(
    ...
    # Style
    style = Style.from_dict({"selected": f"fg:{_UI_ACCENT} bold", "question": f"fg:{_UI_TEXT}"})
...
def _validate_field_constraint(value: Any, field_info) -> str | None:
    ...
    for m in field_info.metadata:
        ...
        if hasattr(m, "gt") and isinstance(value, (int, float)) and value <= m.gt:
            return f"Value must be > {m.gt}"
        if hasattr(m, "le") and isinstance(value, (int, float)) and value > m.le:
            return f"Value must be <= {m.le}"
        if hasattr(m, "lt") and isinstance(value, (int, float)) and value >= m.lt:
...
def _show_main_menu_header() -> None:
    ...
    from agenite_claw import __logo__, __version__

    console.print()
    body = Table.grid(expand=True)
    body.add_column(ratio=1)
    body.add_row(f"{__logo__} [bold {_UI_TEXT}]agenite_claw[/] [{_UI_MUTED}]v{__version__}[/]")
...
def _input_bool(display_name: str, current: bool | None) -> bool | None:
    ...
    return (
        _get_questionary()
        .confirm(display_name, default=bool(current) if current is not None else False)
        .ask()
    )
...
def _input_text(display_name: str, current: Any, field_type: str, field_info=None) -> Any:
    ...
    value = _ask_prompt(
        _get_questionary().text(
            f"{display_name}:", default=default, key_bindings=_input_back_key_bindings()
        )
    )
...
def _input_secret(display_name: str) -> str | None | object:
    ...
    value = _ask_prompt(
        prompt_factory(f"{display_name}:", key_bindings=_input_back_key_bindings())
    )
    if value is _BACK_PRESSED or value is None:
        return None if value is None else _BACK_PRESSED
...
def _input_context_window_with_recommendation(
    ...
    choice = (
        _get_questionary().select(display_name, choices=choices, default="Enter new value").ask()
    )
...
def _handle_fallback_models_field(
    ...
    from agenite_claw.config.schema import InlineFallbackConfig
    ...
    while True:
        ...
        answer = (
            _get_questionary().select("Manage fallback models:", choices=choices, qmark=">").ask()
        )
...
def _handle_search_provider_field(
    ...
    from agenite_claw.agent.tools.web import SEARCH_PROVIDER_OPTIONS
...
def _resolve_field_handler(model: BaseModel, field_name: str) -> Any:
    ...
    if field_name == "provider":
        from agenite_claw.agent.tools.web import WebSearchConfig
...
def _configure_pydantic_model(
    model: BaseModel, display_name: str, *, skip_fields: set[str] | None = None
) -> BaseModel | None:
...
def _try_auto_fill_context_window(model: BaseModel, new_model_name: str) -> None:
    ...
    Note:
        This function imports AgentDefaults from agenite_claw.config.schema to get
    ...
    from agenite_claw.config.schema import AgentDefaults
    ...
    while True:
        try:
            ...
            _show_section_header(
                "Model Presets", "Create, edit or delete named model presets for quick switching"
            )
            ...
            action = _select_with_back(f"Preset: {preset_name}", choices, default="Edit")
            ...
            if action == "Delete":
                confirm = (
                    _get_questionary()
                    .confirm(f"Delete preset '{preset_name}'?", default=False)
                    .ask()
                )
                if confirm:
...
def _get_channel_info() -> dict[str, tuple[str, type[BaseModel]]]:
    ...
    for name, channel_cls in discover_all().items():
        try:
            mod = importlib.import_module(f"agenite_claw.channels.{name}")
...
def _run_channel_login(
    config: Config, channel_name: str, model: BaseModel, display_name: str
) -> bool:
...
def _configure_channel(config: Config, channel_name: str) -> None:
    ...
    updated_channel = _configure_pydantic_model(model, display_name)
...
def _set_primary_quick_start_preset(config: Config, provider_name: str, model: str) -> None:
    ...
    config.model_presets["primary"] = ModelPresetConfig(
        label="Primary", model=model, provider=provider_name
    )
...
def _quick_start_requires_api_key(
...
def _select_quick_start_api_base(
    provider_name: str, provider_display: str, info: _QuickStartProviderInfo | None
    ...
    base_answer = _input_text("Provider base URL", api_base, "str")
...
def _configure_quick_start_provider(config: Config) -> bool | object:
    ...
    while True:
        ...
        answer = _select_with_back(
            "Which provider do you want to use?", [*list(provider_choices), "<- Back"]
        )
        if answer is _BACK_PRESSED or answer is None or answer == "<- Back":
            return _BACK_PRESSED
        assert isinstance(answer, str)
        ...
        if api_base and (base_was_prompted or not provider_config.api_base):
...
def _enable_quick_start_websocket(config: Config) -> bool:
    ...
    if hasattr(model, "token_issue_secret"):
        model.token_issue_secret = webui_secret
    if hasattr(model, "websocket_requires_token"):
        model.websocket_requires_token = True
    config.channels.websocket = model.model_dump(by_alias=True, exclude_none=True)
    return True
...
def _configure_quick_start_gateway(config: Config) -> bool:
    ...
    console.print()
    console.print(
        f"[{_UI_MUTED}]The gateway runs agenite-claw as a background service, enabling[/]"
    )
    console.print(
    ...
    if install_service:
        ...
        console.print(f"  [cyan]agenite-claw gateway install-service[/cyan]")
    console.print()
    return True
...
def _configure_quick_start_profile(config: Config) -> bool:
    ...
    console.print()
    console.print(f"[{_UI_MUTED}]This info helps agenite-claw personalize its responses.[/]")
...
def _show_quick_start_summary(config: Config) -> None:
    ...
    console.print(f"  1. Start:  [cyan]agenite-claw gateway[/cyan]")
    console.print(f"  2. Open:   [cyan]http://127.0.0.1:{config.gateway.port}[/cyan]")
    console.print(f'  3. Chat:   [cyan]agenite-claw agent -m "Hello!"[/cyan]')
    console.print()
    console.print(f"[{_UI_MUTED}]Add more channels anytime with: agenite-claw onboard --wizard[/]")
...
def _configure_quick_start(config: Config) -> bool:
    ...
    # Step 3: Gateway
    console.clear()
    _show_section_header("Gateway", "Configure how agenite-claw runs as a service.")
    _configure_quick_start_gateway(draft)

    # Step 4: Personal Profile
    console.clear()
    _show_section_header("Your Profile", "Personalize agenite-claw with your info.")
...
def _prompt_main_menu_exit(has_unsaved_changes: bool) -> str:
    ...
    answer = (
        ...
        .select(
            ...
            choices=["[S] Save and Exit", "[X] Exit Without Saving", "[R] Resume Editing"],
...
def _get_main_menu_choices(has_unsaved_changes: bool) -> list[str]:
    ...
    choices = [_QUICK_START_MENU_CHOICE, "[A] Advanced Settings"]
...
def _configure_advanced_settings(config: Config) -> None:
    ...
    while True:
        try:
            console.clear()
            _show_section_header(
                "Advanced Settings", "Use these when the default API-key setup is not enough."
            )
            answer = _select_with_back(
                "What would you like to configure?", choices, default=last_choice
            )
        except KeyboardInterrupt:
            break
...
def run_onboard(initial_config: Config | None = None) -> OnboardResult:
    ...
    if initial_config is not None:
        ...
        base_config = load_config() if config_path.exists() else Config()
