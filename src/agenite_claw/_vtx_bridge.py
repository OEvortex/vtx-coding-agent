...
# ═══════════════════════════════════════════════════════════════════════
# Vtx config bridge — read ~/.vtx/config.yml and merge into agenite_claw Config
# ═══════════════════════════════════════════════════════════════════════
...
def _already_configured(config: Any) -> bool:
    """Return True if any agenite-claw provider already has an API key set."""
...
def _set_provider(
    ...
    *,
    ...
    """Set a provider on agenite_claw's Config using the vtx slug as field name.
    ...
    so agenite_claw's ``_match_provider`` resolves via the **forced-provider**
    ...
    # Set the active provider and model on agent defaults so agenite_claw's
    ...
    # chosen a provider yet in their agenite-claw config).
...
def merge_vtx_config(config: Any) -> Any:
    """Merge settings from vtx config into a agenite_claw Config.
    ...
    2. If the user has NOT explicitly configured a provider in agenite-claw
    ...
    # This ensures agenite-claw can find credentials regardless of which provider
