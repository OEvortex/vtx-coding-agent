"""OpenAI-compatible HTTP API server for a fixed agenite_claw session.
...
from agenite_claw.utils.media_decode import MAX_FILE_SIZE
from agenite_claw.utils.media_decode import FileSizeExceeded as _FileSizeExceeded
from agenite_claw.utils.media_decode import save_base64_data_url as _save_base64_data_url
...
async def handle_chat_completions(request: web.Request) -> web.StreamResponse | web.Response:
    ...
    model_name: str = request.app.get("model_name", "agenite_claw")
...
async def handle_models(request: web.Request) -> web.Response:
    """GET /v1/models"""
    model_name = request.app.get("model_name", "agenite_claw")
    return web.json_response(
        {
            "object": "list",
            "data": [
                {"id": model_name, "object": "model", "created": 0, "owned_by": "agenite_claw"}
            ],
        }
    )
...
def create_app(
    agent_loop, model_name: str = "agenite_claw", request_timeout: float = 120.0
) -> web.Application:
