...
class RunStream:
    ...
    def __init__(
        self, task: asyncio.Task[RunResult], queue: asyncio.Queue[StreamEvent | object]
    ) -> None:
        self._task = task
        self._queue = queue
...
class SDKStreamEmitter:
    ...
    async def text_delta(self, delta: str, *, iteration: int | None = None) -> None:
        ...
        await self.emit(
            StreamEvent(type=STREAM_EVENT_TEXT_DELTA, delta=delta, iteration=iteration)
        )

    async def text_completed(
        self, *, resuming: bool = False, iteration: int | None = None, force: bool = True
    ) -> None:
...
class SDKStreamingHook(AgentHook):
    ...
    async def emit_reasoning(self, reasoning_content: str | None) -> None:
        ...
        await self._emitter.emit(
            StreamEvent(type=STREAM_EVENT_REASONING_DELTA, delta=reasoning_content)
        )
