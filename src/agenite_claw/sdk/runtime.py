...
def ensure_single_model_selector(*, model: str | None, model_preset: str | None) -> None:
...
class SDKRuntimeController:
    ...
    @asynccontextmanager
    async def override(
        self, *, model: str | None, model_preset: str | None
    ) -> AsyncIterator[None]:
        ...
        async with self._gate.slot(exclusive=exclusive):
            ...
            if override is not None:
                self._loop._apply_provider_snapshot(
                    override, publish_update=False, model_preset=model_preset
                )
            try:
                yield
            finally:
                if restore is not None:
                    self._restore_snapshot(restore, provider_signature=restore_signature)
    ...
    def model_override_snapshot(
        ...
        from agenite_claw.agent.model_presets import build_static_preset_snapshot
    ...
    def _restore_snapshot(
        self, snapshot: ProviderSnapshot, *, provider_signature: tuple[object, ...] | None
    ) -> None:
