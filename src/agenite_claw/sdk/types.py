...
@dataclass(slots=True)
class StreamEvent:
    """A typed event emitted by ``AgeniteClaw.stream()`` and ``RunStream``."""
...
class SessionSnapshot:
    """A durable snapshot of one agenite_claw session."""
...
def result_from_response(response: Any, capture: Any) -> RunResult:
    ...
    if isinstance(stop_reason, str):
        from agenite_claw._vtx_bridge import claw_verdict_to_vtx
