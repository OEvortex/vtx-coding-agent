...
from agenite_claw.cron.types import CronSchedule

if TYPE_CHECKING:
    from agenite_claw.agent.tools.cli_apps import CliAppsToolConfig
    from agenite_claw.agent.tools.filesystem import FileToolsConfig
    ...
    from agenite_claw.agent.tools.web import WebToolsConfig
...
class TranscriptionConfig(Base):
    ...
    enabled: bool = True
    provider: str | None = None  # Validated by agenite_claw.audio.transcription_registry.
    model: str | None = None
...
class ModelPresetConfig(Base):
    ...
    def to_generation_settings(self) -> Any:
        from agenite_claw.providers.base import GenerationSettings
...
class AgentDefaults(Base):
    ...
    bot_name: str = (
        "agenite_claw"  # Display name shown in CLI prompts (e.g. "{name} is thinking...")
    )
    bot_icon: str = (
...
class ProviderConfig(Base):
    ...
    # agenite_claw/providers/openai_compat_provider.py. Kept duplicated here to
...
class ToolsConfig(Base):
    ...
    web: WebToolsConfig = Field(
        default_factory=lambda: _lazy_default("agenite_claw.agent.tools.web", "WebToolsConfig")
    )
    exec: ExecToolConfig = Field(
        default_factory=lambda: _lazy_default("agenite_claw.agent.tools.shell", "ExecToolConfig")
    )
    file: FileToolsConfig = Field(
        default_factory=lambda: _lazy_default(
            "agenite_claw.agent.tools.filesystem", "FileToolsConfig"
        )
    )
    cli_apps: CliAppsToolConfig = Field(
        default_factory=lambda: _lazy_default(
            "agenite_claw.agent.tools.cli_apps", "CliAppsToolConfig"
        )
    )
    my: MyToolConfig = Field(
        default_factory=lambda: _lazy_default("agenite_claw.agent.tools.self", "MyToolConfig")
    )
    image_generation: ImageGenerationToolConfig = Field(
        default_factory=lambda: _lazy_default(
            "agenite_claw.agent.tools.image_generation", "ImageGenerationToolConfig"
        )
    )
...
class Config(BaseSettings):
    """Root configuration for agenite_claw."""
    ...
    model_config = ConfigDict(env_prefix="AGENITE_CLAW_", env_nested_delimiter="__")  # type: ignore[call-arg]
...
def _resolve_tool_config_refs() -> None:
    ...
    like ``from agenite_claw.config.schema import ExecToolConfig`` continue to work.
    """
    import sys
