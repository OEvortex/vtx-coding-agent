...
from agenite_claw.utils.helpers import ensure_dir


def get_config_path() -> Path:
    """Get the configuration file path (lazy import to break circular dependency).

    Delegates to ``agenite_claw.config.loader.get_config_path`` at call time so
    that importing this module never triggers a circular import during startup.
    """
    from agenite_claw.config.loader import get_config_path as _loader_get_config_path
...
def is_default_workspace(workspace: str | Path | None) -> bool:
    """Return whether a workspace resolves to agenite_claw's default workspace path."""
    current = (
