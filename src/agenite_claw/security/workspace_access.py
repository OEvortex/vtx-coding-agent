...
_CURRENT_WORKSPACE_SCOPE: ContextVar[WorkspaceScope | None] = ContextVar(
    "agenite_claw_workspace_scope", default=None
)
...
class WorkspaceScopeResolver:
    ...
    def for_turn(
        self, *, channel: str | None, message_metadata: Any, session_metadata: Any
    ) -> WorkspaceScope:
...
def workspace_sandbox_status(
    *, restrict_to_workspace: bool, workspace: str | Path, environ: dict[str, str] | None = None
    ...
    return WorkspaceSandboxStatus(
        ...
        provider="none",
        provider_label=_provider_label("none"),
        summary="Workspace restriction uses agenite_claw application-level guards.",
    )
...
def build_workspace_scope(
    project_path: str | Path, access_mode: str, *, source_channel: str | None = None
) -> WorkspaceScope:
    ...
    *,
    ...
    if raw is None:
        return default_workspace_scope(
            default_workspace, default_restrict_to_workspace, source_channel=source_channel
        )
...
def _env_system_provider(environ: dict[str, str] | None = None) -> str | None:
    ...
    explicit_provider = env.get("AGENITE_CLAW_WORKSPACE_SANDBOX_PROVIDER")
    enforced = env.get("AGENITE_CLAW_WORKSPACE_SANDBOX_ENFORCED")
    compatibility = env.get("AGENITE_CLAW_SANDBOX_ENFORCED")
