"""High-level programmatic interface to agenite_claw."""
...
from agenite_claw.agent.hook import AgentHook, SDKCaptureHook
...
from agenite_claw.sdk.clients import MemoryClient, RuntimeClient, SessionClient
from agenite_claw.sdk.runtime import (
    SDKRuntimeController,
    build_process_direct_kwargs,
    ensure_single_model_selector,
)
from agenite_claw.sdk.streaming import RunStream, SDKStreamEmitter, SDKStreamingHook
from agenite_claw.sdk.types import (
...
class AgeniteClaw:
    """Programmatic facade for running the agenite_claw agent.

    Usage::

        bot = AgeniteClaw.from_config()
    ...
    def from_config(
        ...
        *,
        ...
        """Create a AgeniteClaw instance from a config file.

        Args:
            config_path: Path to ``config.json``.  Defaults to
                ``~/.agenite_claw/config.json``.
        ...
        loop = AgentLoop.from_config(
            config, image_generation_provider_configs=image_gen_provider_configs(config)
        )
    ...
    async def run(
        ...
        *,
        ...
        async with self._runtime_overrides.override(model=model, model_preset=model_preset):
            ...
            response = await self._loop.process_direct(message, **kwargs, hooks=per_run_hooks)
            ...
            async with self._runtime_overrides.override(model=model, model_preset=model_preset):
                ...
                try:
                    response = await self._loop.process_direct(
                        message, **kwargs, hooks=per_run_hooks
                    )
    ...
    async def __aenter__(self) -> AgeniteClaw:
        return self
