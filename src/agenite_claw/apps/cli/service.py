...
from agenite_claw.apps.protocol import app_manifest, compact_dict
...
AGENITE_CLAW_EXTENSION_REGISTRY_URL = (
    "https://raw.githubusercontent.com/Re-bin/agenite_claw-extension/main/registry.json"
)
AGENITE_CLAW_EXTENSION_RAW_BASE = (
    "https://raw.githubusercontent.com/Re-bin/agenite_claw-extension/main"
)
_CATALOG_SOURCES = (
    ...
    ("extensions", AGENITE_CLAW_EXTENSION_REGISTRY_URL, AGENITE_CLAW_EXTENSION_RAW_BASE, False),
)
...
class CliAppManager:
    ...
    def _fetch_registry(
        self, url: str, cache_path: Path, *, force_refresh: bool = False
    ) -> dict[str, Any]:
        ...
        for source, url, _raw_base, required in _CATALOG_SOURCES:
            try:
                await self._fetch_registry_async(
                    url, self._cache_path(source), force_refresh=force_refresh
                )
            except Exception:
                if required:
                    raise
    ...
    def _app_payload(self, app: dict[str, Any], installed: dict[str, Any]) -> dict[str, Any]:
        name = str(app["name"])
    ...
    def _manifest_payload(
        self, app: dict[str, Any], *, logo_url: str | None, brand_color: str | None
    ) -> dict[str, Any]:
        name = str(app["name"])
        entry_point = str(app.get("entry_point") or "")
        strategy = self._strategy(app)
        ...
        capabilities = [
            compact_dict(
                {"type": "cli", "entry_point": entry_point, "package": self._package_ref(app)}
            ),
        ...
        remove = compact_dict(
            {
                ...
                "verification": (
                    ...
                    else ["agenite_claw_state_absent", "managed_paths_absent"]
                ),
            }
        )
        return app_manifest(
    ...
    def installed_payload(self) -> dict[str, Any]:
        ...
        return {"apps": rows, "installed_count": len(rows), "catalog_updated_at": None}
    ...
    def _pip_uninstall_argv(
        self, app: dict[str, Any], installed_entry: dict[str, Any] | None = None
    ) -> list[str]:
    ...
    def _retry_stale_npm_install(
        self, app: dict[str, Any], argv: list[str], result: subprocess.CompletedProcess[str]
    ...
    def _argv_for_action(
        self, app: dict[str, Any], action: str, installed_entry: dict[str, Any] | None = None
    ) -> list[str] | None:
        strategy = self._strategy(app)
        if strategy == "pip":
            if action == "install":
    ...
    def _fallback_skill(self, app: dict[str, Any]) -> str:
        ...
        description = _catalog_description(app) or f"Use {display} from agenite_claw."
        return f"""---
...
Use this skill when the user asks agenite_claw to operate {display} through its installed CLI app.
...
"""
    ...
    def _with_agenite_claw_skill_note(self, content: str, app: dict[str, Any]) -> str:
        marker = "<!-- agenite_claw-cli-app-note -->"
...
"""
    ...
    def install_skill(self, app: dict[str, Any]) -> Path:
        ...
        content = self._with_agenite_claw_skill_note(content, app)
    ...
    def install(self, name: str) -> dict[str, Any]:
        ...
        if strategy == "bundled":
            ...
            note = (
                app.get("install_notes")
                or f"{app['display_name']} is bundled with its parent app."
            )
    ...
    def uninstall(self, name: str) -> dict[str, Any]:
        ...
        if strategy != "bundled":
            ...
            if still_managed or (not managed_entry_path and still_available):
                ...
                message = (
                    ...
                    "so agenite_claw kept it installed."
                )
                return self.payload() | {
                    "last_action": {
                        "ok": False,
        ...
        if strategy == "bundled" and still_available:
            message = (
                f"Removed {app['display_name']} from agenite_claw. {entry_point} "
                "is still available because it is managed outside agenite_claw."
            )
    ...
    def _resolve_cwd(self, working_dir: str | None, *, restrict_to_workspace: bool) -> Path:
    ...
    def _changed_artifacts(self, cwd: Path, before: dict[Path, tuple[int, int]]) -> list[Path]:
