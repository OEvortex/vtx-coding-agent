...
from agent.dispatcher import DispatcherContext, get_context
from agent.tools.base import BaseTool
from coding_agent.config import config as vtx_config
from protocol.types import StopReason, TextContent, ToolResult, Usage
...
class SubagentSpec:
    ...
    def to_agentdef(self) -> Any:
        ...
        from agent.agents.schema import AgentDef, InstructionsMode, ThinkingLevel
...
def _build_subagent_tool_list(parent_ctx: DispatcherContext, spec: SubagentSpec) -> list[Any]:
    ...
    from agent.tools import DEFAULT_TOOLS, tools_by_name
    ...
    from agent.agents.activate import compose_active_tools
...
def _resolve_api_and_base_url(
    ...
    from agent.runtime import default_base_url_for_api, default_base_url_for_provider
...
async def _run_subagent(
    ...
    from agent.runtime import create_provider
...
class TaskTool(BaseTool[TaskParams]):
    ...
    async def _execute_background(self, params: TaskParams, parent_ctx: Any) -> ToolResult:
        ...
        """
        from agent.tools.background import get_manager
