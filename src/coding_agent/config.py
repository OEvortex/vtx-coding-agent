...
from coding_agent.themes import ColorsConfig, get_theme, get_theme_ids
...
def _load_default_config_yaml() -> dict[str, Any]:
    ...
    return (
        yaml.safe_load(
            resources.files("coding_agent.defaults")
            .joinpath("config.yml")
...
def _resolve_default_system_prompt() -> str:
    ...
    """
    from ai.agent.prompts.identity import DEFAULT_VTX_BASE
...
def get_config_dir() -> Path:
    from core.paths import get_config_dir as _get_config_dir

    return _get_config_dir()


def get_agents_dir() -> Path:
    from core.paths import get_agents_dir as _get_agents_dir
...
def _migrate_v10_to_v11(data: dict[str, Any]) -> dict[str, Any]:
    """No-op migration. Goal config removed."""
    migrated = Config._apply_legacy_key_shims(data)
...
def set_model_provider_filter(provider: str) -> Config:
    ...
    """
    from ai.provider_catalog import _load as _load_providers
...
class _ConfigProxy:
    """Proxy that delegates to get_config() for runtime reloading."""

    def __getattr__(self, name):
        return getattr(get_config(), name)


# Lives in this module (not the package __init__) so that cross-package
# imports (`from coding_agent.config import config`) resolve identically
