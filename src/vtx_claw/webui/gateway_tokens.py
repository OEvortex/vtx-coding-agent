...
from vtx_claw.webui.http_utils import bearer_token, parse_query, query_first
...
class GatewayTokenStore:
    ...
    def can_issue(self, *, include_api_token: bool = False) -> bool:
        ...
        if len(self.issued_tokens) >= self.max_tokens:
            return False
        return not (include_api_token and len(self.api_tokens) >= self.max_tokens)
    ...
    def take_issued_token_if_valid(self, token_value: str | None) -> bool:
        ...
        if expiry is None:
            return False
        return not time.monotonic() > expiry
