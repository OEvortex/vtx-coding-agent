...
from vtx_claw.session.manager import _message_preview_text
...
class _CronServiceLike(Protocol):
    ...
    def list_bound_cron_jobs_for_session(
        self, session_key: str, *, include_disabled: bool = True
    ) -> list[CronJob]: ...
...
def session_automation_jobs(
    cron_service: _CronServiceLike | None, session_key: str
) -> list[CronJob]:
    """Return user automations attached to the WebUI session."""
    if cron_service is None:
        return []
    return cron_service.list_bound_cron_jobs_for_session(session_key, include_disabled=True)
    ...
    *,
    ...
    return {
        "jobs": serialize_automation_jobs(
            session_automation_jobs(cron_service, session_key), pending_job_ids=pending_job_ids
        )
    }
...
def _origin_payload(
    job: CronJob, session_manager: _SessionManagerLike | None
) -> dict[str, Any] | None:
    ...
    if channel != "websocket":
        return {"channel": channel, "title": title, "preview": preview}
