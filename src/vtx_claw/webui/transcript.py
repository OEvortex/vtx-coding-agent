...
from vtx_claw.config.paths import get_webui_dir
...
_INLINE_MARKDOWN_VIDEO_EXTS: frozenset[str] = frozenset({".mp4", ".mov", ".webm"})
_INLINE_MARKDOWN_MEDIA_EXTS = _INLINE_MARKDOWN_IMAGE_EXTS | _INLINE_MARKDOWN_VIDEO_EXTS
_FILE_EDIT_TOOL_NAMES: frozenset[str] = frozenset({"write_file", "edit_file", "apply_patch"})
_TURN_DISPLAY_EVENTS: frozenset[str] = frozenset(
    {"reasoning_delta", "reasoning_end", "delta", "stream_end", "message", "file_edit", "turn_end"}
)


def rewrite_local_markdown_images(
    text: str, *, workspace_path: Path, sign_path: Callable[[Path], Mapping[str, Any] | None]
) -> str:
...
def _encode_page_cursor(before_turn_ordinal: int) -> str:
    raw = json.dumps(
        {"before_turn": before_turn_ordinal}, separators=(",", ":"), ensure_ascii=False
    ).encode("utf-8")
        ...
        if active_turn_count > 0:
            refs.append(
                _TranscriptChunkRef(
                    ...
                    active_turn_count,
                    sum(
                        1 for turn in active_turns for row in turn if _is_user_transcript_row(row)
                    ),
                )
            )
    return refs
...
def _count_user_messages_before_ordinal(
    session_key: str, chunks: list[_TranscriptChunkRef], before_ordinal: int
) -> int:
    total = 0
    for chunk in chunks:
...
def _select_transcript_page(
    session_key: str, *, limit: int | None, before: str | None, _manifest_rebuilt: bool = False
    ...
    for chunk in reversed(chunks):
        ...
        if (
            ...
            return _select_transcript_page(
                session_key, limit=limit, before=before, _manifest_rebuilt=True
            )
    ...
    page = {
        ...
        "user_message_offset": _count_user_messages_before_ordinal(
            session_key, chunks, first_ref.ordinal
        ),
    }
    return lines, page
    ...
    def append_user_message(
        ...
        *,
        ...
        payload = build_user_transcript_event(
            chat_id, text, media_paths=media_paths, cli_apps=cli_apps, mcp_presets=mcp_presets
        )
        if payload is None:
            return
...
def fork_transcript_before_user_index(
    source_key: str, target_key: str, before_user_index: int
) -> bool:
...
def append_fork_marker(session_key: str) -> None:
    ...
    append_transcript_object(
        session_key,
        {"event": WEBUI_FORK_MARKER_EVENT, "chat_id": _chat_id_from_session_key(session_key)},
    )


def write_session_messages_as_transcript(target_key: str, messages: list[dict[str, Any]]) -> None:
...
def build_user_transcript_event(
    ...
    *,
    ...
    if not text and not paths:
        return None
    event: dict[str, Any] = {"event": "user", "chat_id": chat_id, "text": text}
    if paths:
...
def _session_user_event(session_key: str, message: dict[str, Any]) -> dict[str, Any] | None:
...
def _session_backfill_turns(
    session_key: str, session_messages: list[dict[str, Any]]
...
def _with_backfilled_user(
    records: list[dict[str, Any]], user_event: dict[str, Any]
) -> list[dict[str, Any]]:
...
def inject_missing_user_events_from_session(
    session_key: str, lines: list[dict[str, Any]], session_messages: list[dict[str, Any]] | None
) -> list[dict[str, Any]]:
    ...
    for turn in _split_transcript_turns(lines):
        ...
        if match_index is None:
            ...
            continue
        out.extend(
            turn if has_user else _with_backfilled_user(turn, session_turns[match_index][0])
        )
...
def _message_has_file_edit_for_tool_event(message: dict[str, Any], event: dict[str, Any]) -> bool:
...
def _strip_covered_file_edit_tool_hints(
    message: dict[str, Any], edits: list[dict[str, Any]]
) -> dict[str, Any]:
...
def _merge_unique_tool_trace_lines(
    previous_traces: list[str], lines: list[str]
) -> tuple[list[str], bool]:
...
def _media_from_signed_urls(value: Any) -> list[dict[str, Any]]:
    ...
    for m in urls:
        if isinstance(m, dict) and m.get("url"):
            ...
            media.append({"kind": _media_kind_from_name(name), "url": str(m["url"]), "name": name})
    return media
...
def replay_transcript_to_ui_messages(
    ...
    *,
    ...
    Mirrors the core fold in ``useVtxClawStream.ts`` (delta, reasoning,
    ...
    def attach_reasoning_chunk(
        prev: list[dict[str, Any]], chunk: str, idx: int, turn_fields: dict[str, Any] | None = None
    ) -> None:
    ...
    def find_active_placeholder(
        prev: list[dict[str, Any]], turn_fields: dict[str, Any] | None = None
    ) -> str | None:
        turn_fields = turn_fields or {}
    ...
    def stamp_latency(latency_ms: int) -> None:
        for i in range(len(messages) - 1, -1, -1):
            if messages[i].get("role") == "assistant" and messages[i].get("kind") != "trace":
                messages[i] = {**messages[i], "latencyMs": latency_ms, "isStreaming": False}
                return
    ...
    def absorb_complete(extra: dict[str, Any], idx: int) -> None:
        ...
        if last and is_reasoning_only_placeholder(last) and _same_turn(last, extra):
            messages[-1] = {**last, **extra, "isStreaming": False, "reasoningStreaming": False}
        else:
            messages.append(
                {
                    "id": _new_id("as", idx),
    ...
    def find_file_edit_trace_index(segment: str | None, edits: list[dict[str, Any]]) -> int | None:
    ...
    def upsert_file_edits(
        edits: list[dict[str, Any]], idx: int, turn_fields: dict[str, Any] | None = None
    ) -> None:
...
def has_pending_tool_calls(lines: list[dict[str, Any]]) -> bool:
    ...
    for rec in reversed(lines):
        ...
        if ev in {"delta", "stream_end", "reasoning_delta", "reasoning_end", "file_edit"}:
            return True
