...
from vtx_claw.command.builtin import builtin_command_palette
from vtx_claw.cron.session_turns import is_bound_cron_job
from vtx_claw.cron.types import CronJob, CronSchedule
from vtx_claw.utils.subagent_channel_display import scrub_subagent_messages_for_channel
from vtx_claw.webui.file_preview import WebUIFilePreviewError, file_preview_payload
from vtx_claw.webui.gateway_tokens import GatewayTokenStore, token_response_payload
from vtx_claw.webui.http_utils import case_insensitive_header as _case_insensitive_header
from vtx_claw.webui.http_utils import host_for_url as _host_for_url
from vtx_claw.webui.http_utils import http_error as _http_error
from vtx_claw.webui.http_utils import http_json_response as _http_json_response
from vtx_claw.webui.http_utils import http_response as _http_response
from vtx_claw.webui.http_utils import is_localhost as _is_localhost
...
from vtx_claw.webui.http_utils import safe_host_header as _safe_host_header
from vtx_claw.webui.media_gateway import WebUIMediaGateway
from vtx_claw.webui.session_automations import (
    all_automations_payload,
    serialize_automation_jobs,
    session_automation_jobs,
    session_automations_payload,
)
from vtx_claw.webui.session_list_index import list_webui_sessions
from vtx_claw.webui.sidebar_state import read_webui_sidebar_state, write_webui_sidebar_state
from vtx_claw.webui.skills_api import webui_skill_detail_payload, webui_skills_payload
from vtx_claw.webui.thread_disk import delete_webui_thread
from vtx_claw.webui.transcript import build_webui_thread_response
from vtx_claw.webui.workspaces import WebUIWorkspaceController

_SLOW_WEBUI_HTTP_LOG_MS = 1_000
_AUTOMATION_VALUES_HEADER = "X-VtxClaw-Automation-Values"
...
def _resolve_bootstrap_model_name(runtime_name: Callable[[], str | None] | None) -> str:
...
class GatewayHTTPHandler:
    ...
    def __init__(
        ...
        *,
        ...
        from vtx_claw.webui.settings_api import runtime_capabilities as _rc
        from vtx_claw.webui.settings_routes import WebUISettingsRouter
    ...
    def _handle_file_preview(self, request: WsRequest, key: str) -> Response:
        ...
        try:
            payload = file_preview_payload(
                path, scope=self.workspaces.scope_for_session_key(decoded_key)
            )
    ...
    def _handle_session_automations(self, request: WsRequest, key: str) -> Response:
        ...
        return _http_json_response(
            session_automations_payload(
                self.cron_service, decoded_key, pending_job_ids=pending_job_ids
            )
        )
    ...
    async def _dispatch_automation_routes(self, request: WsRequest, got: str) -> Response | None:
    ...
    async def _handle_webui_automation_action(self, request: WsRequest, action: str) -> Response:
        if not self.check_api_token(request):
            return _http_error(401, "Unauthorized")
    ...
    def _handle_media_fetch(
        ...
        return self.media.serve_signed_media(sig, payload, request=request)
    ...
    def _handle_webui_skills(self, request: WsRequest) -> Response:
        if not self.check_api_token(request):
            return _http_error(401, "Unauthorized")
        return _http_json_response(
            webui_skills_payload(self.skills_workspace_path, disabled_skills=self.disabled_skills)
        )
    ...
    def _handle_webui_skill_detail(self, request: WsRequest, raw_name: str) -> Response:
        ...
        payload = webui_skill_detail_payload(
            self.skills_workspace_path, name, disabled_skills=self.disabled_skills
        )
        if payload is None:
    ...
    def _serve_static(self, request_path: str) -> Response | None:
        ...
        return _http_response(
            body, status=200, content_type=ctype, extra_headers=[("Cache-Control", cache)]
        )
...
def _parse_automation_update(
    values: dict[str, Any], *, current_job: CronJob | None = None
) -> dict[str, Any] | str:
    update: dict[str, Any] = {}
    if "name" in values:
