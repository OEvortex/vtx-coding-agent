...
from vtx_claw.webui.media_api import (
...
from vtx_claw.webui.transcript import rewrite_local_markdown_images
...
class WebUIMediaGateway:
    ...
    def serve_signed_media(
        ...
        return serve_signed_media(
            sig, payload, secret=self.secret, request=request, media_dir=self._media_dir
        )

    def sign_media_path(self, abs_path: Path) -> str | None:
        return sign_media_path(abs_path, secret=self.secret, media_dir=self._media_dir)

    def sign_or_stage_media_path(self, path: Path) -> dict[str, str] | None:
        return sign_or_stage_media_path(
            path, secret=self.secret, media_dir=self._media_dir, logger=self.logger
        )

    def rewrite_local_markdown_images(
        self, text: str, *, workspace_path: Path | None = None
    ) -> str:
    ...
    def augment_transcript_media(self, paths: list[str]) -> list[dict[str, Any]]:
        return signed_media_attachments(paths, sign_path=self.sign_or_stage_media_path)
