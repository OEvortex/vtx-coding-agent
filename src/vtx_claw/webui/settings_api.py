...
from vtx_claw.config.loader import get_config_path, load_config, save_config
from vtx_claw.config.schema import ModelPresetConfig, ProviderConfig
from vtx_claw.providers.image_generation import get_image_gen_provider, image_gen_provider_names
from vtx_claw.providers.registry import create_dynamic_spec, find_by_name, list_providers
from vtx_claw.security.workspace_access import workspace_sandbox_status
from vtx_claw.webui.token_usage import token_usage_payload
...
def _provider_requires_api_key(spec: Any) -> bool:
    ...
    if spec.is_oauth:
        return False
    return not (spec.is_local or spec.is_direct)
...
def _oauth_provider_status(spec: Any) -> dict[str, Any]:
    ...
    if spec.name == "github_copilot":
        try:
            from vtx_claw.providers.github_copilot_provider import get_github_copilot_login_status
        except Exception:
            return {
                "configured": False,
                "account": None,
...
def login_oauth_provider(query: QueryParams) -> dict[str, Any]:
    ...
    if spec.name == "github_copilot":
        try:
            from vtx_claw.providers.github_copilot_provider import (
