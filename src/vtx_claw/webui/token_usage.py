...
def default_token_usage_state() -> dict[str, Any]:
    return {"schema_version": TOKEN_USAGE_SCHEMA_VERSION, "days": {}, "updated_at": None}
...
def record_response_token_usage(
    response: Any, *, source: str, timezone_name: str | None = None
) -> None:
    try:
        record_token_usage(
            getattr(response, "usage", None), source=source, timezone_name=timezone_name
        )
    except Exception:
...
def token_usage_payload(
    *, days: int = 371, timezone_name: str | None = None, now: datetime | None = None
) -> dict[str, Any]:
    state = read_token_usage_state()
