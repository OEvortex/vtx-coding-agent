...
_CURRENT_WORKSPACE_SCOPE: ContextVar[WorkspaceScope | None] = ContextVar(
    "vtx_claw_workspace_scope", default=None
)
...
def workspace_sandbox_status(
    *, restrict_to_workspace: bool, workspace: str | Path, environ: dict[str, str] | None = None
    ...
    return WorkspaceSandboxStatus(
        ...
        provider="none",
        provider_label=_provider_label("none"),
        summary="Workspace restriction uses vtx_claw application-level guards.",
    )
...
def _env_system_provider(environ: dict[str, str] | None = None) -> str | None:
    ...
    explicit_provider = env.get("VTX_CLAW_WORKSPACE_SANDBOX_PROVIDER")
    enforced = env.get("VTX_CLAW_WORKSPACE_SANDBOX_ENFORCED")
    compatibility = env.get("VTX_CLAW_SANDBOX_ENFORCED")
