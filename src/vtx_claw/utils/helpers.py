"""Utility functions for vtx_claw."""
...
_TOOL_RESULTS_DIR = ".vtx_claw/tool-results"
...
def build_status_content(
    *,
    ...
    lines = [
        f"\U0001f408 vtx_claw v{version}",
...
def sync_workspace_templates(workspace: Path, silent: bool = False) -> list[str]:
    ...
    try:
        tpl = pkg_files("vtx_claw") / "templates"
    except Exception:
        return []
    if not tpl.is_dir():
        return []
...
def load_bundled_template(template_name: str) -> str | None:
    """Read a bundled template file from the vtx_claw package."""
    from importlib.resources import files as pkg_files

    with suppress(Exception):
        tpl = pkg_files("vtx_claw") / "templates" / template_name
        if tpl.is_file():
