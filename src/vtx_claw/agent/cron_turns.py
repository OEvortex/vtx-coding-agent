...
from vtx_claw.cron.session_turns import cron_run_id, cron_trigger, defer_cron_until_session_idle
...
class CronTurnCoordinator:
    ...
    def should_defer(
        self, msg: InboundMessage, *, session_key: str, active_session_keys: Iterable[str]
    ) -> bool:
    ...
    def defer_if_active(
        ...
        if not self.should_defer(
            msg, session_key=session_key, active_session_keys=active_session_keys
        ):
            return False
        pending_msg = msg
