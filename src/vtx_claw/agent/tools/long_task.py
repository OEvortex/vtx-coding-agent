...
``vtx_claw.session.goal_state.goal_state_runtime_lines``) so compaction cannot hide them.
...
from vtx_claw.agent.tools.schema import StringSchema, tool_parameters_schema
from vtx_claw.bus.runtime_events import GoalStateChanged, RuntimeEventBus, RuntimeEventContext
...
class _GoalToolsMixin(ContextAware):
    ...
    def __init__(
        self, sessions: SessionManager, runtime_events: RuntimeEventBus | None = None
    ) -> None:
        ...
        self._request_ctx: ContextVar[RequestContext | None] = ContextVar(
            f"{self.__class__.__name__}_request_ctx", default=None
        )
...
class LongTaskTool(Tool, _GoalToolsMixin):
    ...
    def create(cls, ctx: Any) -> Tool:
        ...
        return cls(sessions=sess, runtime_events=getattr(ctx, "runtime_events", None))
