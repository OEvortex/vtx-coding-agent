...
from vtx_claw.agent.tools.context import current_request_session_key
from vtx_claw.agent.tools.exec_session import (
...
class ExecTool(Tool):
    ...
    def __init__(
        ...
        self.deny_patterns = (
            ...
            + [
                ...
                r":\(\)\s*\{.*\};\s*:",  # fork bomb
                # Block writes to vtx_claw internal state files (#2989).
        ...
        self.allow_local_preview_access = allow_local_preview_access or False
    ...
    async def _execute_session(
        self, prepared: _PreparedCommand, yield_time_ms: int | None, max_output_chars: int | None
    ) -> str:
        try:
    ...
    def _prepare_command(
        ...
        if self.sandbox:
            if _IS_WINDOWS:
                logger.warning(
                    "Sandbox '{}' is not supported on Windows; running unsandboxed", self.sandbox
                )
            else:
    ...
    def _wrap_path_export(self, command: str, env: dict[str, str]) -> str:
        segments = []
        if self.path_prepend:
            env["VTX_CLAW_PATH_PREPEND"] = self.path_prepend
            segments.append("$VTX_CLAW_PATH_PREPEND")
        segments.append("$PATH")
        if self.path_append:
            env["VTX_CLAW_PATH_APPEND"] = self.path_append
            segments.append("$VTX_CLAW_PATH_APPEND")
    ...
    def _guard_command(
        ...
        *,
        ...
        from vtx_claw.security.network import contains_internal_url

        if contains_internal_url(
            cmd,
            allow_loopback=current_scope_allows_loopback(enabled=self.allow_local_preview_access),
        ):
