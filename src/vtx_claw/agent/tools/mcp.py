"""MCP client: connects to MCP servers and wraps their tools as native vtx_claw tools."""
...
from vtx_claw.bus.events import (
...
async def _validate_mcp_request_url(request: httpx.Request) -> None:
    ...
    if not ok:
        raise httpx.RequestError(
            f"Blocked unsafe MCP URL {request.url} ({error})", request=request
        )
...
def _normalize_windows_stdio_command(
    command: str, args: list[str] | None, env: dict[str, str] | None
...
class _MCPWrapperBase(Tool):
    ...
    async def _refresh_session_after_termination(
        self, exc: BaseException, already_refreshed: bool, capability_kind: str
    ) -> bool:
...
class MCPToolWrapper(_MCPWrapperBase):
    """Wraps a single MCP server tool as a vtx_claw Tool."""
        ...
        while True:
            try:
                ...
                if _is_transient(exc):
                    ...
                    logger.exception(
                        "MCP tool '{}' failed after retry: {}", self._name, type(exc).__name__
                    )
    ...
    def _store_image_block(
        ...
        from vtx_claw.utils.artifacts import ArtifactError, store_generated_image_artifact
        ...
        try:
            ...
            logger.warning(
                "MCP tool '{}' returned an image that could not be stored: {}", self._name, exc
            )
            return None


class MCPResourceWrapper(_MCPWrapperBase):
    """Wraps an MCP resource URI as a read-only vtx_claw Tool."""
            ...
            try:
                ...
                if _is_transient(exc):
                    if not retried_transient:
                        ...
                        continue
                    logger.exception(
                        "MCP resource '{}' failed after retry: {}", self._name, type(exc).__name__
                    )
                    return f"(MCP resource read failed after retry: {type(exc).__name__})"
                logger.exception(
                    "MCP resource '{}' failed: {}: {}", self._name, type(exc).__name__, exc
                )
...
class MCPPromptWrapper(_MCPWrapperBase):
    """Wraps an MCP prompt as a read-only vtx_claw Tool."""
            ...
            try:
                ...
                if _is_transient(exc):
                    if not retried_transient:
                        ...
                        continue
                    logger.exception(
                        "MCP prompt '{}' failed after retry: {}", self._name, type(exc).__name__
                    )
                    return f"(MCP prompt call failed after retry: {type(exc).__name__})"
                logger.exception(
                    "MCP prompt '{}' failed: {}: {}", self._name, type(exc).__name__, exc
                )
    ...
    async def connect_single_server(name: str, cfg) -> tuple[str, AsyncExitStack | None]:
        ...
        try:
            ...
            if transport_type in {"sse", "streamableHttp"}:
                ...
                if not ok:
                    logger.warning(
                        "MCP server '{}': blocked unsafe URL {} ({})", name, cfg.url, error
                    )
                    await server_stack.aclose()
                    return name, None
            ...
            if transport_type == "stdio":
                ...
                params = StdioServerParameters(
                    command=command, args=args, env=env, cwd=cfg.cwd or None
                )
...
def runtime_lines(
    ...
    *,
    ...
    metadata = (
        message.metadata if isinstance(getattr(message, "metadata", None), Mapping) else None
    )
    ...
    for item in structured[:8]:
        ...
        if configured_server_names is not None and raw_name not in configured_server_names:
            lines.append(
                ...
                "tell the user to restart vtx_claw."
            )
            continue
        if connected_server_names is not None and raw_name not in connected_server_names:
            lines.append(
                ...
                "run the preset test, and restart vtx_claw only if hot reload is unavailable."
            )
            continue
        lines.append(
            "MCP Preset Attachment: "
...
async def reload_servers(state: Any, registry: ToolRegistry) -> dict[str, Any]:
    ...
    async with _reload_lock(state):
        try:
            ...
            return {
                "ok": False,
                "message": "Could not reload MCP config. Restart vtx_claw to pick up changes.",
        ...
        if failed:
            ...
            message = "MCP connections refreshed without restarting vtx_claw."
        else:
            message = "MCP config reloaded without restarting vtx_claw."
...
async def request_mcp_reload(bus: Any, *, timeout: float = 15.0) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "ok": False,
            "message": "MCP hot reload timed out. Restart vtx_claw to pick up changes.",
            "requires_restart": True,
        }
    return (
        result
...
async def handle_runtime_control(state: Any, msg: InboundMessage, registry: ToolRegistry) -> bool:
    ...
    try:
        ...
        result = {
            "ok": False,
            "message": "MCP hot reload failed. Restart vtx_claw to pick up changes.",
            "requires_restart": True,
            "error": str(exc),
        }
...
async def _refresh_terminated_server(
    state: Any, registry: ToolRegistry, server_name: str, tool_name: str, stale_tool: Tool
) -> Tool | None:
    async with _reload_lock(state):
        cfg = state._mcp_servers.get(server_name)
        if cfg is None:
            logger.warning(
                "MCP server '{}' session terminated but is no longer configured", server_name
            )
            return None
