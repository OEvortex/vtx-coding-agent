...
from vtx_claw.utils.helpers import build_image_content_blocks
...
_VOLCENGINE_TRAFFIC_TAG = "vtx_claw"
...
async def _stream_with_safe_redirects(
    ...
    for _ in range(MAX_REDIRECTS + 1):
        ...
        stream = client.stream("GET", current_url, headers=headers, follow_redirects=False)
...
class WebSearchTool(Tool):
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        if provider == "duckduckgo":
            ...
            return await self._search_bocha(query, n, freshness=kwargs.get("freshness", "noLimit"))
    ...
    async def _search_keenable(self, query: str, n: int) -> str:
        ...
        try:
            async with httpx.AsyncClient(proxy=self.proxy) as client:
                r = await client.post(
                    url, headers=headers, json={"query": query}, timeout=float(self.config.timeout)
                )
                r.raise_for_status()
            items = [
                {
    ...
    async def _search_jina(self, query: str, n: int) -> str:
        ...
        try:
            ...
            async with httpx.AsyncClient(proxy=self.proxy) as client:
                r = await client.get(
                    f"https://s.jina.ai/{encoded_query}", headers=headers, timeout=15.0
                )
                r.raise_for_status()
    ...
    async def _search_exa(self, query: str, n: int) -> str:
        ...
        try:
            ...
            body = {"query": query, "numResults": n, "contents": {"highlights": True}}
            async with httpx.AsyncClient(proxy=self.proxy) as client:
                r = await client.post(
    ...
    async def _search_duckduckgo(self, query: str, n: int) -> str:
        try:
            ...
            raw = await asyncio.wait_for(
                asyncio.to_thread(ddgs.text, query, max_results=n), timeout=self.config.timeout
            )
            if not raw:
                return f"No results for: {query}"
            items = [
    ...
    async def _search_bocha(self, query: str, n: int, freshness: str = "noLimit") -> str:
        ...
        try:
            ...
            if self.user_agent:
                headers["User-Agent"] = self.user_agent
            payload = {"query": query, "freshness": freshness, "summary": True, "count": n}
            async with httpx.AsyncClient(proxy=self.proxy) as client:
                r = await client.post(
...
class WebFetchTool(Tool):
        ...
        try:
            ...
            if "application/json" in ctype:
                ...
                try:
                    ...
                    logger.warning(
                        "Readability failed for {}, using raw HTML fallback: {}", url, e
                    )
    ...
    def _extract_readable_html(self, html_content: str, extract_mode: str) -> str:
        ...
        summary = doc.summary()
        content = (
            self._to_markdown(summary) if extract_mode == "markdown" else _strip_tags(summary)
        )
