...
def _bound_session_delivery_context(
    job: CronJob, *, turn_seed: str, source_label: str | None
...
async def run_bound_cron_job(
    job: CronJob, *, agent: BoundCronAgent, cron: CronRunRecorder
) -> str | None:
    ...
    prompt = render_template("agent/cron_reminder.md", strip=True, message=job.payload.message)
    ...
    try:
        ...
        cron.write_run_record(run_id, {**run_record_base, "status": "error", "error": error_text})
        raise
    finally:
    ...
    cron.write_run_record(run_id, {**run_record_base, "status": "ok", "response": response})
    return response
