...
from vtx_claw.bus import progress as bus_progress
...
from vtx_claw.utils.helpers import strip_think, truncate_text
...
async def maybe_generate_webui_title(
    *, sessions: SessionManager, session_key: str, provider: LLMProvider, model: str
) -> bool:
...
async def maybe_generate_webui_title_after_turn(
    *,
    ...
    return await maybe_generate_webui_title(
        sessions=sessions, session_key=session_key, provider=provider, model=model
    )
...
def build_bus_progress_callback(
    bus: MessageBus, msg: InboundMessage
...
async def publish_turn_run_status(
    bus: MessageBus, msg: InboundMessage, status: str, *, started_at: float | None = None
) -> None:
    ...
    await bus.publish_outbound(
        OutboundMessage(channel=msg.channel, chat_id=cid, content="", metadata=meta)
    )
...
class WebuiTurnCoordinator:
    ...
    def subscribe(self, runtime_events: RuntimeEventBus) -> Callable[[], None]:
        ...
        unsubscribe = [
            ...
            runtime_events.subscribe(self._handle_turn_completed_event, TurnCompleted),
    ...
    async def _handle_run_status_changed(self, event: TurnRunStatusChanged) -> None:
        ...
        await publish_turn_run_status(
            self.bus, self._ctx_msg(event.context), event.status, started_at=event.started_at
        )
    ...
    def capture_title_context(
        self, session_key: str, msg: InboundMessage, llm: LLMRuntime
    ) -> None:
    ...
    async def publish_run_status(
        self, msg: InboundMessage, status: str, *, started_at: float | None = None
    ) -> None:
    ...
    async def handle_turn_end(
        ...
        *,
        ...
        await self.bus.publish_outbound(
            OutboundMessage(
                channel=msg.channel, chat_id=msg.chat_id, content="", metadata=turn_metadata
            )
        )
    ...
    def _schedule_title_update(self, msg: InboundMessage, *, session_key: str) -> None:
        ...
        async def _generate_title_and_notify(title_llm: LLMRuntime = title_context) -> None:
