...
def sustained_goal_turn(
    metadata: Mapping[str, Any] | None, *, message_metadata: Mapping[str, Any] | None = None
) -> bool:
...
def runner_wall_llm_timeout_s(
    ...
    *,
    ...
    """Wall-clock cap for :class:`~vtx_claw.agent.runner.AgentRunner` when streaming an LLM.

    Returns ``0.0`` to disable ``asyncio.wait_for`` around the request when this is a
    sustained-goal turn; ``None`` means use ``VTX_CLAW_LLM_TIMEOUT_S``. Pass in-memory
    ``metadata`` when the caller already holds :attr:`~vtx_claw.session.manager.Session.metadata`
    for this turn.
    """
