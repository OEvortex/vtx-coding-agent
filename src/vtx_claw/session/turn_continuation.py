...
def should_finalize_on_max_iterations(
    *,
    ...
    return not (
        ...
        and _goal_continuation_available(session_metadata, message_metadata=message_metadata)
    )
...
async def maybe_continue_turn(ctx: Any) -> bool:
    ...
    metadata = _internal_continuation_metadata(
        ctx.msg.metadata, run_started_at=getattr(ctx, "visible_run_started_at", None)
    )
...
def _continuation_available(
    *,
    ...
    if stop_reason != "max_iterations" or not pending_queue_available:
        return False
    return _goal_continuation_available(session_metadata, message_metadata=message_metadata)
...
def _internal_continuation_metadata(
    message_metadata: Mapping[str, Any] | None, *, run_started_at: float | None = None
) -> dict[str, Any]:
...
def _strip_terminal_assistant(
    messages: list[dict[str, Any]], final_content: str | None
) -> list[dict[str, Any]]:
