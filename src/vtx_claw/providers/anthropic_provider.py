...
class AnthropicProvider(LLMProvider):
            ...
            if role == "tool":
                ...
                if raw and raw[-1]["role"] == "user":
                    ...
                    if isinstance(prev_c, list):
                        ...
                        raw[-1]["content"] = [{"type": "text", "text": prev_c or ""}, block]
                else:
    ...
    def _tool_result_block(
        msg: dict[str, Any], *, map_tool_result_id: Callable[[Any], str] | None = None
    ) -> dict[str, Any]:
        content = msg.get("content")
    ...
    def _assistant_blocks(
        msg: dict[str, Any], *, map_tool_id: Callable[[Any], str] | None = None
    ) -> list[dict[str, Any]]:
    ...
    def _convert_user_content(content: Any) -> Any:
        ...
        for item in content:
            ...
            if not item.get("type"):
                ...
                result.append(
                    {"type": "text", "text": AnthropicProvider._stringify_typeless_block(item)}
                )
                continue
            result.append(item)
    ...
    def _convert_tool_choice(
        tool_choice: str | dict[str, Any] | None, thinking_enabled: bool = False
    ) -> dict[str, Any] | None:
        if thinking_enabled:
            return {"type": "auto"}
    ...
    def _parse_response(response: Any) -> LLMResponse:
        ...
        for block in response.content:
            if block.type == "text":
                ...
                tool_calls.append(
                    ToolCallRequest(id=tool_id, name=block.name, arguments=block.input)
                )
        ...
        try:
            async with self._client.messages.stream(**kwargs) as stream:
                ...
                response = await asyncio.wait_for(
                    stream.get_final_message(), timeout=idle_timeout_s
                )
            return self._parse_response(response)
        except TimeoutError:
            return LLMResponse(
