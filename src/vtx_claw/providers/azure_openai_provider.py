...
helpers from :mod:`vtx_claw.providers.openai_responses`.
...
2. **Microsoft Entra ID (AAD)** — when ``api_key`` is empty the provider
   ...
   is an optional dependency installed via ``pip install vtx-claw[azure]``.
"""
    ...
    def __init__(self, scope: str = _AZURE_OPENAI_SCOPE) -> None:
        try:
            ...
            raise RuntimeError(
                ...
                "Install it with: pip install 'vtx-claw[azure]'"
            ) from exc
...
class AzureOpenAIProvider(LLMProvider):
    ...
    def __init__(self, api_key: str = "", api_base: str = "", default_model: str = "gpt-5.2-chat"):
