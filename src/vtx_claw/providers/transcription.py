...
async def _post_transcription_with_retry(
    ...
    *,
    ...
    def build_request() -> dict[str, Any]:
        files = {"file": (path.name, data, _audio_mime_type(path)), "model": (None, model)}
        if language:
        ...
        async with httpx.AsyncClient() as client:
            ...
            for attempt in range(_ASSEMBLYAI_POLL_ATTEMPTS):
                ...
                if status in {"error", "failed"}:
                    logger.error(
                        "AssemblyAI transcription failed: {}", payload.get("error") or payload
                    )
                    return ""
