...
from vtx_claw.providers.openai_responses import (
    consume_sdk_stream,
    convert_messages,
    convert_tools,
...
def _openai_compat_timeout_s() -> float:
    ...
    return _float_env("VTX_CLAW_OPENAI_COMPAT_TIMEOUT_S", _OPENAI_COMPAT_REQUEST_TIMEOUT_S)
...
def _uses_openrouter_attribution(spec: ProviderSpec | None, api_base: str | None) -> bool:
    """Apply VtxClaw attribution headers to OpenRouter requests by default."""
...
def _is_local_endpoint(spec: ProviderSpec | None, api_base: str | None) -> bool:
...
def _responses_circuit_key(
    model: str | None, default_model: str, reasoning_effort: str | None
) -> str:
...
def _merge_responses_extra_body(
    body: dict[str, Any], extra_body: dict[str, Any]
) -> dict[str, Any]:
...
class OpenAICompatProvider(LLMProvider):
    ...
    def _sanitize_messages(self, messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        def unique_tool_id(value: Any, used_ids: set[str], idx: int) -> str:
            base = map_id(value) if isinstance(value, str) and value else _short_tool_id()
    ...
    def _should_use_responses_api(self, model: str | None, reasoning_effort: str | None) -> bool:
    ...
    def _record_responses_failure(self, model: str | None, reasoning_effort: str | None) -> None:
        ...
        if count >= _RESPONSES_FAILURE_THRESHOLD:
            ...
            logger.warning(
                "Responses API circuit open for {} — falling back to Chat Completions", key
            )
    ...
    @staticmethod
    def _handle_error(
        e: Exception, *, spec: ProviderSpec | None = None, api_base: str | None = None
    ) -> LLMResponse:
        body = (
            getattr(e, "doc", None)
            ...
            if self._should_use_responses_api(model, reasoning_effort):
                try:
                    ...
                    (
                        ...
                        _timed_stream(), on_content_delta, on_tool_call_delta=on_tool_call_delta
                    )
                    self._record_responses_success(model, reasoning_effort)
                    return LLMResponse(
