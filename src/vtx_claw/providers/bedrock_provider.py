...
class BedrockProvider(LLMProvider):
    ...
    def _finish_reason(stop_reason: str | None) -> str:
        return {"end_turn": "stop", "tool_use": "tool_calls", "max_tokens": "length"}.get(
            ...
            if reasoning_buf:
                if reasoning_buf.get("text"):
                    ...
                    if isinstance(redacted, (bytes, bytearray)):
                        ...
                        redacted_block = {"type": "redacted_thinking", "redactedContent": redacted}
    ...
    def _stream_result(
        ...
        *,
        ...
        for buf in tool_buffers.values():
            ...
            tool_calls.append(
                ToolCallRequest(id=buf.get("id") or "", name=buf.get("name") or "", arguments=args)
            )
        return LLMResponse(
            content="".join(content_parts) or None,
            tool_calls=tool_calls,
        ...
        try:
            ...
            while True:
                event = await asyncio.wait_for(
                    asyncio.to_thread(_next_or_none, stream), timeout=idle_timeout_s
                )
                if event is None:
                    break
