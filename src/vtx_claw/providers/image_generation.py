...
_OPENROUTER_ATTRIBUTION_HEADERS = {
    "HTTP-Referer": "https://github.com/HKUDS/vtx_claw",
    "X-OpenRouter-Title": "vtx_claw",
    ...
    async def generate(
        ...
        *,
        ...
        return GeneratedImageResponse(
            images=images, content="\n".join(part for part in text_parts if part).strip(), raw=data
        )
...
class AIHubMixImageGenerationClient(ImageGenerationProvider):
    ...
    async def _generate_with_client(
        ...
        *,
        ...
        input_body: dict[str, Any] = {"prompt": prompt, "n": 1, "size": size}
    ...
    async def generate(
        ...
        *,
        ...
        try:
            ...
            logger.error(
                "Ollama image generation failed (HTTP {}): {}", response.status_code, detail
            )
...
class GeminiImageGenerationClient(ImageGenerationProvider):
    ...
    async def _generate_imagen(
        self, *, prompt: str, model: str, aspect_ratio: str | None
    ) -> GeneratedImageResponse:
        parameters: dict[str, Any] = {"sampleCount": 1}
        if aspect_ratio in _GEMINI_IMAGEN_ASPECT_RATIOS:
            parameters["aspectRatio"] = aspect_ratio
        body: dict[str, Any] = {"instances": [{"prompt": prompt}], "parameters": parameters}
        body.update(self.extra_body)
    ...
    async def _generate_gemini_flash(
        self, *, prompt: str, model: str, reference_images: list[str]
    ) -> GeneratedImageResponse:
        ...
        return GeneratedImageResponse(
            images=images, content="\n".join(t for t in text_parts if t).strip(), raw=data
        )
...
class MiniMaxImageGenerationClient(ImageGenerationProvider):
    ...
    async def generate(
        ...
        *,
        ...
        body: dict[str, Any] = {"model": model, "prompt": prompt, "response_format": "base64"}
...
_OPENAI_GPT_IMAGE_SUPPORTED_SIZES = {"1024x1024", "1536x1024", "1024x1536", "auto"}
...
class OpenAIImageGenerationClient(ImageGenerationProvider):
    ...
    async def _post_image_edit(
        self, *, headers: dict[str, str], body: dict[str, Any], reference_images: list[str]
    ) -> httpx.Response:
    ...
    async def generate(
        ...
        *,
        ...
        edit_headers = {"Authorization": f"Bearer {self.api_key}", **self.extra_headers}

        body: dict[str, Any] = {"model": clean_model, "prompt": prompt}
        ...
        if refs:
            ...
            response = await self._post_image_edit(
                headers=edit_headers, body=edit_body, reference_images=refs
            )
        else:
            logger.info(
            ...
            response = await self._http_post(
                f"{self.api_base}/images/generations", headers=generation_headers, body=body
            )
...
class CustomImageGenerationClient(ImageGenerationProvider):
    ...
    async def generate(
        ...
        *,
        ...
        response = await self._http_post(
            f"{self.api_base}/images/generations", headers=headers, body=body
        )
...
class CodexImageGenerationClient(ImageGenerationProvider):
    ...
    async def generate(
        ...
        *,
        ...
        logger.info("Using Codex OAuth token for image generation (account: {})", token.account_id)
        ...
        response = await self._http_post(
            f"{self.api_base}/codex/responses", headers=headers, body=body
        )
...
def _openai_size(model: str, aspect_ratio: str | None, image_size: str | None) -> str:
    ...
    if explicit_size and _openai_explicit_size_supported(
        explicit_size, supported_sizes=supported_sizes
    ):
...
def _openai_multipart_form_body(body: dict[str, Any]) -> dict[str, str]:
    ...
    for key, value in body.items():
        ...
        if isinstance(value, bool):
            ...
            logger.warning(
                "OpenAI image edit parameter '{}' is not a scalar form field; ignoring it", key
            )
    return form
...
def _openai_explicit_size_supported(size: str, *, supported_sizes: set[str] | None) -> bool:
...
async def _parse_codex_sse_images(response: httpx.Response) -> tuple[list[str], str]:
    ...
    async def generate(
        ...
        *,
        ...
        if refs and "1x" in model:
            body["style_reference"] = {"source_url": image_path_to_data_url(refs[0])}
...
def _stepfun_size(aspect_ratio: str | None, image_size: str | None) -> str:
...
class ZhipuImageGenerationClient(ImageGenerationProvider):
    ...
    async def generate(
        ...
        *,
        ...
        client = self._client or httpx.AsyncClient(timeout=self.timeout)
        try:
            return await self._generate_with_client(client, headers=headers, body=body, url=url)
        finally:
            if self._client is None:
                await client.aclose()

    async def _generate_with_client(
        self, client: httpx.AsyncClient, *, headers: dict[str, str], body: dict[str, Any], url: str
    ) -> GeneratedImageResponse:
        try:
...
def _zhipu_size(aspect_ratio: str | None, image_size: str | None) -> str:
