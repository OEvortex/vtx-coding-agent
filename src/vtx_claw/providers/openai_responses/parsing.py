...
from vtx_claw.providers.base import LLMResponse, ToolCallRequest, parse_tool_arguments
...
def _parse_tool_call_arguments(args_raw: Any, name: str | None) -> Any:
    ...
    if parsed == args_raw and isinstance(args_raw, str) and args_raw.strip():
        logger.warning("Failed to parse tool call arguments for '{}': {}", name, args_raw[:200])
    return parsed
...
async def consume_sse(
    ...
    content, tool_calls, finish_reason, _, _ = await consume_sse_with_reasoning(
        response, on_content_delta=on_content_delta, on_tool_call_delta=on_tool_call_delta
    )
    ...
    async for event in iter_sse(response):
        ...
        if event_type == "response.output_item.added":
            ...
            if item.get("type") == "function_call":
                ...
                args = _parse_tool_call_arguments(args_raw, buf.get("name") or item.get("name"))
                tool_calls.append(
                    ToolCallRequest(
...
def parse_response_output(response: Any) -> LLMResponse:
    ...
    for item in output:
        ...
        if item_type == "message":
            ...
            tool_calls.append(
                ToolCallRequest(
                    id=f"{call_id}|{item_id}", name=item.get("name") or "", arguments=args
                )
            )
        ...
        if event_type == "response.output_item.added":
            ...
            if item and getattr(item, "type", None) == "function_call":
                ...
                args_raw = _tool_arguments_source(
                    buf.get("arguments"), getattr(item, "arguments", None)
                )
                if on_tool_call_delta and str(call_id) not in tool_call_args_emitted:
                    tool_call_args_emitted.add(str(call_id))
                    await on_tool_call_delta(
                ...
                args = _parse_tool_call_arguments(
                    args_raw, buf.get("name") or getattr(item, "name", None)
                )
                tool_calls.append(
                    ToolCallRequest(
