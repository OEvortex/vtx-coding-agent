...
from vtx_claw.gateway import GatewayStartOptions, build_gateway_command
...
class GatewayServiceInstaller:
    ...
    def _install_systemd(
        self, options: GatewayServiceOptions, *, dry_run: bool
    ) -> GatewayServiceResult:
        ...
        content = _systemd_unit_content(
            description=f"VtxClaw Gateway ({options.name})",
            command=command,
    ...
    def _uninstall_systemd(self, *, name: str, dry_run: bool) -> GatewayServiceResult:
    ...
    def _uninstall_launchd(self, *, name: str, dry_run: bool) -> GatewayServiceResult:
...
def _launchd_label(name: str) -> str:
    if name.startswith("ai.vtx_claw."):
        return name
    suffix = _safe_service_name(name).removeprefix("vtx_claw-").replace("-", ".")
    return f"ai.vtx_claw.{suffix}"
...
def _safe_service_name(name: str) -> str:
    ...
    value = value.strip(".-")
    return value or "vtx_claw-gateway"
...
def _systemd_unit_content(*, description: str, command: list[str], working_directory: str) -> str:
