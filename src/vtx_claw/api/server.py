"""OpenAI-compatible HTTP API server for a fixed vtx_claw session.
...
from vtx_claw.utils.media_decode import MAX_FILE_SIZE
from vtx_claw.utils.media_decode import FileSizeExceeded as _FileSizeExceeded
from vtx_claw.utils.media_decode import save_base64_data_url as _save_base64_data_url
...
async def handle_chat_completions(request: web.Request) -> web.Response:
    ...
    model_name: str = request.app.get("model_name", "vtx_claw")
...
async def handle_models(request: web.Request) -> web.Response:
    """GET /v1/models"""
    model_name = request.app.get("model_name", "vtx_claw")
    return web.json_response(
        {
            "object": "list",
            "data": [{"id": model_name, "object": "model", "created": 0, "owned_by": "vtx_claw"}],
        }
    )
...
def create_app(
    agent_loop, model_name: str = "vtx_claw", request_timeout: float = 120.0
) -> web.Application:
