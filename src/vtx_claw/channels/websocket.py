"""WebSocket server channel: vtx_claw acts as a WebSocket server and serves connected clients."""
...
class WebSocketConfig(Base):
    ...
    - ``token_issue_path``: If non-empty, **GET** (HTTP/1.1) to this path returns JSON
      ...
      vtx_claw and shares the asyncio loop, use a thread or async HTTP client for GET—do not call
      blocking ``urllib`` or synchronous ``httpx`` from inside a coroutine.
    - ``token_issue_secret``: If non-empty, token requests must send ``Authorization: Bearer <secret>`` or
      ``X-VtxClaw-Auth: <secret>``.
...
def _is_websocket_upgrade(request: WsRequest) -> bool:
    ...
    if not upgrade or "websocket" not in upgrade.lower():
        return False
    return not (not connection or "upgrade" not in connection.lower())
...
class WebSocketChannel(BaseChannel):
    ...
    def _save_envelope_media(self, media: list[Any]) -> tuple[list[str], str | None]:
        ...
        for item in media:
            mime = (
                _extract_data_url_mime(item.get("data_url", ""))
        ...
        if t == "new_chat":
            ...
            scope = await self._workspace_scope_or_error(
                ...
                lambda: self._workspaces.scope_for_new_chat(
                    envelope, controls_available=self._workspace_controls_available(connection)
                ),
            )
            if scope is None:
                return
        ...
        if t == "message":
            ...
            if raw_media is not None:
                if not isinstance(raw_media, list):
                    await self._send_event(
                        connection, "error", detail="image_rejected", reason="malformed"
                    )
                    return
                media_paths, reason = self._save_envelope_media(raw_media)
                if reason is not None:
                    await self._send_event(
                        connection, "error", detail="image_rejected", reason=reason
                    )
                    return
    ...
    async def send(self, msg: OutboundMessage) -> None:
        if msg.metadata.get("_runtime_model_updated"):
            await self.send_runtime_model_updated(
                model_name=msg.metadata.get("model"), model_preset=msg.metadata.get("model_preset")
            )
            return
        ...
        if msg.metadata.get("_turn_end"):
            ...
            await self.send_turn_end(
                msg.chat_id, latency_ms=lat_i, goal_state=gs_blob, metadata=msg.metadata
            )
        ...
        if msg.metadata.get("_session_updated"):
            if conns:
                ...
                await self.send_session_updated(
                    msg.chat_id, scope=scope if isinstance(scope, str) else None
                )
            return
        if msg.metadata.get("_file_edit_events"):
            edits = msg.metadata.get("_file_edit_events")
            await self.send_file_edit_events(
                msg.chat_id, edits if isinstance(edits, list) else [], msg.metadata
            )
            return
        text = msg.content
        wire_text = self._media.rewrite_local_markdown_images(text)
        payload: dict[str, Any] = {"event": "message", "chat_id": msg.chat_id, "text": wire_text}
        if msg.media:
    ...
    async def send_reasoning_delta(
        ...
        self._transcripts.prepare_and_append(chat_id, body, metadata=meta, phase="reasoning")
    ...
    async def send_file_edit_events(
        ...
        conns = list(self._subs.get(chat_id, ()))
        payload: dict[str, Any] = {"event": "file_edit", "chat_id": chat_id, "edits": edits}
        self._transcripts.prepare_and_append(chat_id, payload, metadata=metadata, phase="activity")
        raw = json.dumps(payload, ensure_ascii=False)
        if not conns:
            return
        for connection in conns:
    ...
    async def send_delta(
        ...
        self._transcripts.prepare_and_append(chat_id, body, metadata=meta, phase="answer")
        raw = json.dumps(body, ensure_ascii=False)
        if not conns:
            return
        for connection in conns:
    ...
    async def send_turn_end(
        ...
        *,
        ...
        self._transcripts.prepare_and_append(chat_id, body, metadata=metadata, phase="complete")
        raw = json.dumps(body, ensure_ascii=False)
        if not conns:
            return
        for connection in conns:
    ...
    async def send_goal_status(
        self, chat_id: str, status: str, *, started_at: float | None = None
    ) -> None:
    ...
    async def send_runtime_model_updated(
        self, *, model_name: Any, model_preset: Any = None
    ) -> None:
