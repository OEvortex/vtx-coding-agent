...
try:
    ...
    raise ImportError(
        "Matrix dependencies not installed. Run: pip install vtx-claw[matrix]"
    ) from e
...
from vtx_claw.config.paths import get_data_dir, get_media_dir
    ...
    def _register_to_device_callbacks(self) -> None:
        if self.config.e2ee_enabled and self.config.sas_verification:
            self.client.add_to_device_callback(
                self._on_key_verification_event, (KeyVerificationEvent,)
            )
