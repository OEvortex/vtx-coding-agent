...
from vtx_claw.pairing import is_approved
from vtx_claw.utils.helpers import safe_filename, split_message
...
class SlackChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        try:
            await asyncio.wait_for(
                self._socket_client.connect(), timeout=SLACK_SOCKET_CONNECT_TIMEOUT_S
            )
        except TimeoutError:
            self.logger.error(
            ...
            for media_path in msg.media or []:
                try:
                    await self._web_client.files_upload_v2(
                        channel=target_chat_id, file=media_path, thread_ts=thread_ts_param
                    )
                except Exception:
    ...
    async def _on_socket_request(self, client: SocketModeClient, req: SocketModeRequest) -> None:
        ...
        if not self._is_allowed(sender_id, chat_id, channel_type):
            if channel_type == "im" and self.config.dm.enabled:
                await self._handle_message(
                    sender_id=sender_id, chat_id=chat_id, content="", is_dm=True
                )
            return
        ...
        try:
            if self._web_client and event.get("ts"):
                await self._web_client.reactions_add(
                    channel=chat_id, name=self.config.react_emoji, timestamp=event.get("ts")
                )
        except Exception as e:
        ...
        try:
            await self._handle_message(
                ...
                media=media_paths,
                metadata={
                    "slack": {"event": event, "thread_ts": thread_ts, "channel_type": channel_type}
                },
    ...
    def _download_failure_marker(marker_type: str, name: str, reason: str) -> str:
        return (
            f"[{marker_type}: {name}: {reason}; not available to vtx_claw. "
        ...
        *,
        ...
        try:
            response = await self._web_client.conversations_replies(
                channel=chat_id, ts=thread_ts, limit=max(1, self.config.thread_context_limit)
            )
        except Exception as e:
            self.logger.warning("thread context unavailable for {}: {}", key, e)
            return text

        lines = self._format_thread_context(response.get("messages", []), current_ts=current_ts)
        if not lines:
            return text
        return (
    ...
    async def _update_react_emoji(self, chat_id: str, ts: str | None) -> None:
        ...
        try:
            await self._web_client.reactions_remove(
                channel=chat_id, name=self.config.react_emoji, timestamp=ts
            )
        except Exception as e:
        ...
        if self.config.done_emoji:
            try:
                await self._web_client.reactions_add(
                    channel=chat_id, name=self.config.done_emoji, timestamp=ts
                )
            except Exception as e:
