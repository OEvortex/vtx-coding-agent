...
class FeishuChannel(BaseChannel):
    ...
    async def login(self, force: bool = False) -> bool:
        ...
        _LOGIN_CONSOLE.print(
            "Authorize with the mobile app. vtx_claw will save the new bot credentials.\n"
        )
        ...
        if not result:
            _LOGIN_CONSOLE.print(
                ...
                "Run 'vtx_claw channels login feishu --force' to retry."
            )
            return False
    ...
    async def start(self) -> None:
        ...
        if not self.config.app_id or not self.config.app_secret:
            self.logger.error(
                ...
                "Run 'vtx_claw channels login feishu' to set up via QR code."
            )
            return
            ...
            if not self.is_allowed(sender_id):
                if chat_type == "p2p":
                    ...
                    await self._handle_message(
                        sender_id=sender_id, chat_id=sender_id, content="", is_dm=True
                    )
                return
