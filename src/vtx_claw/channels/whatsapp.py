...
def _load_neonize() -> _NeonizeAPI:
    ...
    try:
        ...
        raise RuntimeError(
            'WhatsApp dependencies not installed. Run: pip install "vtx-claw[whatsapp]"'
        ) from exc
...
class WhatsAppChannel(BaseChannel):
    ...
    def __init__(self, config: Any, bus: MessageBus):
        ...
        if legacy_bridge_fields:
            self.logger.warning(
                ...
                "Run 'vtx_claw channels login whatsapp' to create a neonize session.",
    ...
    def _fail_login_on_connect_task_done(
        connect_task: asyncio.Task[Any] | None, login_result: asyncio.Future[None]
    ) -> None:
    ...
    async def _send_media(self, client: Any, to: Any, media_path: str) -> None:
        ...
        if mimetype.startswith("image/"):
            ...
            await client.send_document(to, path, filename=Path(path).name, mimetype=mimetype)
