...
class NapcatChannel(BaseChannel):
    ...
    async def _on_message(self, ev: dict[str, Any]) -> None:
        ...
        if message_type == "group":
            ...
            if not self._should_reply_in_group(
                group_id=group_id, mentioned_self=mentioned_self, replying_to_bot=replying_to_bot
            ):
                return

            chat_id = f"group:{group_id}"
            content = self._format_group_content(text=text, nickname=nickname, user_id=user_id)
        else:
        ...
        for seg in segments:
            ...
            if stype == "text":
                ...
                if isinstance(url, str) and url.startswith(("http://", "https://")):
                    images.append(
                        {"url": url, "file": data.get("file"), "file_size": data.get("file_size")}
                    )
                else:
    ...
    @staticmethod
    def _format_group_content(*, text: str, nickname: str, user_id: Any) -> str:
    ...
    async def _on_notice(self, ev: dict[str, Any]) -> None:
        ...
        await self._handle_message(
            ...
            metadata={"is_group": True, "event": "group_increase"},
        )
    ...
    async def _build_image_segment(self, ref: str) -> dict[str, Any] | None:
        ...
        # different host/container than vtx_claw.
    ...
    async def _download_image(self, info: dict[str, Any]) -> str | None:
        ...
        name = safe_filename(filename_hint) if filename_hint else f"{int(time.time() * 1000)}.jpg"
