...
from vtx_claw.config.paths import get_media_dir, get_runtime_subdir
...
class WeixinConfig(Base):
    ...
    state_dir: str = ""  # Default: ~/.vtx_claw/weixin/
...
class WeixinChannel(BaseChannel):
                ...
                if status == "confirmed":
                    ...
                    if token:
                        ...
                        if base_url:
                            self.config.base_url = base_url
                        self._save_state()
                        self.logger.info("login successful! bot_id={} user_id={}", bot_id, user_id)
                        return True
                    else:
    ...
    async def login(self, force: bool = False) -> bool:
        ...
        self._client = httpx.AsyncClient(
            timeout=httpx.Timeout(60, connect=30), follow_redirects=True
        )
    ...
    async def start(self) -> None:
        ...
        if self.config.token:
            ...
            self.logger.error(
                "login failed. Run 'vtx_claw channels login weixin' to authenticate."
            )
            self._running = False
            return
    ...
    async def _download_media_item(
        self, typed_item: dict, media_type: str, filename: str | None = None
    ) -> str | None:
    ...
    async def _refresh_context_token_if_stale(self, chat_id: str, context_token: str) -> str:
        ...
        self.logger.debug(
            "WeChat context_token for {} is {:.0f}s old; refreshing via getconfig", chat_id, age
        )
        ...
        if new_token and new_token != context_token:
            self.logger.info(
                "WeChat context_token refreshed for {} (age {:.0f}s -> fresh)", chat_id, age
            )
    ...
    async def _flush_tool_hints(self, chat_id: str) -> None:
        ...
        self.logger.info("Flushing {} buffered tool hint(s) for {}", len(hints), chat_id)

        ctx_token = self._context_tokens.get(chat_id, "")
        ctx_token = await self._refresh_context_token_if_stale(chat_id, ctx_token)
        if not ctx_token:
            self.logger.warning(
                "Dropped {} buffered tool hint(s) for {}: no context_token", len(hints), chat_id
            )
            return
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        if is_progress and not content and not (msg.media or []):
            self.logger.debug(
                "Skipped empty progress message for {} (no visible content)", msg.chat_id
            )
            return
    ...
    async def _send_text(self, to_user_id: str, text: str, context_token: str) -> None:
        ...
        client_id = f"vtx_claw-{uuid.uuid4().hex[:12]}"
