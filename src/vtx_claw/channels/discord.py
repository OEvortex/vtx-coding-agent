    ...
    class DiscordBotClient(discord.Client):
        ...
        def _register_app_commands(self) -> None:
            ...
            async def help_command(interaction: discord.Interaction) -> None:
                ...
                if not self._channel.is_allowed(sender_id):
                    await self._reply_ephemeral(
                        interaction, "You are not allowed to use this bot."
                    )
                    return
                channel = await self._resolve_interaction_channel(interaction)
                if not await self._interaction_channel_allowed(interaction, channel):
        ...
        async def send_outbound(self, msg: OutboundMessage) -> None:
            """Send a vtx_claw outbound message using Discord transport rules."""
...
class DiscordChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if not DISCORD_AVAILABLE:
            self.logger.error("discord.py not installed. Run: pip install vtx-claw[discord]")
            return
        ...
        try:
            ...
            if has_user and has_pass:
                ...
                proxy_auth = aiohttp.BasicAuth(
                    login=self.config.proxy_username, password=self.config.proxy_password
                )
            ...
            self._client = DiscordBotClient(
                self, intents=intents, proxy=self.config.proxy, proxy_auth=proxy_auth
            )
        except Exception:
