...
class MSTeamsChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if not MSTEAMS_AVAILABLE:
            self.logger.error("PyJWT not installed. Run: pip install vtx-claw[msteams]")
            return
        ...
        self._server_thread = threading.Thread(
            target=self._server.serve_forever, name="vtx_claw-msteams", daemon=True
        )
    ...
    async def _validate_inbound_auth(self, auth_header: str, activity: dict[str, Any]) -> None:
        ...
        if not MSTEAMS_AVAILABLE:
            raise RuntimeError("PyJWT not installed. Run: pip install vtx-claw[msteams]")
