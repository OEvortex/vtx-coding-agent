...
from vtx_claw.security.network import validate_resolved_url, validate_url_target
...
class VtxClawDingTalkHandler(CallbackHandler):
    """
    Standard DingTalk Stream SDK Callback Handler.
    Parses incoming messages and forwards them to the VtxClaw channel.
    """
    ...
    async def process(self, message: CallbackMessage):
        ...
        try:
            ...
            # Forward to VtxClaw via _on_message (non-blocking).
...
class DingTalkChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            handler = VtxClawDingTalkHandler(self)
    ...
    async def _send_markdown_text(self, token: str, chat_id: str, content: str) -> bool:
        return await self._send_batch_message(
            token, chat_id, "sampleMarkdown", {"text": content, "title": "VtxClaw Reply"}
        )
    ...
    async def _on_message(
        ...
        """Handle incoming message (called by VtxClawDingTalkHandler).
