---
name: update-setup
description: One-time setup wizard for the vtx-claw upgrade skill. Triggers: setup update, configure update, 切设置更新, 初始化更新.
---
...
Use `exec` to run `vtx-claw --version`. Tell the user the current version.

Then collect install clues with `exec`. These commands are best-effort; if one fails,
keep going and show the useful output:

```
command -v vtx-claw || true
python -m pip show vtx-claw || true
pipx list | sed -n '/vtx-claw/,+3p' || true
uv tool list | sed -n '/vtx-claw/,+3p' || true
```
...
answer, stop and ask the user to rerun this setup when they know how vtx-claw was
installed.
...
`question: "Where is your vtx-claw source checkout? Enter an absolute path or a path relative to this workspace:"`.
...
| Method | Command |
|--------|---------|
| uv | `uv tool install "vtx-claw[EXTRAS]" --force` |
| pipx | `pipx install --force "vtx-claw[EXTRAS]"` |
| pip | `python -m pip install --upgrade "vtx-claw[EXTRAS]"` |
...
---
name: update
description: "Upgrade vtx-claw to the latest version. Triggers: upgrade vtx-claw, update vtx-claw, 升级vtx-claw, 更新vtx-claw."
---
...
4. Use `exec` to verify: `vtx-claw --version`
5. Tell the user the new version. Say: "Run `/restart` to restart vtx-claw and apply the update. If `/restart` is unavailable in this channel, restart the vtx-claw process manually."
```

## Step 5: Confirm

Tell the user: "Upgrade skill created. Say 'upgrade vtx-claw' when you want to update."
