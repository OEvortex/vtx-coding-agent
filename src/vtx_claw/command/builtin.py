...
from vtx_claw.utils.restart import set_restart_notice_to_env
...
BUILTIN_COMMAND_SPECS: tuple[BuiltinCommandSpec, ...] = (
    ...
    BuiltinCommandSpec(
        "/restart", "Restart vtx_claw", "Restart the bot process in place.", "rotate-cw"
    ),
    BuiltinCommandSpec(
...
async def cmd_restart(ctx: CommandContext) -> OutboundMessage:
    ...
    async def _do_restart():
        ...
        os.execv(sys.executable, [sys.executable, "-m", "vtx_claw", *sys.argv[1:]])
...
async def cmd_status(ctx: CommandContext) -> OutboundMessage:
    ...
    with suppress(Exception):
        from vtx_claw.utils.searchusage import fetch_search_usage
...
async def cmd_pairing(ctx: CommandContext) -> OutboundMessage:
    """List, approve, deny or revoke pairing requests."""
    from vtx_claw.pairing import PAIRING_COMMAND_META_KEY, handle_pairing_command
