...
# Remove default handler and re-add with unified vtx_claw format
logger.remove()
...
from vtx_claw import __logo__, __version__  # noqa: E402
from vtx_claw.agent.loop import AgentLoop  # noqa: E402
from vtx_claw.cli.gateway import create_gateway_app  # noqa: E402
from vtx_claw.cli.stream import StreamRenderer, ThinkingSpinner  # noqa: E402
from vtx_claw.config.paths import get_workspace_path, is_default_workspace  # noqa: E402
from vtx_claw.config.schema import Config  # noqa: E402
from vtx_claw.utils.evaluator import evaluate_response  # noqa: E402
from vtx_claw.utils.helpers import sync_workspace_templates  # noqa: E402
from vtx_claw.utils.restart import (  # noqa: E402
...
app = typer.Typer(
    ...
    help=f"{__logo__} vtx-claw - Personal AI Assistant",
...
def _init_prompt_session() -> None:
    ...
    from vtx_claw.config.paths import get_cli_history_path
...
def _print_agent_response(
    ...
    if show_header:
        console.print()
        console.print(f"[cyan]{__logo__} vtx_claw[/cyan]")
    console.print(body)
    console.print()
...
async def _print_interactive_response(
    ...
    def _write() -> None:
        ...
        ansi = _render_interactive_ansi(
            lambda c: (
                c.print(),
                c.print(f"[cyan]{__logo__} vtx_claw[/cyan]"),
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} vtx_claw v{__version__}")
        raise typer.Exit()
...
def main(
    ...
    """vtx-claw - Personal AI Assistant."""
    pass
...
def onboard(
    ...
    if wizard:
        ...
        try:
            ...
            console.print(
                "[yellow]Please run 'vtx_claw onboard' again to complete setup.[/yellow]"
            )
            raise typer.Exit(1)
    ...
    agent_cmd = 'vtx_claw agent -m "Hello!"'
    gateway_cmd = "vtx-claw gateway"
    if config:
        agent_cmd += f" --config {config_path}"
        gateway_cmd += f" --config {config_path}"

    console.print(f"\n{__logo__} vtx_claw is ready!")
    console.print("\nNext steps:")
    if wizard:
    ...
    console.print(
        "\n[dim]Want Telegram/WhatsApp? See: https://github.com/HKUDS/vtx_claw#-chat-apps[/dim]"
    )
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from vtx_claw.config.loader import load_config, resolve_config_env_vars, set_config_path
    ...
    # the vtx_claw config has no API keys configured.
...
def _warn_deprecated_config_keys(config_path: Path | None) -> None:
    ...
    from vtx_claw.config.loader import get_config_path
...
def _migrate_cron_store(config: "Config") -> None:
    ...
    from vtx_claw.config.paths import get_cron_dir
...
def serve(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show vtx-claw runtime logs"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
):
    ...
    try:
        ...
        console.print("[red]aiohttp is required. Install with: pip install 'vtx-claw[api]'[/red]")
        raise typer.Exit(1)

    from loguru import logger

    from vtx_claw.api.server import create_app
    ...
    if verbose:
        logger.enable("vtx_claw")
    else:
        logger.disable("vtx_claw")
...
def _run_gateway(
    ...
    *,
    ...
    from vtx_claw.cron.bound_runner import run_bound_cron_job
    from vtx_claw.cron.service import CronJobSkippedError, CronService
    from vtx_claw.cron.session_turns import is_bound_cron_job
    ...
    console.print(f"{__logo__} Starting vtx-claw gateway version {__version__} on port {port}...")
    ...
    async def run():
        ...
        try:
            await cron.start()
            tasks = [
                asyncio.create_task(agent.run(), name="vtx_claw-agent-loop"),
                asyncio.create_task(channels.start_all(), name="vtx_claw-channels"),
            ]
            if health_server_enabled:
                tasks.append(
                    asyncio.create_task(
                        _health_server(config.gateway.host, port), name="vtx_claw-health-server"
                    )
                )
            if open_browser_url:
                tasks.append(
                    asyncio.create_task(_open_browser_when_ready(), name="vtx_claw-open-browser")
                )
            runtime_tasks = asyncio.gather(*tasks)
            shutdown_task = asyncio.create_task(
                shutdown_event.wait(), name="vtx_claw-gateway-shutdown"
            )
...
def agent(
    ...
    logs: bool = typer.Option(
        False, "--logs/--no-logs", help="Show vtx-claw runtime logs during chat"
    ),
):
...
def channels_status(
    ...
    """Show channel status."""
    from vtx_claw.channels.registry import discover_all
    from vtx_claw.config.loader import load_config, set_config_path
...
def status():
    ...
    console.print(f"{__logo__} vtx_claw Status\n")
    ...
    if config_path.exists():
        ...
        # Check API keys from registry (vtx catalog + vtx_claw-specific OAuth)
...
def _resolve_oauth_provider(provider: str):
    ...
    from vtx_claw.providers.registry import ProviderSpec, find_by_name, list_providers

    key = provider.replace("-", "_")
    spec = find_by_name(provider)
    if not spec or not spec.is_oauth:
        # VtxClaw-specific OAuth providers not in vtx's catalog
...
def _login_github_copilot() -> None:
    try:
        from vtx_claw.providers.github_copilot_provider import login_github_copilot
