"""Interactive onboarding questionnaire for vtx_claw."""
...
from vtx_claw.cli.models import format_token_count, get_model_context_limit, get_model_suggestions
...
def _show_main_menu_header() -> None:
    ...
    from vtx_claw import __logo__, __version__

    console.print()
    body = Table.grid(expand=True)
    body.add_column(ratio=1)
    body.add_row(f"{__logo__} [bold {_UI_TEXT}]vtx_claw[/] [{_UI_MUTED}]v{__version__}[/]")
...
def _handle_fallback_models_field(
    ...
    from vtx_claw.config.schema import InlineFallbackConfig
...
def _handle_search_provider_field(
    ...
    from vtx_claw.agent.tools.web import SEARCH_PROVIDER_OPTIONS
...
def _try_auto_fill_context_window(model: BaseModel, new_model_name: str) -> None:
    ...
    Note:
        This function imports AgentDefaults from vtx_claw.config.schema to get
...
def _get_channel_info() -> dict[str, tuple[str, type[BaseModel]]]:
    ...
    for name, channel_cls in discover_all().items():
        try:
            mod = importlib.import_module(f"vtx_claw.channels.{name}")
