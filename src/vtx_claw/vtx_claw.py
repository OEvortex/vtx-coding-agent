"""High-level programmatic interface to vtx_claw."""
...
from vtx_claw.agent.hook import AgentHook, SDKCaptureHook
...
from vtx_claw.sdk.clients import MemoryClient, RuntimeClient, SessionClient
from vtx_claw.sdk.runtime import (
    SDKRuntimeController,
    build_process_direct_kwargs,
    ensure_single_model_selector,
)
from vtx_claw.sdk.streaming import RunStream, SDKStreamEmitter, SDKStreamingHook
...
class VtxClaw:
    """Programmatic facade for running the vtx_claw agent.

    Usage::

        bot = VtxClaw.from_config()
    ...
    def from_config(
        ...
        *,
        ...
        """Create a VtxClaw instance from a config file.

        Args:
            config_path: Path to ``config.json``.  Defaults to
                ``~/.vtx_claw/config.json``.
    ...
    async def __aenter__(self) -> VtxClaw:
        return self
