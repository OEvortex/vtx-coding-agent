...
from ai.agent.async_utils import OperationCancelledError, await_or_cancel
from ai.agent.context_governance import prepare_for_model
from ai.agent.extensions import TOOL_CALL, TOOL_RESULT, EventBus
from ai.agent.hooks.agent_hook import AgentHook, AgentHookContext, AgentRunHookContext
from ai.agent.tools import BaseTool, get_tool, get_tool_definitions
from ai.agent.tools.ask_user import AskUserParams
...
class _TurnRunner:
    ...
    def _finalize_current_state(self, include_empty: bool = True) -> list[StreamEvent]:
        ...
        if self._current_state == StreamState.THINK:
            ...
            from ai.tool_parser import extract_text_and_tool_calls, has_text_tool_calls
