...
from ai.agent.sdk.guardrails import InputGuardrail, OutputGuardrail
from ai.agent.sdk.handoffs import Handoff
from ai.agent.sdk.tools import FunctionTool
...
class Agent[TContext]:
    ...
    def _coerce_to_tools(self, item: Any) -> list[BaseTool]:
        ...
        # Raw callable - wrap.
        if callable(item):
            from ai.agent.sdk.tools import tool
    ...
    def as_tool(
        ...
        """
        from ai.agent.sdk.tools import _AgentAsTool
    ...
    def resolve_provider(self) -> Any:
        ...
        3. Otherwise (``self.provider`` is ``None``), fall back to env
           vars and ``self.model``.
        """
        from ai import BaseProvider as _BaseProvider

        if isinstance(self.provider, _BaseProvider):
            return self.provider

        from ai import ApiType, ProviderConfig, get_model, get_provider_class
        from ai.base import resolve_api_key
        ...
        if name is not None and self._is_builtin_provider(name):
            ...
            if info is not None:
                ...
                from ai.providers import resolve_provider_api_type
