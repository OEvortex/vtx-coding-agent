...
from ai.agent.dispatcher import DispatcherContext, get_context
from ai.agent.tools.base import BaseTool
from coding_agent.config import config as vtx_config
from core.types import StopReason, TextContent, ToolResult, Usage

if TYPE_CHECKING:
    from ai.agent.session import Session
    from ai.base import BaseProvider  # noqa: F401

log = logging.getLogger("agent.tools.task")
...
class SubagentSpec:
    ...
    def to_agentdef(self) -> Any:
        ...
        from ai.agent.agents.schema import AgentDef, InstructionsMode, ThinkingLevel
...
def _build_subagent_tool_list(parent_ctx: DispatcherContext, spec: SubagentSpec) -> list[Any]:
    ...
    from ai.agent.tools import DEFAULT_TOOLS, tools_by_name
    ...
    from ai.agent.agents.activate import compose_active_tools
...
def _resolve_api_and_base_url(
    ...
    from ai import get_model, resolve_provider_api_type
    from ai.agent.runtime import default_base_url_for_api, default_base_url_for_provider
    from ai.dynamic_models import find_dynamic_model
...
async def _run_subagent(
    ...
    from ai import get_max_tokens
    from ai.agent.loop import Agent
    from ai.agent.runtime import create_provider
...
class TaskTool(BaseTool[TaskParams]):
    ...
    def ui_block(self) -> type | None:  # type: ignore[override]
        from tui.blocks import TaskToolBlock
    ...
    async def _execute_background(self, params: TaskParams, parent_ctx: Any) -> ToolResult:
        ...
        """
        from ai.agent.tools.background import get_manager
