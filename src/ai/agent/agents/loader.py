...
from ai.agent.agents.api import AgentAPI, LoadedAgent
from ai.agent.agents.schema import AgentDef
from ai.agent.extensions import AGENT_CHANGED
...
def _wrap_callable_as_tool(fn: Callable[..., Any], fallback_name: str) -> BaseTool:
    ...
    from ai.agent.sdk.tools import tool as sdk_tool
    from ai.agent.tools.base import BaseTool
...
def load_agent(path: Path, *, cwd: str, config_dir: Path, on_event: Any = None) -> LoadedAgent:
    ...
    if spec is None or spec.loader is None:
        raise AgentLoadError(f"Could not import ai.agent at {path}")
...
def load_all_agents(
    *,
    ...
    """
    from ai.agent.agents.discovery import find_agent_paths
