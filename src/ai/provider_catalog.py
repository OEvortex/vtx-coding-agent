...
from ai.models import ApiType, Model
...
def detect_provider_from_env() -> ProviderInfo:
    ...
    for slug in order:
        ...
        if slug == "supercode":
            try:
                from ai.oauth.supercode import is_supercode_logged_in
...
def _provider_info_to_model(p: ProviderInfo, model_id: str) -> Model:
    ...
    from ai.context_length import context_length_manager
...
def get_all_catalog_models() -> list[Model]:
    from ai.model_fetcher import get_fetched_models
...
def _sync_dynamic_provider(info: ProviderInfo) -> None:
    ...
    if not info.base_url:
        return
    try:
        from ai.dynamic_models import ApiType as _DynApiType
        from ai.dynamic_models import DynamicProviderConfig, register_dynamic_provider
