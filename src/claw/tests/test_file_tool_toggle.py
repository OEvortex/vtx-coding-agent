...
from vtx_claw.agent.tools.filesystem import FileToolsConfig, ReadFileTool
from vtx_claw.agent.tools.loader import ToolLoader
from vtx_claw.agent.tools.registry import ToolRegistry
from vtx_claw.config.schema import Config, ToolsConfig
...
def test_file_tool_loader_skips_all_builtin_file_tools_when_disabled(tmp_path):
    ...
    ctx = ToolContext(config=cfg, workspace=str(tmp_path), file_state_store=FileStates())
