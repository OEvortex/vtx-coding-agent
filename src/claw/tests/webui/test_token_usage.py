...
from vtx_claw.webui.token_usage import (
    TokenUsageHook,
...
def test_record_token_usage_aggregates_by_local_day(tmp_path, monkeypatch) -> None:
    ...
    record_token_usage(
        ...
        timezone_name="Asia/Shanghai",
        now=datetime(2026, 6, 2, 19, 0, tzinfo=UTC),
    )

    payload = token_usage_payload(
        timezone_name="Asia/Shanghai", now=datetime(2026, 6, 3, 12, 0, tzinfo=UTC)
    )
...
def test_record_token_usage_skips_empty_usage(tmp_path, monkeypatch) -> None:
    ...
    payload = token_usage_payload(now=datetime(2026, 6, 3, tzinfo=UTC))
    assert payload["days"] == []
...
def test_record_response_token_usage_uses_response_usage(tmp_path, monkeypatch) -> None:
    monkeypatch.setattr("vtx_claw.webui.token_usage.get_webui_dir", lambda: tmp_path / "webui")
    monkeypatch.setattr("vtx_claw.webui.token_usage._local_day", lambda *_, **__: "2026-06-03")

    record_response_token_usage(
        SimpleNamespace(usage={"prompt_tokens": 20, "completion_tokens": 5}), source="dream"
    )
