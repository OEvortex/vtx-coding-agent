...
async def test_drain_injections_extracts_content_from_inbound_messages():
    ...
    result = await runner._drain_injections(spec)
    assert result == [{"role": "user", "content": "hello"}, {"role": "user", "content": "world"}]
...
async def test_loop_injected_followup_preserves_image_media(tmp_path):
    ...
    await pending_queue.put(
        InboundMessage(
            channel="cli", sender_id="u", chat_id="c", content="", media=[str(image_path)]
        )
    )
...
async def test_injection_cycles_capped_at_max():
    ...
    from vtx_claw.agent.runner import _MAX_INJECTION_CYCLES, AgentRunner, AgentRunSpec
    from vtx_claw.bus.events import InboundMessage
...
async def test_submitted_cron_turn_reports_pending_until_completed(tmp_path):
    ...
    from vtx_claw.cron.session_turns import CRON_TRIGGER_META
    ...
    response = OutboundMessage(channel="websocket", chat_id="chat-1", content="done")
...
async def test_pending_queue_preserves_overflow_for_next_injection_cycle(tmp_path):
    ...
    from vtx_claw.agent.loop import AgentLoop
    from vtx_claw.agent.runner import _MAX_INJECTIONS_PER_TURN
    from vtx_claw.bus.events import InboundMessage
    from vtx_claw.bus.queue import MessageBus
    ...
    for idx in range(total_followups):
        await pending_queue.put(
            InboundMessage(channel="cli", sender_id="u", chat_id="c", content=f"follow-up-{idx}")
        )
...
async def test_drain_injections_on_llm_error():
    ...
    async def chat_with_retry(*, messages, **kwargs):
        call_count["n"] += 1
        if call_count["n"] == 1:
            return LLMResponse(content=None, tool_calls=[], finish_reason="error", usage={})
        # Second call: respond normally to the injected follow-up
...
async def test_drain_injections_on_empty_final_response():
    ...
    from vtx_claw.agent.runner import _MAX_EMPTY_RETRIES, AgentRunner, AgentRunSpec
    from vtx_claw.bus.events import InboundMessage
...
async def test_drain_injections_on_max_iterations():
    ...
    async def chat_with_retry(*, messages, **kwargs):
        ...
        return LLMResponse(
            ...
            tool_calls=[
                ToolCallRequest(
                    id=f"c{call_count['n']}", name="read_file", arguments={"path": "x"}
                )
            ],
            usage={},
        )
