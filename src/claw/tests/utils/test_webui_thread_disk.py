...
from vtx_claw.webui.thread_disk import delete_webui_thread, webui_thread_file_path
...
def test_delete_webui_thread_removes_legacy_json_and_transcript(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("vtx_claw.webui.transcript._MAX_TRANSCRIPT_FILE_BYTES", 520)
    monkeypatch.setattr("vtx_claw.webui.transcript._TARGET_ACTIVE_TRANSCRIPT_BYTES", 260)
    key = "websocket:k1"
    ...
    for idx in range(1, 5):
        append_transcript_object(
            key, {"event": "user", "chat_id": "k1", "text": f"question {idx} " + ("x" * 24)}
        )
        append_transcript_object(
            key, {"event": "message", "chat_id": "k1", "text": f"answer {idx} " + ("y" * 24)}
        )
