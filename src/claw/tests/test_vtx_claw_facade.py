"""Tests for the VtxClaw programmatic facade."""
...
def test_from_config_missing_file():
    with pytest.raises(FileNotFoundError):
        VtxClaw.from_config("/nonexistent/config.json")


def test_from_config_creates_instance(tmp_path):
    config_path = _write_config(tmp_path)
    bot = VtxClaw.from_config(config_path, workspace=tmp_path)
...
def test_from_config_accepts_default_model_override(tmp_path):
    ...
    bot = VtxClaw.from_config(config_path, workspace=tmp_path, model="openai/gpt-4.1-mini")
...
def test_from_config_accepts_default_model_preset(tmp_path):
    config_path = _write_config(
        tmp_path,
        {"modelPresets": {"fast": {"model": "openai/gpt-4.1-mini", "provider": "openrouter"}}},
    )

    bot = VtxClaw.from_config(config_path, workspace=tmp_path, model_preset="fast")
...
def test_from_config_rejects_multiple_model_selectors(tmp_path):
    ...
    with pytest.raises(ValueError, match="mutually exclusive"):
        VtxClaw.from_config(
            config_path, workspace=tmp_path, model="openai/gpt-4.1", model_preset="fast"
        )
...
def test_from_config_default_path():
    ...
    with (
        patch("vtx_claw.config.loader.load_config") as mock_load,
        patch("vtx_claw.providers.factory.make_provider") as mock_prov,
    ):
...
async def test_run_returns_result(tmp_path):
    ...
    bot._loop.process_direct.assert_awaited_once_with("hi", session_key="sdk:default", hooks=ANY)


@pytest.mark.asyncio
async def test_run_with_hooks(tmp_path):
    from vtx_claw.agent.hook import AgentHook, AgentHookContext, SDKCaptureHook
    from vtx_claw.bus.events import OutboundMessage
...
def test_workspace_override(tmp_path):
    ...
    bot = VtxClaw.from_config(config_path, workspace=custom_ws)
...
async def test_run_custom_session_key(tmp_path):
    ...
    bot._loop.process_direct.assert_awaited_once_with("hi", session_key="user-alice", hooks=ANY)


def test_import_from_top_level():
    import vtx_claw

    assert vtx_claw.VtxClaw is VtxClaw
    assert vtx_claw.RunResult is RunResult
    assert vtx_claw.RunStream is RunStream
    assert vtx_claw.SessionInfo is SessionInfo
    assert vtx_claw.SessionSnapshot is SessionSnapshot
    assert vtx_claw.StreamEvent is StreamEvent
    assert vtx_claw.StreamEventType is StreamEventType
    assert vtx_claw.STREAM_EVENT_TEXT_DELTA == STREAM_EVENT_TEXT_DELTA
    assert vtx_claw.STREAM_EVENT_RUN_COMPLETED == STREAM_EVENT_RUN_COMPLETED
    assert vtx_claw.STREAM_EVENT_TYPES == STREAM_EVENT_TYPES
...
async def test_run_populates_observability_fields(tmp_path):
    from vtx_claw.agent.hook import AgentRunHookContext
    from vtx_claw.bus.events import OutboundMessage
    ...
    async def fake_process_direct(message, *, session_key, hooks):
        ...
        return OutboundMessage(
            channel="cli", chat_id="direct", content="done", metadata={"latency_ms": 42}
        )
...
async def test_run_ephemeral_still_captures_runner_observability(tmp_path):
    ...
    provider.chat_with_retry = AsyncMock(
        return_value=LLMResponse(content="done", tool_calls=[], usage={"total_tokens": 3})
    )
    bot = VtxClaw(
        AgentLoop(bus=MessageBus(), provider=provider, workspace=tmp_path, model="test-model")
    )
...
async def test_run_allows_parallel_sessions_without_model_override(tmp_path):
    ...
    left, right = await asyncio.gather(
        bot.run("left", session_key="sdk:left"), bot.run("right", session_key="sdk:right")
    )
...
async def test_run_model_override_is_per_run_and_restores_default(tmp_path):
    ...
    bot._runtime_overrides.model_override_snapshot.assert_called_once_with(
        model="openai/gpt-4.1-mini", model_preset=None
    )
...
async def test_concurrent_run_hooks_are_isolated_per_call(tmp_path):
    ...
    alpha, beta = await asyncio.gather(
        bot.run("alpha", hooks=[UserHook("alpha")]), bot.run("beta", hooks=[UserHook("beta")])
    )
...
async def test_run_streamed_wait_returns_full_result_without_consuming_events(tmp_path):
    ...
    async def fake_process_direct(message, *, session_key, on_stream, on_stream_end, hooks):
        ...
        return OutboundMessage(
            channel="cli", chat_id="direct", content="done", metadata={"latency_ms": 5}
        )
...
async def test_stream_rejects_multiple_model_selectors(tmp_path):
    ...
    with pytest.raises(ValueError, match="mutually exclusive"):
        _ = [
            event async for event in bot.stream("hi", model="openai/gpt-4.1", model_preset="fast")
        ]
...
async def test_sdk_capture_prefers_run_level_snapshot():
    from vtx_claw.agent.hook import AgentHookContext, AgentRunHookContext, SDKCaptureHook
    from vtx_claw.providers.base import ToolCallRequest
