...
def test_exec_accepts_command_aliases(tmp_path):
    async def run() -> str:
        ...
        return await tool.execute(
            cmd=_python_command("import os; print(os.getcwd())"), workdir=str(tmp_path)
        )
...
def test_exec_session_accepts_max_output_tokens_alias(tmp_path):
    async def run() -> str:
        ...
        return await tool.execute(command=command, yield_time_ms=1000, max_output_tokens=1000)
...
def test_write_stdin_can_close_stdin(tmp_path):
    async def run() -> tuple[str, str]:
        ...
        result = await stdin_tool.execute(
            session_id=sid, chars="payload", close_stdin=True, yield_time_ms=1500
        )
        return initial, result
...
def test_write_stdin_can_terminate_session(tmp_path):
    async def run() -> tuple[str, str]:
        ...
        waited = await stdin_tool.execute(
            session_id=sid, wait_for="ready", wait_timeout_ms=3000, yield_time_ms=0
        )
        result = await stdin_tool.execute(session_id=sid, terminate=True, yield_time_ms=0)
...
def test_write_stdin_accepts_max_output_tokens_alias(tmp_path):
    async def run() -> tuple[str, str, str]:
        ...
        sid = _session_id(initial)
        poll = await stdin_tool.execute(session_id=sid, yield_time_ms=500, max_output_tokens=1000)
...
def test_write_stdin_wait_for_reports_timeout_without_killing_session(tmp_path):
    async def run() -> tuple[str, str, str]:
        ...
        sid = _session_id(initial)
        waited = await stdin_tool.execute(
            session_id=sid, wait_for="never-ready", wait_timeout_ms=200, yield_time_ms=0
        )
        cleanup = await stdin_tool.execute(session_id=sid, terminate=True, yield_time_ms=0)
        return initial, waited, cleanup
...
def test_exec_session_mode_reuses_exec_safety_guard(tmp_path):
    manager = ExecSessionManager()
    tool = ExecTool(
        working_dir=str(tmp_path), deny_patterns=[r"echo\s+blocked"], session_manager=manager
    )
