...
import vtx_claw.agent.tools.mcp as mcp_mod
from vtx_claw.agent.tools.mcp import (
    MCPPromptWrapper,
    MCPResourceWrapper,
    MCPToolWrapper,
...
def _fake_mcp_module(
    ...
    class _FakeStdioServerParameters:
        def __init__(
            self, command: str, args: list[str], env: dict | None = None, cwd: str | None = None
        ) -> None:
...
def test_wrapper_normalizes_nullable_property_type_union() -> None:
    tool_def = SimpleNamespace(
        name="demo",
        description="demo tool",
        inputSchema={"type": "object", "properties": {"name": {"type": ["string", "null"]}}},
    )
...
def test_normalize_windows_stdio_command_is_noop_off_windows(
    ...
    command, args, env = _normalize_windows_stdio_command(
        "npx", ["-y", "chrome-devtools-mcp@latest"], {"FOO": "bar"}
    )
...
def test_normalize_windows_stdio_command_wraps_npx_on_windows(
    ...
    monkeypatch.setattr(
        mcp_mod.shutil, "which", lambda command, path=None: r"C:\Program Files\nodejs\npx.cmd"
    )
    monkeypatch.setenv("COMSPEC", r"C:\Windows\System32\cmd.exe")

    command, args, env = _normalize_windows_stdio_command(
        "npx", ["-y", "chrome-devtools-mcp@latest"], None
    )
...
def test_normalize_windows_stdio_command_skips_existing_shells(
    ...
    command, args, env = _normalize_windows_stdio_command("cmd.exe", ["/c", "echo", "hello"], None)
...
async def test_execute_persists_image_block_as_artifact(tmp_path: Path) -> None:
    ...
    async def call_tool(_name: str, arguments: dict) -> object:
        return SimpleNamespace(
            content=[_FakeTextContent("here you go"), _FakeImageContent(_PNG_B64, "image/png")]
        )
...
async def test_connect_mcp_servers_enabled_tools_supports_raw_names(
    ...
    stacks = await connect_mcp_servers(
        {"test": MCPServerConfig(command="fake", enabled_tools=["demo"])}, registry
    )
...
async def test_connect_mcp_servers_enabled_tools_supports_wrapped_names(
    ...
    registry = ToolRegistry()
    stacks = await connect_mcp_servers(
        {"test": MCPServerConfig(command="fake", enabled_tools=["mcp_test_demo"])}, registry
    )
    for stack in stacks.values():
        await stack.aclose()
...
async def test_connect_mcp_servers_enabled_tools_empty_list_registers_none(
    ...
    registry = ToolRegistry()
    stacks = await connect_mcp_servers(
        {"test": MCPServerConfig(command="fake", enabled_tools=[])}, registry
    )
    for stack in stacks.values():
        await stack.aclose()
...
async def test_connect_mcp_servers_enabled_tools_empty_list_blocks_resources_and_prompts(
    ...
    fake_mcp_runtime["session"] = _make_fake_session_with_capabilities(
        tool_names=["demo"], resource_names=["secret_data"], prompt_names=["admin_prompt"]
    )
    registry = ToolRegistry()
    stacks = await connect_mcp_servers(
        {"test": MCPServerConfig(command="fake", enabled_tools=[])}, registry
...
async def test_connect_mcp_servers_enabled_tools_specific_list_blocks_resources_and_prompts(
    ...
    fake_mcp_runtime["session"] = _make_fake_session_with_capabilities(
        tool_names=["demo", "other"], resource_names=["secret_data"], prompt_names=["admin_prompt"]
    )
    registry = ToolRegistry()
    stacks = await connect_mcp_servers(
        {"test": MCPServerConfig(command="fake", enabled_tools=["demo"])}, registry
...
async def test_connect_mcp_servers_enabled_tools_wildcard_allows_resources_and_prompts(
    ...
    fake_mcp_runtime["session"] = _make_fake_session_with_capabilities(
        tool_names=["demo"], resource_names=["public_data"], prompt_names=["help_prompt"]
    )
    registry = ToolRegistry()
    stacks = await connect_mcp_servers(
        {"test": MCPServerConfig(command="fake", enabled_tools=["*"])}, registry
...
async def test_connect_mcp_servers_enabled_tools_warns_on_unknown_entries(
    ...
    monkeypatch.setattr("vtx_claw.agent.tools.mcp.logger.warning", _warning)

    stacks = await connect_mcp_servers(
        {"test": MCPServerConfig(command="fake", enabled_tools=["unknown"])}, registry
    )
    for stack in stacks.values():
        await stack.aclose()
...
async def test_connect_mcp_servers_logs_stdio_pollution_hint(
    ...
    monkeypatch.setattr("vtx_claw.agent.tools.mcp.logger.exception", _error)
...
async def test_connect_mcp_servers_rejects_unsafe_http_urls_before_probe(
    config: MCPServerConfig, monkeypatch: pytest.MonkeyPatch
) -> None:
...
async def test_connect_mcp_servers_http_clients_reject_unsafe_redirect_targets(
    config: MCPServerConfig, expected_transport: str, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    assert checked_urls == [config.url, "https://example.com/start", "http://127.0.0.1/private"]
...
async def test_connect_mcp_servers_one_failure_does_not_block_others(
    ...
    registry = ToolRegistry()
    stacks = await connect_mcp_servers(
        {"good": MCPServerConfig(command="good"), "bad": MCPServerConfig(command="bad")}, registry
    )
    for stack in stacks.values():
        await stack.aclose()
...
async def test_connect_mcp_servers_streamable_http_uses_finite_timeout(
    ...
    registry = ToolRegistry()
    stacks = await connect_mcp_servers(
        {"test": MCPServerConfig(url="https://mcp.example.com/mcp")}, registry
    )
    for stack in stacks.values():
        await stack.aclose()
...
async def test_connect_mcp_servers_wraps_windows_stdio_launchers(
    ...
    registry = ToolRegistry()
    stacks = await connect_mcp_servers(
        {"test": MCPServerConfig(command="npx", args=["-y", "chrome-devtools-mcp@latest"])},
        registry,
    )
    for stack in stacks.values():
        await stack.aclose()
...
async def test_connect_mcp_servers_passes_stdio_cwd(
    ...
    registry = ToolRegistry()
    stacks = await connect_mcp_servers(
        {"test": MCPServerConfig(command="fake", cwd="/tmp/vtx_claw-mcp-test")}, registry
    )
    for stack in stacks.values():
        await stack.aclose()

    assert captured["cwd"] == "/tmp/vtx_claw-mcp-test"
...
def _make_resource_def(
    name: str = "myres", uri: str = "file:///tmp/data.txt", description: str = "A test resource"
) -> SimpleNamespace:
...
def _make_prompt_def(
    name: str = "myprompt", description: str = "A test prompt", arguments: list | None = None
) -> SimpleNamespace:
...
async def test_prompt_wrapper_execute_returns_text() -> None:
    async def get_prompt(name: str, arguments: dict | None = None) -> object:
        ...
        msg1 = SimpleNamespace(
            role="user", content=[_FakeTextContent("You are an expert on {{topic}}.")]
        )
        msg2 = SimpleNamespace(
            role="assistant", content=[_FakeTextContent("Understood. Ask me anything.")]
        )
...
def _make_fake_session_with_capabilities(
    ...
    async def list_resources() -> SimpleNamespace:
        ...
        for rname in resource_names or []:
            resources.append(
                SimpleNamespace(
                    name=rname, uri=f"file:///{rname}", description=f"{rname} resource"
                )
            )
    ...
    async def list_prompts() -> SimpleNamespace:
        ...
        for pname in prompt_names or []:
            prompts.append(
                SimpleNamespace(name=pname, description=f"{pname} prompt", arguments=None)
            )
...
async def test_connect_registers_resources_and_prompts(
    ...
    fake_mcp_runtime["session"] = _make_fake_session_with_capabilities(
        tool_names=["tool_a"], resource_names=["res_b"], prompt_names=["prompt_c"]
    )
    registry = ToolRegistry()
    stacks = await connect_mcp_servers({"test": MCPServerConfig(command="fake")}, registry)
    for stack in stacks.values():
...
def test_resource_wrapper_sanitizes_name() -> None:
    resource_def = SimpleNamespace(
        name="PostgreSQL System Information", uri="file:///pg/info", description="PG info"
    )
...
def test_prompt_wrapper_sanitizes_name() -> None:
    prompt_def = SimpleNamespace(name="design-schema", description="Design schema", arguments=None)
...
async def test_connect_mcp_servers_sanitizes_resource_names(
    ...
    fake_mcp_runtime["session"] = _make_fake_session_with_capabilities(
        tool_names=[], resource_names=["PostgreSQL System Information"], prompt_names=[]
    )
    registry = ToolRegistry()
    stacks = await connect_mcp_servers({"test": MCPServerConfig(command="fake")}, registry)
    for stack in stacks.values():
...
async def test_connect_mcp_servers_enabled_tools_matches_sanitized_name(
    ...
    registry = ToolRegistry()
    stacks = await connect_mcp_servers(
        {"test": MCPServerConfig(command="fake", enabled_tools=["mcp_test_My_Tool"])}, registry
    )
    for stack in stacks.values():
        await stack.aclose()
