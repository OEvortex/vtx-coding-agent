...
export default defineConfig(({ mode }) => {
  ...
  const target = env.VTX_CLAW_API_URL ?? "http://127.0.0.1:8765";
  const hmrPath = "/__vtx-claw_vite_hmr";
  ...
  return {
    ...
    server: {
      ...
      port: 5173,
      strictPort: true,
      // Keep Vite's HMR socket on a dedicated path. VtxClaw's app WebSocket is
