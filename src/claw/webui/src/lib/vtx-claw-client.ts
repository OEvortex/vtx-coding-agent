...
const WS_OPEN = 1;
const WS_CLOSING = 2;
const HOST_SOCKET_URL_PREFIX = "vtx-claw-host://";
 ...
 * - **Production**: off unless ``localStorage.setItem('vtx-claw_debug_ws','1')`` (or ``true``).
 * - **Silence anywhere**: ``localStorage.setItem('vtx-claw_debug_ws','0')`` (or ``false`` / ``off``).
...
function wsInboundDebugEnabled(): boolean {
  ...
  try {
    ...
    const raw = ls?.getItem("vtx-claw_debug_ws")?.trim().toLowerCase() ?? "";
...
export interface VtxClawClientOptions {
  url: string;
  reconnect?: boolean;
...
export class VtxClawClient {
  ...
  constructor(private options: VtxClawClientOptions) {
  ...
  private handleMessage(ev: MessageEvent): void {
    ...
    try {
      ...
      if (wsInboundDebugEnabled()) {
        ...
        console.warn(
          "[vtx-claw ws inbound] invalid JSON",
    ...
    if (wsInboundDebugEnabled()) {
      console.log("[vtx-claw ws inbound]", summarizeInboundWsPayload(parsed));
    }
  ...
  private dispatch(chatId: string, ev: InboundEvent): void {
    ...
    q.push(ev);
    const over = q.length - VtxClawClient.PENDING_INBOUND_MAX;
    if (over > 0) {
      q.splice(0, over);
    }
  }
