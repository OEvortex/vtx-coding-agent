...
vi.mock("@/lib/vtx-claw-client", () => {
  ...
  return { VtxClawClient: MockClient };
});
...
describe("App layout", () => {
  beforeEach(async () => {
    ...
    localStorage.removeItem("vtx-claw-webui.sidebar");
    localStorage.removeItem("vtx-claw-webui.sidebar.completed-runs.v1");
    localStorage.removeItem("vtx-claw-webui.sidebar.session-updates.v1");
  ...
  it("opens Skills from the main sidebar", async () => {
    ...
    expect(document.title).toBe("Skills · vtx-claw");
  ...
  it("opens Automations from the main sidebar", async () => {
    ...
    expect(document.title).toBe("Automations · vtx-claw");
  ...
  it("edits a past one-time automation without resubmitting its old schedule", async () => {
    ...
    expect(JSON.parse(decodeURIComponent(headers["X-VtxClaw-Automation-Values"]))).toEqual({
      name: "Past one-shot",
  ...
  it("localizes the Automations surface", async () => {
    ...
    expect(document.title).toBe("自动任务 · vtx-claw");
  });
  ...
  it("restores sidebar run indicators after a page reload", async () => {
    ...
    localStorage.setItem(
      "vtx-claw-webui.sidebar.session-updates.v1",
  ...
  it("restores the active chat from the URL hash after a page reload", async () => {
    ...
    await waitFor(() => expect(connectSpy).toHaveBeenCalled());
    await waitFor(() => expect(document.title).toBe("Active after reload · vtx-claw"));
    const sidebar = screen.getByRole("navigation", { name: "Sidebar navigation" });
    expect(
  ...
  it("opens the settings view from the sidebar footer", async () => {
    ...
    expect(document.title).toBe("Settings · vtx-claw");
    ...
    expect(screen.queryByTestId("overview-logo-vtx-claw-gateway")).not.toBeInTheDocument();
    expect(screen.queryByTestId("overview-logo-vtx-claw-workspace")).not.toBeInTheDocument();
  ...
  it("opens Apps from the main sidebar without replacing the sidebar", async () => {
    ...
    expect(document.title).toBe("Apps · vtx-claw");
  });
  ...
  it("returns from settings to the blank start page when no session was active", async () => {
    ...
    await waitFor(() => expect(document.title).toBe("vtx-claw"));
