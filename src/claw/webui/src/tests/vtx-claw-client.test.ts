...
/**
 * Minimal fake WebSocket implementing the subset VtxClawClient touches.
...
describe("VtxClawClient", () => {
  ...
  it("can swap the socket factory when the runtime URL changes", () => {
    ...
    client.close();
    client.updateUrl("vtx-claw-host://engine/", hostFactory);
    client.connect();

    expect(hostFactory).toHaveBeenCalledWith("vtx-claw-host://engine/");
    expect(lastSocket().url).toBe("host:vtx-claw-host://engine/");
  });
  ...
  it("uses the host socket bridge for native host URLs", async () => {
    ...
    expect(openSocket).toHaveBeenCalledWith("vtx-claw-host://engine/");
