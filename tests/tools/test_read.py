...
async def test_read(read_tool, text_file, monkeypatch):
    monkeypatch.setattr("ai.agent.tools.read.MAX_LINES_PER_FILE", 5)
    monkeypatch.setattr("ai.agent.tools.read.MAX_CHARS_PER_LINE", 10)
...
async def test_read_directory_uses_fd_with_depth_3_when_small(read_tool, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("ai.agent.tools.read.ensure_tool", mock_ensure_tool)
