...
from vtx.ai.agent.extension_manager import (
    InstalledExtension,
...
def test_installed_extensions_default_empty():
    ...
    with patch("vtx.ai.agent.extension_manager._get_installed_path") as mock_path:
...
def test_save_and_load_installed(tmp_path: Path):
    ...
    with patch("vtx.ai.agent.extension_manager._get_installed_path", return_value=fake_path):
        _save_installed(
            {
...
def test_get_installed_extension_missing():
    with patch("vtx.ai.agent.extension_manager._load_installed", return_value={}):
