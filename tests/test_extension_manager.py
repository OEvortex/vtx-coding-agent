...
from agent.extension_manager import (
    InstalledExtension,
...
def test_installed_extensions_default_empty():
    ...
    with patch("agent.extension_manager._get_installed_path") as mock_path:
...
def test_save_and_load_installed(tmp_path: Path):
    ...
    with patch("agent.extension_manager._get_installed_path", return_value=fake_path):
        _save_installed(
            {
...
def test_get_installed_extension_missing():
    with patch("agent.extension_manager._load_installed", return_value={}):
...
def test_discover_entry_points(tmp_path: Path):
    ...
    entry_points = (
        '[project.entry-points."agent.extensions"]\n'
        'crs = "vtx_crs.ext:register"\n\n'
        '[project.entry-points."agent.agents"]\n'
