...
""")
        ...
        monkeypatch.setattr("agent.context.skills.get_user_skills_dir", lambda: global_dir)
...
""")
        ...
        monkeypatch.setattr(
            "agent.context.skills.get_user_skills_dir", lambda: home_dir / ".agents"
        )
...
class TestBuiltinCommandSkills:
    ...
    def test_loads_builtin_github_skill(self):
        ...
        skill = next((s for s in result.skills if s.name == "github"), None)
        assert skill is not None
        assert skill.register_cmd is True
        assert skill.bundled is True
        assert skill.path.replace("\\", "/").endswith(
...
class TestFormatSkillsForPrompt:
    ...
    def test_groups_skills_by_category(self):
        skills = [
            Skill(
                name="alpha", description="Alpha desc", path="/a/SKILL.md", category="workflows"
            ),
            Skill(name="beta", description="Beta desc", path="/b/SKILL.md", category="review"),
        ]
