...
from agent.agents.schema import AGENT_NAME_RE
from agent.extensions import AGENT_START, EventBus, ExtensionCommand, ExtensionTool
...
def test_compose_active_tools_local_tools_bypass_allow_deny():
    ...
    from agent.extensions import _json_schema_to_pydantic
...
def test_compose_active_commands_merges_local():
    from agent.extensions import CommandOutcome
...
def test_when_predicate_matches_substring():
    from agent.agents.api import _when_predicate
...
def test_extension_register_local_tool(tmp_path: Path):
    ...
    from agent.extensions import load_extension
...
def test_tool_group_changed_event_in_all_events():
    from agent.extensions import ALL_EVENTS, TOOL_GROUP_CHANGED
